...
describe('computeOrderBlockStatus — 우선순위 8단계 (세션 9)', () => {
  ...
  describe('0순위 — 휴장일 (최우선, 장 자체 미개장)', () => {
    it('buy: marketPhase.krx=휴장일 시 휴장일 — 매수 중지 표시 (holiday=true)', () => {
      const ui = makeCleanUiState()
      ui.marketPhase = { ...ui.marketPhase, krx: '휴장일', nxt: '휴장일' }
      const r = computeOrderBlockStatus('buy', ui, makeCleanSettings())
      expect(r.blocked).toBe(true)
      expect(r.holiday).toBe(true)
      expect(r.text).toBe('휴장일 — 매수 중지')
    })

    it('sell: marketPhase.nxt=휴장일 시 휴장일 — 매도 중지 표시 (holiday=true)', () => {
      const ui = makeCleanUiState()
      ui.marketPhase = { ...ui.marketPhase, krx: '휴장일', nxt: '휴장일' }
      const r = computeOrderBlockStatus('sell', ui, makeCleanSettings())
      expect(r.blocked).toBe(true)
      expect(r.holiday).toBe(true)
      expect(r.text).toBe('휴장일 — 매도 중지')
    })

    it('휴장일은 서킷브레이커/리스크/자동매매 OFF 보다 우선 (장 안 열리면 매매 의미 없음)', () => {
      const ui = makeCleanUiState()
      ui.marketPhase = { ...ui.marketPhase, krx: '휴장일', nxt: '휴장일' }
      ui.circuitBreakerOpen = { message: '서킷브레이커' }  // 1순위
      ui.realtimeLatencyExceeded = true                    // 2순위
      ui.riskBlockStatus = { side: 'buy', reason: '손실한도' }  // 3순위
      const settings = makeCleanSettings()
      settings.auto_buy_on = false                          // 7순위
      const r = computeOrderBlockStatus('buy', ui, settings)
      expect(r.holiday).toBe(true)
      expect(r.text).toBe('휴장일 — 매수 중지')
    })

    it('KRX만 휴장일이어도 휴장일 판정 (양쪽 중 하나라도 휴장일)', () => {
      const ui = makeCleanUiState()
      ui.marketPhase = { ...ui.marketPhase, krx: '휴장일', nxt: '정규장' }
      const r = computeOrderBlockStatus('buy', ui, makeCleanSettings())
      expect(r.holiday).toBe(true)
      expect(r.text).toBe('휴장일 — 매수 중지')
    })
  ...
  describe('3순위 — 리스크 차단 (side 일치 시에만)', () => {
    it('buy: riskBlockStatus.side=buy 시 buy 화면에 리스크 사유 표시', () => {
      const ui = makeCleanUiState()
      ui.riskBlockStatus = { side: 'buy', reason: '코스피 급락 차단 · 코스닥 매수 가능', partial: true }
      const r = computeOrderBlockStatus('buy', ui, makeCleanSettings())
      expect(r.blocked).toBe(true)
      expect(r.partial).toBe(true)
      expect(r.text).toBe('차단: 리스크(코스피 급락 차단 · 코스닥 매수 가능)')
    })
