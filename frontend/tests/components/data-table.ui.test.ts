...
describe('createDataTable — virtual scroll mode', () => {
  ...
  it('keeps the virtual scroll viewport constrained for large datasets', () => {
    const table = createDataTable({
      columns: COLUMNS,
      virtualScroll: true,
      keyFn: (row) => row.name,
    ...
    document.body.appendChild(table.el)
    const rows = Array.from({ length: 1339 }, (_, index) => ({
      name: `종목-${index}`,
      price: index,
      change: 0,
    }))
    ...
    const sentinel = table.el.querySelector('[data-vs-sentinel]')
    const scrollContainer = sentinel?.parentElement
    const renderedRows = sentinel?.querySelectorAll('[data-row-type="data"]') ?? []
