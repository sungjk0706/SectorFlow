...
export type ProfitDateRangeMode = 'default' | 'manual' | 'quick'
...
// '전일'은 quickRange에서 제거됨 (다단계 1세션 결정 1) — 레거시 '전일'/'직전'은 마이그레이션 시 제거 대상.
const LEGACY_QUICK_LABEL_MAP: Record<string, string | undefined> = {
  '전체': '누적',
  '직전': undefined,
  '전일': undefined,
}
...
function loadProfitDateRange(): ProfitDateRange | null {
  try {
    ...
    if (!raw) return null
    const parsed = JSON.parse(raw) as { from?: string; to?: string; quickLabel?: string; mode?: ProfitDateRangeMode }
    // quickLabel이 있는 경우(5거래일/누적 등) from/to가 빈 문자열일 수 있음
    if (parsed.quickLabel) {
      // 레거시 라벨 마이그레이션 (전체→누적, 직전/전일→제거) — 한 번 치환 후 영구 저장
      const migrated = LEGACY_QUICK_LABEL_MAP[parsed.quickLabel] ?? parsed.quickLabel
      if (migrated === undefined) {
        // '전일'/'직전'은 제거됨 — quickLabel 없이 from/to만 저장
        saveProfitDateRange(parsed.from ?? '', parsed.to ?? '', undefined, 'manual')
        return { from: parsed.from ?? '', to: parsed.to ?? '', mode: 'manual' }
      }
      if (migrated !== parsed.quickLabel) {
        saveProfitDateRange(parsed.from ?? '', parsed.to ?? '', migrated, 'quick')
      }
      return { from: parsed.from ?? '', to: parsed.to ?? '', quickLabel: migrated, mode: 'quick' }
    }
    // 저장 방식이 기록된 기본 기간은 현재 거래일 기준으로 다시 계산한다.
    if (parsed.mode === 'default') {
      return { from: parsed.from ?? '', to: parsed.to ?? '', mode: 'default' }
    }
    ...
    if (parsed.from && parsed.to && /^\d{4}-\d{2}-\d{2}$/.test(parsed.from) && /^\d{4}-\d{2}-\d{2}$/.test(parsed.to) && parsed.from <= parsed.to) {
      return { from: parsed.from, to: parsed.to, mode: parsed.mode === 'quick' ? 'quick' : 'manual' }
    }
    return null
  } catch (e) {
...
export function saveProfitDateRange(
  from: string,
  to: string,
  quickLabel?: string,
  mode: ProfitDateRangeMode = quickLabel ? 'quick' : 'manual',
): void {
  try {
    localStorage.setItem(PROFIT_DATE_KEY, JSON.stringify({ from, to, quickLabel, mode }))
  } catch (e) {
...
export function initDateRange(): { saved: ProfitDateRange | null; from: string; to: string } {
  ...
  if (saved?.mode === 'default') {
    const { from, to } = defaultDateRange()
    saveProfitDateRange(from, to, undefined, 'default')
    return { saved: { ...saved, from, to }, from, to }
  }
  if (saved) {
