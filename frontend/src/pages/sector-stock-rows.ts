...
export function computeRows(
  ...
  for (const sector of orderedSectors) {
    ...
    rows.push({
      type: 'group',
      sector,
      // label은 업종명만 담고 순위는 rank 필드로 분리 — 그룹행 렌더링 시 순위를 고정폭 영역에
      // 배치해 순위 변동(1자리↔2자리)에도 업종명·점수 위치가 흔들리지 않도록 한다.
      label: sector,
      rank: sectorRankMap.get(sector) ?? 0,
      score,
      opacity,
      bgColor,
    })
