// frontend/src/pages/sector-stock.ui.ts
// 업종별 종목 실시간 시세 — 순수 UI 껍데기 (Dumb Component)
...
export interface SectorStockProps {
  // 종목 데이터
...
function computeRows(
  ...
  for (const sector of orderedSectors) {
    ...
    // selectedSector 모드: 종목코드 기준 안정 정렬
...
export function createSectorStockCard(props: SectorStockProps): { el: HTMLElement; update: (newProps: SectorStockProps) => void; destroy: () => void } {
  let rootEl: HTMLElement | null = document.createElement('div')
  ...
  const titleH3 = createCardTitleWithContent(titleContent)
  ...
  clearBtn.addEventListener('click', () => props.onClearSectorFilter())
  ...
  function buildRows(): RowItem[] {
    currentMatchedCodes = filterStocksBySearch(Object.values(props.sectorStocks), searchTerm)
    return computeRows(
      props.sectorStocks,
      props.sectorOrder,
      props.sectorScores,
  ...
  function updateUI(rows: RowItem[]): void {
    const stockCount = Object.keys(props.sectorStocks).length

    // 타이틀 갱신
    if (titleFilterSpan && titleCountSpan) {
      if (props.minTradeAmt > 0) {
        titleFilterSpan.textContent = `5일평균최소거래대금(${props.minTradeAmt})억`
    ...
    // 업종 필터 배지
    if (filterBadge) {
      const selected = props.selectedSector
      if (selected) {
    ...
    // WS 상태 배지
    if (wsBadge) {
      wsBadge.update(props.wsSubscribed, 'kiwoom')
    }
  ...
  // Props 업데이트 함수
  function update(newProps: SectorStockProps): void {
