...
import { virtualScrollOptions, TABLE_DISPLAY_VISIBLE, TABLE_DISPLAY_HIDDEN } from '../components/common/table-options'
...
/* ── 하단 통계 카드 라벨 접두사 동기화 (상단 선택 기간과 일관성 — P23 용어 통일) ──
...
// 'YYYY-MM-DD' → 'M/D' (월/일, 선행 0 제거). 빈 문자열이면 빈 문자열 반환.
function toShortDate(iso: string): string {
  if (!iso || iso.length < 10) return ''
  const [, m, d] = iso.slice(0, 10).split('-')
  return `${Number(m)}/${Number(d)}`
}
function buildCustomRangePrefix(state: ProfitDetailState): string {
  const range = state.dateRangeInput?.getValue() ?? { from: '', to: '' }
  const from = toShortDate(range.from)
  const to = toShortDate(range.to)
  if (!from && !to) return ''
  if (from === to) return from
  return `${from}~${to}`
}
function updateStatLabels(state: ProfitDetailState): void {
  const prefix = state.selectedView ? VIEW_PREFIX[state.selectedView] : buildCustomRangePrefix(state)
  for (let i = 0; i < state.statLabelEls.length; i++) {
    const base = STAT_BASE_LABELS[i] ?? ''
    state.statLabelEls[i].textContent = prefix ? `${prefix} ${base}` : base
  }
}
...
export function showTable(state: ProfitDetailState): void {
  ...
  state.tableViewContainer.style.display = 'flex'
  ...
  if (!state.sellTable) {
    state.sellTable = createDataTable<Record<string, unknown>>(
      virtualScrollOptions<Record<string, unknown>>({
        columns: SELL_COLS,
  ...
  state.sellTable.el.style.display = isSell ? TABLE_DISPLAY_VISIBLE : TABLE_DISPLAY_HIDDEN
  state.buyTable.el.style.display = isSell ? TABLE_DISPLAY_HIDDEN : TABLE_DISPLAY_VISIBLE
