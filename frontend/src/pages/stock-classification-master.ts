...
import { fixedTableOptions, TABLE_DISPLAY_VISIBLE, TABLE_DISPLAY_HIDDEN } from '../components/common/table-options'
...
function handleSearchQuery(state: StockClassificationPageState, query: string): void {
  ...
  if (!query) {
    state.searchResultTableRef.el.style.display = TABLE_DISPLAY_HIDDEN
    state.masterTableRef.el.style.display = TABLE_DISPLAY_VISIBLE
    return
  }
  ...
  state.searchResultTableRef.el.style.display = TABLE_DISPLAY_VISIBLE
  state.masterTableRef.el.style.display = TABLE_DISPLAY_HIDDEN
}
...
function buildSearchResultTable(state: StockClassificationPageState): HTMLElement {
  ...
  state.searchResultTableRef = createDataTable<SearchResultRow>(
    fixedTableOptions<SearchResultRow>({
