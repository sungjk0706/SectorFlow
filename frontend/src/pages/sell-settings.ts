...
import { createSettingRow, createNumInput, createToggleBtn, createFixedValue, createSettingToggleRow } from '../components/common/setting-row'
...
function buildSellTypeSection(root: HTMLElement): void {
  ...
  // 익절 (토글 + 입력)
  tpValInput = createNumInput({ value: 0, onChange: v => { const orig = Number(vals.tp_val); vals.tp_val = v; saveHelper!.autoSave('tp_val', v, () => { vals.tp_val = orig; tpValInput!.setValue(orig) }) }, step: 0.1, min: 0, max: 100, suffix: '%', name: 'tp_val' })
  {
  ...
  lossValInput = createNumInput({ value: 0, onChange: v => { const orig = Number(vals.loss_val); vals.loss_val = v; saveHelper!.autoSave('loss_val', v, () => { vals.loss_val = orig; lossValInput!.setValue(orig) }) }, step: 0.1, min: -100, max: 0, suffix: '%', name: 'loss_val' })
  {
    const r = createSettingToggleRow({
      label: '손절 (하락률)',
