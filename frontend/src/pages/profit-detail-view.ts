...
export function loadProfitDetailView(): ProfitDetailViewState | null {
  try {
    ...
    const validViews: string[] = ['today', 'today_intraday', 'fiveday', 'month', 'total']
    const sv = parsed.selectedView ?? null
    if (sv !== null && !validViews.includes(sv)) return null
    // total은 from/to가 빈 문자열일 수 있음
    const from = parsed.from ?? ''
    const to = parsed.to ?? ''
    // 수동 날짜 범위(sv === null) 또는 today/today_intraday/fiveday/month인 경우 from/to 유효성 검증
    if (sv === null || sv === 'today' || sv === 'today_intraday' || sv === 'fiveday' || sv === 'month') {
