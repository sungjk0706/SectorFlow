...
export function applyRealtimeReset(): void {
  hotStore.setState((state) => {
    ...
    // masterStocks: 백엔드 _REALTIME_FIELDS 8개와 동일 (P10 SSOT, P23 일관성)
    // cur_price/change/change_rate/trade_amount/strength/order_ratio/program_net_buy/news_boost
    const masterStocks: Record<string, MasterStock> = {}
    let masterChanged = false
    for (const [code, stock] of Object.entries(state.masterStocks)) {
      const n = nullifyFields(stock, [
