...
class TestFetchKa10081DailyPrice:
    ...
    async def test_normal(self):
        ...
        # 공통 자료 계약 — raw_payload 필드만 반환 (설계서 4.1, 4.2)
        assert "status" not in result
        assert "request_date" not in result
        assert "response_date" not in result
    ...
    @pytest.mark.asyncio
    async def test_close_zero_not_failure(self):
        """종가 "0"은 정상적인 0으로 처리 — 0을 실패로 취급하지 않음 (W8 폴백 금지).

        기존: close_px <= 0이면 None 반환 (실패)
        변경: "0"은 숫자로 해석 가능하므로 0으로 전달
        """
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_price
        api = _mock_api()
    ...
    @pytest.mark.asyncio
    async def test_numeric_missing_cur_price_none(self):
        """종가 누락(빈 문자열)을 0이 아닌 None으로 전달 (설계서 4.1, 5.2)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_price
        api = _mock_api()
        row = _make_daily_row(cur_prc="")
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": [row]}), False)
        result = await fetch_ka10081_daily_price(api, "005930", "20260710")
        assert result is not None
        assert result["cur_price"] is None

    @pytest.mark.asyncio
    async def test_high_missing_not_replaced_by_close(self):
        """고가 누락을 종가로 대체하지 않고 None으로 전달 (설계서 4.1, 5.2)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_price
        api = _mock_api()
        row = _make_daily_row(cur_prc="70,000", high_pric="")
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": [row]}), False)
        result = await fetch_ka10081_daily_price(api, "005930", "20260710")
        assert result is not None
        assert result["cur_price"] == 70000
        assert result["high_price"] is None  # 종가(70000)로 대체하지 않음

    @pytest.mark.asyncio
    async def test_trade_amount_missing_none(self):
        """거래대금 누락을 0이 아닌 None으로 전달 (설계서 4.1, 5.2)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_price
        api = _mock_api()
        row = _make_daily_row(cur_prc="70,000", trde_prica="")
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": [row]}), False)
        result = await fetch_ka10081_daily_price(api, "005930", "20260710")
        assert result is not None
        assert result["trade_amount"] is None

    @pytest.mark.asyncio
    async def test_dt_field_from_api_response(self):
        """요청 기준일과 응답 기준일이 다를 때 dt는 API 실제 거래일 (설계서 4.2)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_price
        api = _mock_api()
        # 요청은 20260715, API는 아직 20260714 일봉을 latest로 반환
        row = _make_daily_row(dt="20260714", cur_prc="70,000")
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": [row]}), False)
        result = await fetch_ka10081_daily_price(api, "005930", "20260715")
        assert result is not None
        assert result["dt"] == "20260714"  # API 실제 거래일

    @pytest.mark.asyncio
    async def test_response_date_missing(self):
        """응답 거래일이 없으면 dt도 None (설계서 4.2)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_price
        api = _mock_api()
        row = _make_daily_row(dt="", cur_prc="70,000")
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": [row]}), False)
        result = await fetch_ka10081_daily_price(api, "005930", "20260710")
        assert result is not None
...
class TestFetchKa10081Daily5dData:
    ...
    async def test_normal_5_rows(self):
        ...
        # 공통 자료 계약 — raw_payload 필드만 반환 (설계서 4.1, 4.2, 4.5)
        assert "status" not in result
        assert "request_date" not in result
        assert "response_date" not in result
        assert "problems" not in result
    ...
    @pytest.mark.asyncio
    async def test_numeric_missing_in_5d_highs(self):
        """5일 고가 누락을 0이 아닌 None으로 전달 (설계서 4.1, 4.5)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_5d_data
        api = _mock_api()
        rows = [_make_daily_row(dt=f"2026071{i}", high_pric="") for i in range(5, 10)]
        rows = list(reversed(rows))
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": rows}), False)
        result = await fetch_ka10081_daily_5d_data(api, "005930", "20260710")
        assert result is not None
        # 모든 high_pric가 빈 문자열 → None으로 전달 (0이 아님)
        assert all(h is None for h in result["highs_5d_array"])

    @pytest.mark.asyncio
    async def test_numeric_missing_in_5d_amts(self):
        """5일 거래대금 누락을 0이 아닌 None으로 전달 (설계서 4.1, 4.5)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_5d_data
        api = _mock_api()
        rows = [_make_daily_row(dt=f"2026071{i}", trde_prica="") for i in range(5, 10)]
        rows = list(reversed(rows))
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": rows}), False)
        result = await fetch_ka10081_daily_5d_data(api, "005930", "20260710")
        assert result is not None
        assert all(a is None for a in result["amts_5d_array"])

    @pytest.mark.asyncio
    async def test_dts_5d_latest_first(self):
        """5일 조회에서 dts_5d_array의 첫 번째가 가장 최신 일봉 (설계서 4.2)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_5d_data
        api = _mock_api()
        rows = [_make_daily_row(dt=f"2026071{i}") for i in range(5, 10)]
        rows = list(reversed(rows))
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": rows}), False)
        result = await fetch_ka10081_daily_5d_data(api, "005930", "20260720")
        assert result is not None
        assert result["dts_5d_array"][0] == "20260719"  # 가장 최신 일봉의 dt
...
class TestFetchAllStocksDailyConfirmed:
    ...
    async def test_normal(self):
        ...
        # 새 반환 구조: success/failures 분리 (설계서 4.3)
        assert len(result["success"]) == 2
        assert "005930" in result["success"]
        assert "005940" in result["success"]
    ...
    async def test_partial_failure(self):
        """부분 실패 시 실패 종목이 failures에 보존됨 (설계서 4.3)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_all_stocks_daily_confirmed
        api = _mock_api()
        row = _make_daily_row()
        ...
        assert "FAIL01" in result["failures"]
        assert result["failures"]["FAIL01"] == "no_data"

    @pytest.mark.asyncio
    async def test_on_progress_called(self):
        """진행률이 처리 수·성공 수·실패 수·전체 수를 분리해서 전달 (설계서 4.3)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_all_stocks_daily_confirmed
        api = _mock_api()
        row = _make_daily_row()
        api._call_api.return_value = (_mock_resp({"stk_dt_pole_chart_qry": [row]}), False)
        ...
        with patch("backend.app.core.kiwoom_stock_rest.asyncio.sleep", new=AsyncMock()):
            await fetch_ka10081_all_stocks_daily_confirmed(
                api, ["005930"], "20260710",
                on_progress=lambda processed, success, failed, total: progress_calls.append((processed, success, failed, total))
            )
        assert (0, 0, 0, 1) in progress_calls  # 시작
        assert (1, 1, 0, 1) in progress_calls  # 종료

    @pytest.mark.asyncio
    async def test_exception_continues(self):
        """예외 발생 종목이 failures에 예외 사유와 함께 보존됨 (설계서 4.3)."""
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_all_stocks_daily_confirmed
        api = _mock_api()
        row = _make_daily_row()
        api._call_api.side_effect = [
        ...
        assert len(result["failures"]) == 1
        assert "FAIL01" in result["failures"]
        assert "exception" in result["failures"]["FAIL01"]
