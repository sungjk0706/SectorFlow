...
from backend.tests._mock_helpers import swallow_coro_returning, swallow_coro_then_raise
...
class TestEnqueue:
    ...
    def test_enqueue_deduplicates_in_flight_state_message(self):
        """동일 상태 알림은 전송 중 대기열에 한 건만 유지."""
        w = _make_worker()
        w._task = MagicMock(done=MagicMock(return_value=False))
        mock_queue = MagicMock()
        w._queue = mock_queue
        msg = {"type": "telegram", "message": "blocked", "dedup_key": "risk-buy-market"}
        w.enqueue(msg)
        w.enqueue(msg)
        mock_queue.put_nowait.assert_called_once_with(msg)
