...
class TestLifespanShutdown:
    ...
    async def test_shutdown_stops_scheduler_before_engine_and_database(self):
        from backend.app.web.app import lifespan
        mock_app = MagicMock(spec=FastAPI)
        patches, _, _ = _lifespan_patches()
        for p in patches:
        ...
        try:
            ...
            from backend.app.db.database import close_db_connection
            order: list[str] = []
            stop_daily_time_scheduler.side_effect = lambda: order.append('scheduler')
            stop_engine.side_effect = lambda: order.append('engine')
            stop_db_writer.side_effect = lambda: order.append('writer')
            close_db_connection.side_effect = lambda: order.append('database')

            async with lifespan(mock_app):
                pass

            assert order.index('scheduler') < order.index('engine')
            assert order.index('engine') < order.index('writer')
            assert order.index('writer') < order.index('database')
        finally:
            for p in reversed(patches):
                p.stop()
...
class TestScheduledEngineTaskLifecycle:
    """엔진 예약 작업이 종료 수명에 포함되는지 검증."""

    @pytest.mark.asyncio
    async def test_completed_scheduled_task_is_removed(self):
        from backend.app.services.engine_lifecycle import schedule_engine_task
        from backend.app.services.engine_state import state

        original_loop = state.engine_loop_ref
        original_tasks = state.engine_scheduled_tasks
        original_shutdown = state.engine_shutdown_requested
        state.engine_loop_ref = None
        state.engine_scheduled_tasks = set()
        state.engine_shutdown_requested = False

        async def work():
            await asyncio.sleep(0)

        try:
            assert schedule_engine_task(work(), context="테스트 작업") is True
            await asyncio.sleep(0.01)
            assert not state.engine_scheduled_tasks
        finally:
            state.engine_loop_ref = original_loop
            state.engine_scheduled_tasks = original_tasks
            state.engine_shutdown_requested = original_shutdown

    def test_shutdown_rejects_new_scheduled_task(self):
        from backend.app.services.engine_lifecycle import schedule_engine_task
        from backend.app.services.engine_state import state

        original_shutdown = state.engine_shutdown_requested
        state.engine_shutdown_requested = True
        ...
        coro = work()
        try:
            assert schedule_engine_task(coro, context="종료 중 테스트 작업") is False
            assert coro.cr_frame is None
        finally:
            state.engine_shutdown_requested = original_shutdown

    @pytest.mark.asyncio
    async def test_cancelled_scheduled_task_does_not_log_failure(self, caplog):
        from backend.app.services.engine_lifecycle import schedule_engine_task
        from backend.app.services.engine_state import state
        ...
        try:
            with caplog.at_level("WARNING"):
                assert schedule_engine_task(work(), context="취소 테스트 작업") is True
                await asyncio.sleep(0)
                task = next(iter(state.engine_scheduled_tasks))
                task.cancel()
                await asyncio.gather(task, return_exceptions=True)
                await asyncio.sleep(0)
            assert "취소 테스트 작업 작업 실패" not in caplog.text
        finally:
            state.engine_loop_ref = original_loop
            state.engine_scheduled_tasks = original_tasks
            state.engine_shutdown_requested = original_shutdown
