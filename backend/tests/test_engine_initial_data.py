...
class TestResetRealtimeFields:
    ...
    async def test_non_test_mode(self):
        ...
        master_cache = {
            "005930": {"code": "005930", "cur_price": 70000, "change": 500, "change_rate": 0.72,
                       ...
                       "program_net_buy": 1000, "news_boost": 1.0, "news_boost_ts": 123.0,
                       "name": "삼성전자"},
        }
        positions = [
        ...
        with patch("backend.app.services.engine_state.state") as mock_state, \
            ...
            assert master_cache["005930"]["order_ratio"] is None
            assert master_cache["005930"]["program_net_buy"] is None
            assert master_cache["005930"]["news_boost"] is None
            assert master_cache["005930"]["news_boost_ts"] is None
    ...
    @pytest.mark.asyncio
    async def test_program_net_buy_only_preserves_other_realtime_fields(self):
        """정규장 마감 단독 초기화는 프로그램 순매수만 비운다."""
        master_cache = {
            "005930": {
                "cur_price": 70000,
                "change": 500,
                ...
                "strength": 80,
                "order_ratio": [100, 200],
                "program_net_buy": 1000,
                "news_boost": 1.0,
                "news_boost_ts": 123.0,
            },
        }
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.services.engine_account_notify.notify_desktop_sector_stocks_refresh", new=AsyncMock()) as mock_refresh:
            mock_state.master_stocks_cache = master_cache

            await _reset_program_net_buy_only()

            assert master_cache["005930"]["program_net_buy"] is None
            assert master_cache["005930"]["cur_price"] == 70000
            assert master_cache["005930"]["change"] == 500
            assert master_cache["005930"]["change_rate"] == 0.72
            assert master_cache["005930"]["trade_amount"] == 100000
            assert master_cache["005930"]["strength"] == 80
            assert master_cache["005930"]["order_ratio"] == [100, 200]
            assert master_cache["005930"]["news_boost"] == 1.0
            assert master_cache["005930"]["news_boost_ts"] == 123.0
