...
from backend.app.services.buy_order_executor import (
    evaluate_buy_candidates,
    refresh_buy_market_guard_and_recompute,
)
from backend.app.services.risk_manager import MarketGuardStatus
...
def _stock(code="005930", guard_pass=True, cur_price=70000, sector="반도체", name="삼성전자", market_type="0", nxt_enable=False,
    ...
    s = StockScore(
        ...
        cur_price=cur_price, change=700, market_type=market_type, nxt_enable=nxt_enable,
...
class TestEarlyReturnGates:
    ...
    async def test_holding_count_exceeds_max_returns_early(self, fresh_state, reset_cash_gate):
        ...
        with patch("backend.app.services.engine_state.state", fresh_state), \
             ...
             patch("backend.app.services.buy_order_executor.is_virtual_mode", return_value=True), \
    ...
    async def test_buy_amt_on_false_skips_buy_amt_check(self, fresh_state, reset_cash_gate):
        ...
        with patch("backend.app.services.engine_state.state", fresh_state), \
            ...
            mock_rm.return_value.check_buy_market_guard = AsyncMock(return_value=MarketGuardStatus(allowed=True))
            await evaluate_buy_candidates()
...
class TestMarketGuardPreGate:
    ...
    async def test_market_block_applies_only_to_blocked_market(self, fresh_state, reset_cash_gate):
        """KOSPI 급락 시 KOSPI 후보만 차단하고 KOSDAQ 후보는 계속 시도한다."""
        first = _stock(code="A001", market_type="0")
        second = _stock(code="A002", market_type="10")
        fresh_state.sector_summary_cache = _sector_summary(
            stocks=[first, second],
            buy_targets=[
                BuyTarget(rank=1, sector_rank=1, stock=first),
                BuyTarget(rank=2, sector_rank=1, stock=second),
            ],
        )
        with patch("backend.app.services.engine_state.state", fresh_state), \
             patch("backend.app.services.buy_order_executor.auto_buy_effective", return_value=True), \
            ...
            mock_rm.return_value.check_buy_market_guard = AsyncMock(return_value=MarketGuardStatus(
                allowed=False,
                reason="코스피 급락 차단",
                reason_code=BUY_REJECT_RISK_MARKET_DROP,
                changed=True,
                blocked_markets={"0": "코스피 급락 차단"},
            ))
            mock_rm.return_value.get_withdrawable_deposit.return_value = 10_000_000
            await evaluate_buy_candidates()
        assert fresh_state.auto_trade.execute_buy.await_count == 1
    ...
    @pytest.mark.asyncio
    async def test_market_reason_is_restored_when_block_state_is_unchanged(self, fresh_state, reset_cash_gate):
        """후보가 다시 만들어져도 유지 중인 시장 차단 사유를 다시 표시한다."""
        target = fresh_state.sector_summary_cache.buy_targets[0]
        target.stock.market_type = "0"
