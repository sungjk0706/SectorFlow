...
class TestGetBuyTargetsSectorStocks:
    ...
    @pytest.mark.asyncio
    async def test_buy_target_entry_excludes_raw_status_fields(self):
        """매수 후보 엔트리에 자료 상태 관련 필드가 포함되지 않음 (자료 상태 시스템 제거)."""
        mock_stock = MagicMock()
        mock_stock.code = "005930"
        mock_stock.name = "삼성전자"
        mock_stock.market_type = "코스피"
        ...
        with patch("backend.app.services.engine_state.state") as mock_state:
            ...
            mock_state.master_stocks_cache = {
                "005930": {
                    "cur_price": 70000, "change_rate": 0.5, "change": 350,
                    "strength": 80, "trade_amount": 100000, "high_5d_price": 71000,
                    "order_ratio": 0.3, "program_net_buy": 1000,
                }
            }
...
class TestGetAllSectorStocks:
    ...
    @pytest.mark.asyncio
    async def test_excludes_raw_status_fields(self):
        """get_all_sector_stocks가 자료 상태 관련 필드를 전달하지 않음 (자료 상태 시스템 제거)."""
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.core.sector_mapping.get_merged_sectors_batch", new=AsyncMock(return_value={"005930": "반도체"})), \
             patch("backend.app.services.engine_symbol_utils.get_stock_market", return_value="코스피") as _, \
             patch("backend.app.services.engine_symbol_utils.is_nxt_enabled", return_value=True) as _:
...
class TestGetSectorStocks:
    ...
    @pytest.mark.asyncio
    async def test_none_avg_5d_excludes_from_sector_stocks(self):
        """avg_5d_trade_amount가 None이면 5일 자료 부족/수신 실패 — 거래대금 필터에서 명시 제외 (P20 폴백 금지)."""
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.core.sector_mapping.get_merged_sectors_batch", new=AsyncMock(return_value={})), \
             patch("backend.app.services.engine_symbol_utils.get_stock_market", return_value="코스피"), \
             patch("backend.app.services.engine_symbol_utils.is_nxt_enabled", return_value=False):
            ...
            mock_state.master_stocks_cache = {
                "005930": {"name": "삼성전자", "cur_price": 70000, "avg_5d_trade_amount": 50000, "high_5d_price": 71000},
                "000001": {"name": "부족종목", "cur_price": 50000, "avg_5d_trade_amount": None, "high_5d_price": 51000},  # 5일 자료 부족
            }
            mock_state.sector_summary_cache = None
            result = await get_sector_stocks()
            codes = [r["code"] for r in result]
            assert "005930" in codes
            assert "000001" not in codes  # None — 자료 부족으로 제외

    @pytest.mark.asyncio
    async def test_excludes_raw_status_fields(self):
        """get_sector_stocks가 자료 상태 관련 필드를 전달하지 않음 (자료 상태 시스템 제거)."""
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.core.sector_mapping.get_merged_sectors_batch", new=AsyncMock(return_value={})), \
             patch("backend.app.services.engine_symbol_utils.get_stock_market", return_value="코스피"), \
             patch("backend.app.services.engine_symbol_utils.is_nxt_enabled", return_value=False):
            ...
            mock_state.master_stocks_cache = {
                "005930": {
                    "name": "삼성전자", "cur_price": 70000, "avg_5d_trade_amount": 50000, "high_5d_price": 71000,
                },
            }
            mock_state.sector_summary_cache = None
            result = await get_sector_stocks()
...
class TestGetSectorSummaryInputs:
    ...
    @pytest.mark.asyncio
    async def test_none_avg_5d_excludes_from_summary_inputs(self):
        """avg_5d_trade_amount가 None이면 5일 자료 부족/수신 실패 — 전략 계산·구독 모두에서 제외 (P20 폴백 금지).

        자료 상태 시스템 제거 후: None 종목은 거래대금 필터에서 명시적으로 제외되어
        all_codes·all_filter_codes 모두에서 빠진다. 0이 아닌 None으로 "자료 없음"을 명확히 보존.
        """
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.services.daily_time_scheduler.is_nxt_only_window", return_value=False):
            mock_state.integrated_system_settings_cache = {"sector_min_trade_amt": 0.0}
            mock_state.master_stocks_cache = {
                "005930": {"name": "삼성전자", "cur_price": 70000, "avg_5d_trade_amount": 50000},
                "000001": {"name": "부족종목", "cur_price": 50000, "avg_5d_trade_amount": None},  # 5일 자료 부족
                "000002": {"name": "불일치종목", "cur_price": 60000, "avg_5d_trade_amount": None},  # 수신 실패
            }
            result = await get_sector_summary_inputs()
            # None 종목은 전략 계산·구독 모두에서 제외
            assert result["all_codes"] == ["005930"]
            assert result["all_filter_codes"] == ["005930"]
...
class TestRecomputeSectorSummaryNow:
    ...
    @pytest.mark.asyncio
    async def test_recompute_all_blocked_produces_empty_targets(self):
        """전체 종목 5일 자료 부족(None) 시 계산 입력이 비어 매수 후보가 비어야 함 (P20 폴백 금지).

        자료 상태 시스템 제거 후: avg_5d_trade_amount가 None이면 거래대금 필터에서 제외되어
        계산 입력이 비게 되고, 매수 후보도 빈 결과로 이어진다.
        """
        from backend.app.domain.models import SectorSummary
        _ss = SectorSummary(sectors=[], buy_targets=[], blocked_targets=[])
        ...
        with patch("backend.app.services.engine_lifecycle.is_engine_running", return_value=True), \
             patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.services.sector_data_provider.get_sector_summary_inputs", new=AsyncMock(return_value={
                 "all_codes": [], "all_filter_codes": [], "trade_prices": {}, "trade_amounts": {}, "avg_amt_5d": {}
             })), \
             patch("backend.app.domain.sector_calculator.compute_full_sector_summary", new=AsyncMock(return_value=_ss)), \
             patch("backend.app.domain.buy_filter.build_buy_targets_from_settings", return_value=_ss), \
             patch("backend.app.services.engine_account.get_held_codes", new=AsyncMock(return_value=set())), \
             patch("backend.app.services.engine_sector_confirm.cancel_sector_recompute"), \
             patch("backend.app.services.engine_account_notify.notify_desktop_sector_scores", new=AsyncMock()), \
             patch("backend.app.services.engine_account_notify.notify_desktop_sector_stocks_refresh", new=AsyncMock()), \
            ...
            mock_state.integrated_system_settings_cache = settings_cache
            mock_state.auto_trade = None
            # 전체 종목 5일 자료 부족 — get_sector_summary_inputs 가 빈 all_codes 를 반환
            mock_state.master_stocks_cache = {
                "005930": {"avg_5d_trade_amount": None},
                "000660": {"avg_5d_trade_amount": None},
            }
            mock_state.sector_summary_ready_event = MagicMock()
            await recompute_sector_summary_now()
            # 자료 부족 종목만 있으면 매수 후보가 비어 정상 결과처럼 보이지 않음
            assert _ss.buy_targets == []
...
# ── compute_sector_data_status 제거됨 (자료 상태 시스템 전면 제거) ──────
