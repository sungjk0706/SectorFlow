...
class TestIsOrderBlockedByTime:
    ...
    def test_opening_auction_both_blocked(self):
        ...
        없음 → KRX 단독·NXT 종목 모두 차단 (docs/참고자료/참고_KRX_NXT_거래시간.md: 🚫 주문 중단)."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "시가 동시호가", "nxt": "정규장 준비"}
        with patch("backend.app.services.engine_state.state", mock_state), \
...
class TestGetOrderTimeBlockStatus:
    """체결 불가 시간대 주문 상태 (페이즈 기반, 종목 구분 없음) — WS 브로드캐스트용.

    is_order_blocked_by_time(stk_cd)와 동일한 페이즈 판별을 사용하되
    종목별 is_nxt_enabled 분기 없이 (level, reason) 튜플 반환.
    ...
    def test_krx_regular_returns_not_blocked(self):
        """09:00~15:20 정규장/메인마켓 — ("ok", "")."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "정규장", "nxt": "메인마켓"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(10, 0)):
            assert get_order_time_block_status() == ("ok", "")

    def test_krx_inactive_nxt_active_returns_nxt_only_reason(self):
        """08:00~08:50 KRX 장전 대기 + NXT 프리마켓 — ("nxt_only", "NXT만 가능")."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "장전 대기", "nxt": "프리마켓"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(8, 20)):
    ...
    def test_both_inactive_returns_auction_reason(self):
        """15:20~15:30 종가 동시호가 + NXT 조기 마감 — 양쪽 비활성 → ("blocked", "거래 시간 외")."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "종가 동시호가", "nxt": "조기 마감"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(15, 25)):
    ...
    def test_market_closed_returns_auction_reason(self):
        """20:00~24:00 장마감 — 양쪽 비활성 → ("blocked", "거래 시간 외")."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "장마감", "nxt": "장마감"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(21, 0)):
    ...
    def test_holiday_returns_false(self):
        """휴장일 — 장 안 열리므로 칩 표시 불필요 → ("ok", "") (P21 사용자 투명성)."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "휴장일", "nxt": "휴장일"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(10, 0)):
    ...
    def test_aftermarket_returns_nxt_only_reason(self):
        """15:40~20:00 KRX 장후 시간외 + NXT 애프터마켓 — ("nxt_only", "NXT만 가능")."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "장후 시간외", "nxt": "애프터마켓"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(16, 0)):
    ...
    def test_boundary_at_0900_regular_allowed(self):
        """09:00:00 정각 — 페이즈 "정규장"이면 즉시 ("ok", "") (버퍼 없음)."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "정규장", "nxt": "메인마켓"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(9, 0, 0)):
    ...
    def test_boundary_at_152000_closing_auction_blocked(self):
        """15:20:00 — 페이즈 "종가 동시호가" → ("blocked", "거래 시간 외")."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "종가 동시호가", "nxt": "조기 마감"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(15, 20, 0)):
    ...
    def test_boundary_at_080000_premarket_nxt_only(self):
        """08:00:00 — 페이즈 "장전 대기"+"프리마켓" → ("nxt_only", "NXT만 가능")."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "장전 대기", "nxt": "프리마켓"}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler._kst_now", return_value=_make_kst(8, 0, 0)):
...
class TestIsRealtimeResetWindow:
    @pytest.mark.asyncio
    ...
    @pytest.mark.asyncio
    async def test_before_reset_returns_false(self):
        """07:57 — 초기화 구간 시작 전 → False."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "장개시전", "nxt": "장개시전"}
        with patch("backend.app.services.engine_state.state", mock_state), \
    ...
    @pytest.mark.asyncio
    async def test_at_reset_start_returns_true(self):
        """07:58 정각 — 하한 포함 → True."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "장개시전", "nxt": "장개시전"}
        with patch("backend.app.services.engine_state.state", mock_state), \
...
class TestApplyMarketPhase:
    ...
    def test_apply_market_phase_clears_krx_override_on_change(self):
        """krx 페이즈 변경 시 krx_countdown_override 초기화 (stale 카운트다운 방지 — P10/P22)."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "정규장", "nxt": "메인마켓"}
        mock_state.krx_countdown_override = {"label": "정규장 장마감", "remaining_sec": 300}
        mock_state.nxt_countdown_override = None
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler.get_market_phase", return_value={"krx": "종가 동시호가", "nxt": "메인마켓", "krx_countdown": None, "nxt_countdown": None}), \
             patch("backend.app.services.daily_time_scheduler.schedule_engine_task", side_effect=_close_coro):
            _apply_market_phase({"krx": "종가 동시호가", "nxt": "메인마켓"})
            assert mock_state.krx_countdown_override is None

    def test_apply_market_phase_clears_nxt_override_on_change(self):
        """nxt 페이즈 변경 시 nxt_countdown_override 초기화 (stale 카운트다운 방지 — P10/P22)."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "정규장", "nxt": "프리마켓"}
        mock_state.krx_countdown_override = None
        mock_state.nxt_countdown_override = {"label": "프리마켓 장마감", "remaining_sec": 60}
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler.get_market_phase", return_value={"krx": "정규장", "nxt": "정규장 준비", "krx_countdown": None, "nxt_countdown": None}), \
             patch("backend.app.services.daily_time_scheduler.schedule_engine_task", side_effect=_close_coro):
            _apply_market_phase({"krx": "정규장", "nxt": "정규장 준비"})
            assert mock_state.nxt_countdown_override is None

    def test_apply_market_phase_no_change_keeps_override(self):
        """페이즈 미변경 시 override 유지 (불필요한 초기화 금지 — P24 단순성)."""
        mock_state = MagicMock()
        mock_state.market_phase = {"krx": "정규장", "nxt": "메인마켓"}
        krx_override = {"label": "정규장 장마감", "remaining_sec": 300}
        mock_state.krx_countdown_override = krx_override
        mock_state.nxt_countdown_override = None
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler.get_market_phase", return_value={"krx": "정규장", "nxt": "메인마켓", "krx_countdown": None, "nxt_countdown": None}), \
             patch("backend.app.services.daily_time_scheduler.schedule_engine_task", side_effect=_close_coro):
            _apply_market_phase({"krx": "정규장", "nxt": "메인마켓"})
            assert mock_state.krx_countdown_override is krx_override
...
class TestRetryPipelineCatchup:
    ...
    @pytest.mark.asyncio
    async def test_first_stock_fresh_but_another_stale_triggers_fetch(self):
        """첫 종목은 최신이어도 다른 종목이 오래되면 갱신 트리거 (설계서 4.4, 세션 5)."""
        mock_state = MagicMock()
        mock_state.integrated_system_settings_cache = {"timetable.confirmed_download": "20:40"}
        # 005930은 최신(20250105), 000660은 오래됨(20250104) → 전체 최신 아님 → 트리거
        mock_state.master_stocks_cache = {
            "005930": {"date": "20250105"},
            "000660": {"date": "20250104"},
        }
        mock_state.confirmed_done = False
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler.is_realtime_reset_window", new_callable=AsyncMock, return_value=False), \
    ...
    @pytest.mark.asyncio
    async def test_all_stocks_fresh_sets_done(self):
        """전체 종목이 최신이면 확정 완료로 표시 (설계서 4.4, 세션 5)."""
        mock_state = MagicMock()
        mock_state.integrated_system_settings_cache = {"timetable.confirmed_download": "20:40"}
        mock_state.master_stocks_cache = {
            "005930": {"date": "20250105"},
            "000660": {"date": "20250105"},
        }
        mock_state.confirmed_done = False
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler.is_realtime_reset_window", new_callable=AsyncMock, return_value=False), \
    ...
    @pytest.mark.asyncio
    async def test_all_fresh_date_sets_done_even_with_missing_5d(self):
        """날짜가 최신이면 5일 자료 부족 종목이 있어도 확정 완료로 표시 (자료 상태 시스템 제거).

        자료 상태 기반 차단이 제거되어 최신성 판단은 date 기준만으로 수행.
        5일 자료가 부족한 종목(avg_5d_trade_amount=None)이 있어도 date가 최신이면 갱신 트리거 안 함.
        """
        mock_state = MagicMock()
        mock_state.integrated_system_settings_cache = {"timetable.confirmed_download": "20:40"}
        # 날짜는 모두 최신, 한 종목은 5일 자료 부족 → date 기준 최신이므로 갱신 안 함
        mock_state.master_stocks_cache = {
            "005930": {"date": "20250105", "avg_5d_trade_amount": 50000},
            "000660": {"date": "20250105", "avg_5d_trade_amount": None},  # 5일 자료 부족
        }
        mock_state.confirmed_done = False
        with patch("backend.app.services.engine_state.state", mock_state), \
             patch("backend.app.services.daily_time_scheduler.is_realtime_reset_window", new_callable=AsyncMock, return_value=False), \
...
class TestCountdownTimetableEntries:
    ...
    def test_nxt_premarket_countdown_entries_exist(self):
        ...
        # 장개시 3개(10분/5분/1분) + 장마감 2개(5분/1분 — 10분전 없음)
        assert len(countdown_nxt_pre) == 5
    ...
    def test_nxt_aftermarket_countdown_entries_exist(self):
        ...
        # 장개시 3개(10분/5분/1분) + 장마감 2개(5분/1분 — 10분전 없음)
        assert len(countdown_nxt_aft) == 5
...
class TestCountdownConstants:
    ...
    def test_nxt_pre_close_countdown_constants(self):
        """NXT 프리마켓 장마감 카운트다운 임계 — 5분/1분 (10분전 없음)."""
    ...
    def test_nxt_aft_close_countdown_constants(self):
        """NXT 에프터마켓 장마감 카운트다운 임계 — 5분/1분 (10분전 없음)."""
