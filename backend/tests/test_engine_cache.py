...
class TestLoadCachesPrebootWsWindow:
    ...
    async def test_reset_only_window_resets_and_recomputes(self):
        """20:00~20:40 공백 시간 기동 — 초기화 구간 True, WS 구간 False.

        핵심 시나리오: 실시간 수신은 끝났으나 확정 데이터는 도착 전.
        낡은 실시간 값을 비우고(초기화), WS 연결 중이 아니므로 업종 재계산도 수행.
        sector_summary_ready_event는 set하지 않음 (WS 구간 아님).
        """
        from backend.app.services import engine_cache
    ...
    @pytest.mark.asyncio
    async def test_outside_both_windows_no_reset(self):
        """20:40 이후 기동 — 초기화 구간 False, WS 구간 False → 초기화·재계산 생략 없음.

        확정 데이터가 DB에 저장된 구간이므로 초기화 불필요. recompute만 수행.
        """
        from backend.app.services import engine_cache
