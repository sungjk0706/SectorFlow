"""sector_mapping.py 단위 테스트 — 종목→업종 매핑 3함수 검증.

get_merged_sector: 인메모리 캐시 → DB 폴백 → "미분류"
...
class TestGetMergedSector:
    @pytest.mark.asyncio
    async def test_cache_hit_returns_sector(self):
        ...
        with patch("backend.app.services.engine_state.state", mock_state):
            result = await get_merged_sector("005930")
        assert result == "반도체"

    @pytest.mark.asyncio
    async def test_cache_hit_empty_sector_returns_midistributed(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_cache_hit_none_sector_returns_midistributed(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
        mock_state.master_stocks_cache = {"005930": {"sector": None}}
        with patch("backend.app.services.engine_state.state", mock_state):
            result = await get_merged_sector("005930")
        assert result == "미분류"

    @pytest.mark.asyncio
    async def test_cache_miss_db_hit_returns_sector(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
        mock_state.master_stocks_cache = {}
        mock_cursor = AsyncMock()
        mock_cursor.fetchone.return_value = {"sector": "은행"}
        mock_conn = MagicMock()
        ...
        with (
            ...
            result = await get_merged_sector("000660")
        assert result == "은행"

    @pytest.mark.asyncio
    async def test_cache_miss_db_miss_returns_midistributed(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
        mock_state.master_stocks_cache = {}
        mock_cursor = AsyncMock()
        ...
        with (
            ...
            result = await get_merged_sector("999999")
        assert result == "미분류"

    @pytest.mark.asyncio
    async def test_cache_miss_db_row_empty_sector_returns_midistributed(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
        mock_state.master_stocks_cache = {}
        mock_cursor = AsyncMock()
        mock_cursor.fetchone.return_value = {"sector": ""}
        mock_conn = MagicMock()
        mock_conn.cursor = AsyncMock(return_value=mock_cursor)
        with (
            patch("backend.app.services.engine_state.state", mock_state),
    ...
    @pytest.mark.asyncio
    async def test_lowercase_code_uppercased_for_cache(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
        mock_state.master_stocks_cache = {"005930": {"sector": "반도체"}}
        with patch("backend.app.services.engine_state.state", mock_state):
    ...
    @pytest.mark.asyncio
    async def test_state_exception_falls_through_to_db(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
        mock_state.master_stocks_cache = MagicMock(side_effect=Exception("state error"))
        mock_cursor = AsyncMock()
        mock_cursor.fetchone.return_value = {"sector": "증권"}
        mock_conn = MagicMock()
        mock_conn.cursor = AsyncMock(return_value=mock_cursor)
        with (
            patch("backend.app.services.engine_state.state", mock_state),
    ...
    @pytest.mark.asyncio
    async def test_db_exception_returns_midistributed(self):
        from backend.app.core.sector_mapping import get_merged_sector
        mock_state = MagicMock()
        mock_state.master_stocks_cache = {}
        mock_conn = MagicMock()
        mock_conn.cursor = AsyncMock(side_effect=Exception("db error"))
        with (
            patch("backend.app.services.engine_state.state", mock_state),
            patch("backend.app.db.database.get_db_connection", AsyncMock(return_value=mock_conn)),
        ):
