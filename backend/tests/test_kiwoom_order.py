...
class TestMarketSell:
    @pytest.mark.asyncio
    async def test_market_sell_delegates_to_send_order(self):
        from backend.app.core import kiwoom_order
        mock_resp = _mock_httpx_response(200, {"rt_cd": "0", "msg1": "OK"})
        mock_client = _mock_httpx_client(response=mock_resp)
        settings = {"kiwoom_account_no": "12345678"}
        with (
            ...
            result = await kiwoom_order.market_sell(settings, "token123", "005930", 10)
        assert result["success"] is True
        # SELL 주문이므로 api-id가 kt10001이어야 함
        call_args = mock_client.post.call_args
        sent_headers = call_args.kwargs.get("headers", {})
        assert sent_headers.get("api-id") == "kt10001"

    @pytest.mark.asyncio
    async def test_market_sell_communication_failure(self):
        from backend.app.core import kiwoom_order
        mock_client = _mock_httpx_client(side_effect=Exception("network error"))
        settings = {"kiwoom_account_no": "12345678"}
        with (
...
class TestGetUnexecutedOrders:
    @pytest.mark.asyncio
    async def test_all_stk_tp_1_with_code(self):
        from backend.app.core.kiwoom_order import get_unexecuted_orders
        mock_resp = _mock_httpx_response(200, {"rt_cd": "0", "msg1": "", "output1": [{"ord_no": "123"}]})
        mock_client = _mock_httpx_client(response=mock_resp)
        settings = {"kiwoom_account_no": "12345678"}
        with (
            patch("backend.app.core.kiwoom_order.build_broker_urls", return_value={"rest_base": "https://api.kiwoom.com"}),
            ...
            result = await get_unexecuted_orders(settings, "token123", "005930")
        assert result["success"] is True
        call_args = mock_client.post.call_args
        sent_params = call_args.kwargs.get("json", {})
        assert sent_params["all_stk_tp"] == "1"
        assert sent_params["stk_cd"] == "005930"
        assert result["data"] == [{"ord_no": "123"}]

    @pytest.mark.asyncio
    async def test_all_stk_tp_0_without_code(self):
        from backend.app.core.kiwoom_order import get_unexecuted_orders
        mock_resp = _mock_httpx_response(200, {"rt_cd": "0", "msg1": "", "output1": []})
        mock_client = _mock_httpx_client(response=mock_resp)
        settings = {"kiwoom_account_no": "12345678"}
        with (
            patch("backend.app.core.kiwoom_order.build_broker_urls", return_value={"rest_base": "https://api.kiwoom.com"}),
            ...
            result = await get_unexecuted_orders(settings, "token123")
        assert result["success"] is True
        call_args = mock_client.post.call_args
        sent_params = call_args.kwargs.get("json", {})
        assert sent_params["all_stk_tp"] == "0"
        assert sent_params["stk_cd"] == ""

    @pytest.mark.asyncio
    async def test_communication_failure_returns_empty(self):
        from backend.app.core.kiwoom_order import get_unexecuted_orders
        mock_client = _mock_httpx_client(side_effect=Exception("network error"))
        settings = {"kiwoom_account_no": "12345678"}
        with (
    ...
    async def test_rt_cd_non_zero_returns_failure(self):
        from backend.app.core.kiwoom_order import get_unexecuted_orders
        mock_resp = _mock_httpx_response(200, {"rt_cd": "1", "msg1": "에러", "output1": []})
        mock_client = _mock_httpx_client(response=mock_resp)
        settings = {"kiwoom_account_no": "12345678"}
        with (
            patch("backend.app.core.kiwoom_order.build_broker_urls", return_value={"rest_base": "https://api.kiwoom.com"}),
