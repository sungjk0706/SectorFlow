...
class TestKiwoomStockProvider:
    ...
    async def test_fetch_stock_5day_data_delegates(self):
        ...
        auth.rest_api = mock_api
        provider = KiwoomStockProvider(auth)
        mock_detail = {"amts_5d_array": [100000], "highs_5d_array": [72000], "dts_5d_array": ["20260711"]}
        with patch("backend.app.core.kiwoom_stock_rest.fetch_ka10081_daily_5d_data", new_callable=AsyncMock, return_value=mock_detail) as mock_fetch:
            result = await provider.fetch_stock_5day_data("005930", "20260711")
        assert isinstance(result, RawStockFetchResult)
        assert result.code == "005930"
        assert result.raw_payload == mock_detail
        mock_fetch.assert_called_once_with(mock_api, "005930", "20260711")

    async def test_fetch_stock_5day_data_fetch_failed(self):
        """조회 실패 시 raw_payload=None 의 RawStockFetchResult 반환 (설계서 4.3)."""
        from backend.app.core.kiwoom_providers import KiwoomAuthProvider, KiwoomStockProvider
        from backend.app.core.broker_providers import RawStockFetchResult
        auth = MagicMock(spec=KiwoomAuthProvider)
        mock_api = MagicMock()
        auth.rest_api = mock_api
        provider = KiwoomStockProvider(auth)
        with patch("backend.app.core.kiwoom_stock_rest.fetch_ka10081_daily_5d_data", new_callable=AsyncMock, return_value=None):
            result = await provider.fetch_stock_5day_data("005930", "20260711")
        assert isinstance(result, RawStockFetchResult)
        assert result.code == "005930"
    ...
    async def test_fetch_all_stocks_daily_confirmed_delegates(self):
        ...
        mock_detail = {"dt": "20260711", "cur_price": 70000}
        mock_raw_result = {"success": {"005930": mock_detail}, "failures": {}, "total": 1,
                           "success_count": 1, "failed_count": 0}
        with patch("backend.app.core.kiwoom_stock_rest.fetch_ka10081_all_stocks_daily_confirmed", new_callable=AsyncMock, return_value=mock_raw_result) as mock_fetch:
            result = await provider.fetch_all_stocks_daily_confirmed(["005930"], "20260711", interval_sec=0.5, on_progress=on_progress)
        assert "005930" in result
        assert isinstance(result["005930"], RawStockFetchResult)
        assert result["005930"].code == "005930"
        assert result["005930"].raw_payload == mock_detail
        mock_fetch.assert_called_once_with(mock_api, ["005930"], "20260711", interval_sec=0.5, on_progress=on_progress)

    async def test_fetch_all_stocks_daily_confirmed_with_failures(self):
        """실패 종목도 raw_payload=None 의 RawStockFetchResult로 반환 (설계서 4.3)."""
        from backend.app.core.kiwoom_providers import KiwoomAuthProvider, KiwoomStockProvider
        from backend.app.core.broker_providers import RawStockFetchResult
        auth = MagicMock(spec=KiwoomAuthProvider)
        mock_api = MagicMock()
        auth.rest_api = mock_api
        provider = KiwoomStockProvider(auth)
        mock_raw_result = {"success": {}, "failures": {"FAIL01": "no_data"}, "total": 1,
                           "success_count": 0, "failed_count": 1}
        with patch("backend.app.core.kiwoom_stock_rest.fetch_ka10081_all_stocks_daily_confirmed", new_callable=AsyncMock, return_value=mock_raw_result):
            result = await provider.fetch_all_stocks_daily_confirmed(["FAIL01"], "20260711")
        assert "FAIL01" in result
        assert isinstance(result["FAIL01"], RawStockFetchResult)
        assert result["FAIL01"].raw_payload is None
