...
class TestMarketGuard:
    ...
    def test_data_unavailable_is_allowed_in_nxt_only_window(self, risk_manager):
        """NXT 전용 시간대에는 KRX 지수 미수신이 정상이라 매수 가드를 차단하지 않음."""
        risk_manager.market_guard_kospi_on = True
        risk_manager.market_guard_kosdaq_on = True
        with patch("backend.app.services.engine_state.state") as mock_state:
            mock_state.index_data_cache = {}
            ...
            mock_state.integrated_system_settings_cache = {}
            allowed, reason = risk_manager._check_market_drop(require_data=False)
        assert allowed is True

    def test_data_unavailable_blocks_in_regular_window(self, risk_manager):
        """KRX 지수 수신이 기대되는 시간대에는 활성 가드의 자료가 없으면 매수 차단."""
        risk_manager.market_guard_kospi_on = True
        with patch("backend.app.services.engine_state.state") as mock_state:
            mock_state.index_data_cache = {}
            mock_state.market_phase = {"krx": "정규장", "nxt": "메인마켓"}
            mock_state.integrated_system_settings_cache = {}
            allowed, reason = risk_manager._check_market_drop(require_data=True)
        assert allowed is False
        assert "자료 확인 불가" in reason

    def test_drate_parse_failure_blocks_in_regular_window(self, risk_manager):
        """KRX 시간대에 활성 지수의 등락률 변환이 실패하면 매수 차단."""
        risk_manager.market_guard_kospi_on = True
        with patch("backend.app.services.engine_state.state") as mock_state:
    ...
    @pytest.mark.asyncio
    async def test_buy_market_guard_keeps_both_market_blocks(self, risk_manager):
        """두 시장이 동시에 급락하면 두 시장의 차단 사유를 모두 보존."""
        risk_manager.risk_block_buy_on = True
        risk_manager.market_guard_kospi_on = True
        risk_manager.market_guard_kosdaq_on = True
        with patch("backend.app.services.engine_state.state") as mock_state, \
            ...
            status = await risk_manager.check_buy_market_guard()
        assert status.allowed is False
        assert set(status.blocked_markets) == {"0", "10"}
        assert "코스피 급락 차단" in status.blocked_markets["0"]
        assert "코스닥 급락 차단" in status.blocked_markets["10"]

    @pytest.mark.asyncio
    async def test_buy_market_guard_status_notifies_only_on_transition(self, risk_manager):
        """같은 시장 차단 상태는 한 번만 알리고, 회복 시 한 번만 해제 알림."""
        risk_manager.risk_block_buy_on = True
        risk_manager.market_guard_kospi_on = True
        with patch("backend.app.services.engine_state.state") as mock_state, \
            ...
            mock_state.index_data_cache = {"001": {"drate": "-2.0"}}
            recovered = await risk_manager.check_buy_market_guard()
        ...
        first_payload = mock_broadcast.await_args_list[0].args[1]
        assert first_payload["reason"] == "코스피 급락 차단 · 코스닥 매수 가능"
        assert first_payload["partial"] is True

    @pytest.mark.asyncio
    async def test_buy_market_guard_no_renotify_on_drate_only_change(self, risk_manager):
        """같은 시장 차단 중 변동률 숫자만 바뀌면 재알림·로그·배지 브로드캐스트 없음 (업계 표준: 이산 상태 전환만 알림)."""
        risk_manager.risk_block_buy_on = True
        risk_manager.market_guard_kospi_on = True
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.services.engine_account_notify._safe_broadcast", new=AsyncMock()) as mock_broadcast, \
            ...
            mock_state.index_data_cache = {"001": {"drate": "-6.0"}}
            first = await risk_manager.check_buy_market_guard()
            mock_state.index_data_cache = {"001": {"drate": "-5.5"}}
            second = await risk_manager.check_buy_market_guard()
            mock_state.index_data_cache = {"001": {"drate": "-7.2"}}
            third = await risk_manager.check_buy_market_guard()
        assert first.changed is True
        assert second.changed is False
    ...
    @pytest.mark.asyncio
    async def test_buy_market_guard_regular_missing_data_blocks(self, risk_manager):
        """KRX 시간대의 활성 지수 미수신은 매수 차단."""
        risk_manager.risk_block_buy_on = True
        risk_manager.market_guard_kospi_on = True
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch.object(risk_manager, "_sync_thresholds", lambda: None):
            ...
            status = await risk_manager.check_buy_market_guard()
        assert status.allowed is False
        assert "자료 확인 불가" in status.reason
    ...
    @pytest.mark.asyncio
    async def test_buy_market_guard_allows_other_market_at_order_gate(self, risk_manager, settings_cache):
        """KOSPI 급락 중에도 KOSDAQ 종목은 실제 주문 직전 검사에서 허용."""
        risk_manager.risk_block_buy_on = True
        risk_manager.market_guard_kospi_on = True
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.services.risk_manager.is_test_mode", return_value=False), \
            ...
            mock_state.account_snapshot = {"orderable": 100_000_000}
            mock_state.positions = []
            mock_state.master_stocks_cache = {"377300": {"market": "10"}}
            allowed, reason = await risk_manager.check_buy_order_allowed("377300", 70_000, 10)
        assert allowed is True
        assert reason == "승인"

    @pytest.mark.asyncio
    async def test_buy_market_guard_blocks_unknown_market_when_guard_active(self, risk_manager, settings_cache):
        """시장 구분을 확인할 수 없으면 활성 시장 가드를 우회하지 않는다."""
        risk_manager.risk_block_buy_on = True
        risk_manager.market_guard_kospi_on = True
        with patch("backend.app.services.engine_state.state") as mock_state, \
             patch("backend.app.services.risk_manager.is_test_mode", return_value=False), \
            ...
            allowed, reason = await risk_manager.check_buy_order_allowed("UNKNOWN", 70_000, 10)
        assert allowed is False
        assert reason == "시장 구분 확인 불가"
