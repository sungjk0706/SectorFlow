...
async def get_sector_summary_inputs(codes: list[str] | None = None) -> dict:
    ...
    계산 영역 분리 (설계서 4.2): 본 함수가 서비스 경계에서 종목 자료와 업종 매핑을 한 번의
    묶음 조회로 준비해 계산 본체에 전달한다. 계산 본체는 외부 조회를 직접 수행하지 않는다.

    캐시 직접 읽기: 종목 코드와 avg_amt_5d(백만원)만 필요하므로 복사·필드명 변환·정렬 없이 필터링한다.
    증분 재계산은 전달받은 후보만 검사해 전체 종목 순회를 피한다.

    자료 부족 종목 처리: avg_5d_trade_amount가 None이면 5일 자료 부족/수신 실패를 의미.
    거래대금 필터에서 None 종목을 명시적으로 제외하여 "자료 없음"을 0으로 덮어 숨기지 않는다 (P20).
    """
    ...
    for cd in source_codes:
        ...
        if int(entry.get("cur_price") or 0) <= 0 and (not entry.get("name") or entry.get("name") == cd):
            continue
        avg5d_raw = entry.get("avg_5d_trade_amount")
        # 5일 자료 부족/수신 실패 → None 보존 (P20 폴백 금지). None은 거래대금 필터에서 명시 제외.
        if avg5d_raw is None:
            continue
        avg5d_million = int(avg5d_raw)
        avg5d_eok = avg5d_million // 100
        if min_avg_amt_eok > 0 and avg5d_eok < min_avg_amt_eok:
            continue
        # 구독 대상 — 매매 적격 종목 (거래대금 필터 통과)
        all_filter_codes.append(cd)
        # 전략 계산 대상 — 동일 조건 (거래대금 필터로 자료 부족 종목 자연 제외)
        strategy_codes.append(cd)
    ...
    # 전략 계산 대상(strategy_codes)에만 NXT 필터 적용 — 구독 대상은 all_filter_codes 그대로
    if is_nxt_only_window():
        filtered_codes = [cd for cd in strategy_codes if _is_nxt(cd)]
    else:
        filtered_codes = strategy_codes
    ...
    # 업종 매핑 묶음 조회 — 계산 영역 바깥에서 한 번만 수행 (설계서 4.2)
    sector_map = await get_merged_sectors_batch(all_codes) if all_codes else {}
    ...
    return {
        ...
        # 계산 본체에 명시 전달할 종목 자료·업종 매핑 (설계서 4.1 입력 계약)
        "master_stocks_cache": cache,
...
async def get_sector_stocks() -> list:
    ...
    # 자료 부족 종목(avg_5d_trade_amount=None)은 거래대금 필터에서 명시 제외 (P20 폴백 금지)
...
async def recompute_sector_summary_now() -> None:
    ...
    try:
        ...
        # all_filter_codes(구독 대상) 사용 — 매매 적격 종목은 _filtered 플래그 부여하여 실시간 구독 유지.
        # NXT-only 구간에도 KRX 종목 _filtered 플래그 유지.
