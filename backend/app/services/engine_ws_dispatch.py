...
# 카운트다운은 분 단위까지만 표시 (10초 코드는 무시 — 개인 투자자에게 불필요한 노이즈).
...
# "53"(사용안함) + 10코드(10초전 — 분 단위까지만 표시하므로 무시).
_JIF_IGNORE_CODES: frozenset[str] = frozenset({
    "53",                             # 사용안함
    "22", "42",                       # KRX 10초전 — 분 단위 표시만 유지
    "A2", "B2", "C2", "D2",           # NXT 10초전 — 분 단위 표시만 유지
})
...
async def _handle_jif(data: dict) -> None:
    ...
    # 페이즈 전환 시 countdown override 초기화는 _apply_market_phase()에서 처리 (P10 SSOT 단일 경로).
    # JIF 경로와 타임테이블 보완 경로 양쪽이 _apply_market_phase를 경유하므로 여기서는 초기화하지 않음.
    if jangubun in ("1", "2"):
