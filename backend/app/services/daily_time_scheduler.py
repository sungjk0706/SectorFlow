...
async def retry_pipeline_catchup_after_bootstrap() -> None:
    ...
    try:
        from backend.app.core.settings_file import load_settings_async
        _settings = await load_settings_async()
    ...
    # 1) 핵심 캐시 만료 → 통합 확정 조회 (ka10099 + ka10081)
...
async def _apply_auto_toggle_on_startup(settings: dict) -> None:
    ...
    # 기동 시각 기준 단순 ON/OFF — 거래일이고 설정된 시간이면 ON, 아니면 OFF
...
async def _on_ws_subscribe_start() -> None:
    ...
    try:
        ...
        from backend.app.services.engine_service import _invalidate_sector_stocks_cache
        ...
        if ws and ws.is_connected():
            ...
            try:
                ...
                _kiwoom_connector = _mgr.get_connector(broker_nm)
...
async def _on_ws_subscribe_end() -> None:
    ...
    global _ws_subscribe_window_active, _confirmed_done
    try:
        from backend.app.services import engine_service
...
def _trigger_reg_pipeline() -> None:
    ...
    try:
        ws = _connector_manager or _kiwoom_connector
        if ws and ws.is_connected() and _login_ok:
            import asyncio
            try:
                loop = asyncio.get_running_loop()
...
def _trigger_unreg_all() -> None:
    ...
    try:
        ...
        import asyncio
        ws = _connector_manager or _kiwoom_connector
        if not ws or not ws.is_connected() or not _login_ok:
            return
        try:
            loop = asyncio.get_running_loop()
...
def schedule_midnight_timer() -> None:
    ...
    if delay <= 0:
        ...
        delay += 86400
    _midnight_timer_handle = loop.call_later(max(delay, 1), lambda: loop.create_task(_on_midnight()))
...
async def _apply_holiday_guard_on_startup(settings: dict | None) -> None:
