...
async def _broadcast_bootstrap_stage(
    ...
    try:
        ...
        logger.warning("[구동] 부트스트랩 단계 브로드캐스트 실패 %s: %s", stage_name, e, exc_info=True)
    ...
    try:
        ...
        if _inputs.get("all_codes"):
            ...
            try:
                ...
                logger.debug("[구동] 업종순위 화면 전송 완료 (접속화면=%d)", _client_cnt)
            except Exception as e:
                logger.error("[구동] UI 초기 전송 실패: %s", e, exc_info=True)
        else:
            # 종목 없음 — 이벤트만 발행 (대기 해제)
            state.sector_summary_ready_event.set()
    except Exception as _e:
        logger.warning("[구동] 업종순위 후순위 계산 실패(무시): %s", _e, exc_info=True)
...
async def _notify_close_data_ui() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.warning("[구동] 매수후보 갱신 실패: %s", e, exc_info=True)
        try:
            await _account_notify.notify_desktop_sector_refresh()
            logger.debug("[구동] 장외 갱신 — 섹터 분석 패널 갱신 트리거")
        except Exception as e:
            logger.warning("[구동] 섹터 갱신 실패: %s", e, exc_info=True)
    except Exception as _e:
        logger.warning("[구동] 장외 확정 데이터 갱신 실패(무시): %s", _e, exc_info=True)
...
async def _login_post_pipeline() -> None:
    ...
    try:
        ...
        if is_test_mode(state.integrated_system_settings_cache):
            logger.info("[구동] 파이프라인 — 테스트모드 — REST 잔고 조회 생략 (가상잔고 사용)")
        elif not _in_ws_window:
            if not state.account_rest_bootstrapped:
                logger.info("[구동] 파이프라인 — REST 잔고 선행 조회 시작")
                ...
                logger.info("[구동] 파이프라인 — REST 잔고 선행 조회 완료 (보유 %d종목)", len(state.positions))
            else:
        ...
        logger.error("[구동] 로그인 후 파이프라인 예외: %s", _e, exc_info=True)
