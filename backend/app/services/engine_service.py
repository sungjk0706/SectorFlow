...
def get_all_sector_stocks() -> list[dict]:
    ...
    업종분류 커스텀 페이지 전용. 각 종목: { code, name, sector(get_merged_sector 기반), market_type, nxt_enable }
    """
...
def get_status() -> dict:
    ...
    st: dict = {
        "running":         _running,
        "kiwoom_connected": (_broker_tokens.get("ls") is not None) if active_broker == "ls" else (_connector_manager.is_connected() if _connector_manager is not None else (_kiwoom_connector.is_connected() if _kiwoom_connector else False)),
        "login_ok":        _login_ok,
