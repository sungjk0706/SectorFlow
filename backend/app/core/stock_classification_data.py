...
def update_sector_in_cache(code: str, sector: str) -> None:
    ...
    캐시에 종목이 없으면 경고 로그 후 생략 (폴백 없음).
    """
    from backend.app.services.engine_state import state
    entry = state.master_stocks_cache.get(code)
    if entry is None:
        logger.warning("[데이터] 종목 %s이(가) 캐시에 없음 — 업종 갱신 생략", code)
        return
    entry["sector"] = sector
...
async def rename_sector(old_name: str, new_name: str) -> None:
    ...
    try:
        ...
        await conn.rollback()
        logger.error("[데이터] 업종명 변경 실패: %s", e)
        raise e
    ...
    try:
        ...
        logger.warning("[시스템] 인메모리 업종명 변경 실패: %s", e)
...
async def move_stock(stock_code: str, target_sector: str) -> None:
    ...
    try:
        ...
        logger.warning("[시스템] 인메모리 종목 업종 갱신 실패: %s", e)
