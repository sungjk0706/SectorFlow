...
def _migrate_remove_scheduler_5d_download_on(merged: dict) -> tuple[dict, bool]:
    """scheduler_5d_download_on 키 제거 (5거래일 자동 다운로드 레거시 제거, P10 SSOT).
    별도 자동 5거래일 다운로드 기능이 제거되어 런타임 참조 0건.
    매일 일봉 확정시세 자동 다운로드(scheduler_market_close_on)는 별도 키로 유지.
    DB 잔존 키 삭제."""
    dirty = False
    if "scheduler_5d_download_on" in merged:
        del merged["scheduler_5d_download_on"]
        dirty = True
    return merged, dirty
