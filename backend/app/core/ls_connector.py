...
class LsConnector(BrokerConnector):
    ...
    async def subscribe_dynamic(self, codes: list[str]) -> bool:
        ...
        UH1(호가) + UPH(프로그램매매) 순차 등록.
        실패가 있을 때만 경고 로그 출력 (P23/P21).
        ...
        if total_fail > 0:
            logger.warning(
                "[구독] %s 호가·프로그램매매 구독 실패 일부 — %d종목 (성공 %d, 실패 %d) %s",
                _BROKER_DISPLAY, len(codes), total_ok, total_fail, codes,
            )
        return total_ok > 0

    async def unsubscribe_dynamic(self, codes: list[str]) -> None:
        """동적 데이터 구독 해지. UH1 + UPH 순차 해지.
        실패가 있을 때만 경고 로그 출력 (P23/P21).
        """
        if not codes:
            return
        ...
        if total_fail > 0:
            logger.warning(
                "[구독] %s 호가·프로그램매매 구독 해지 실패 일부 — %d종목 (성공 %d, 실패 %d) %s",
                _BROKER_DISPLAY, len(codes), total_ok, total_fail, codes,
            )
