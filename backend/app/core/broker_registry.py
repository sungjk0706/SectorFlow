...
# ── 잔고 갱신 완료 대기 타임아웃 설정 (레지스트리 SSOT) ──────────────────────
# 공통 로직(services/)은 이 딕셔너리를 조회하여 증권사별 타임아웃을 적용.
# 증권사 식별자 문자열을 공통 로직에 두지 않고 이곳에서 단일 관리 (P10).
# 범위: 주문 체결 응답 수신 + 잔고 갱신 완료까지 대기 (결정 2·3·5 — 2단계).
#   키움: REAL 04 실시간 잔고 메시지 도착 지연 여유 → 15초
#   LS: t0424 REST 재조회 완료 시간 여유 → 20초
#   가상매매: 동기식 체결·잔고 갱신이므로 타임아웃 도달 전 완료 (영향 없음).
BROKER_TIMEOUTS: dict[str, float] = {"kiwoom": 15.0, "ls": 20.0}
BROKER_TIMEOUT_DEFAULT: float = 15.0
