...
class KiwoomStockProvider:
    ...
    ) -> RawStockFetchResult | None:
        """ka10081 단건 조회 -- 최근 5개 일봉에서 5거래일 평균 거래대금 및 최고가 계산 반환.

        공통 자료 계약(RawStockFetchResult)으로 변환하여 반환 (설계서 4.1, 4.2).
        증권사별 해석은 kiwoom_stock_rest에 유지하고 공통 계약으로 전달만 담당.
        수신 실패 시 raw_payload=None 으로 구분.
        """
        if self._rest_api is None:
            return None
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_daily_5d_data
        detail = await fetch_ka10081_daily_5d_data(self._rest_api, stk_cd, qry_dt)
        if detail is None:
            return RawStockFetchResult(code=stk_cd)
        return RawStockFetchResult(
            code=stk_cd,
            raw_payload=detail,
        )
    ...
    ) -> dict[str, RawStockFetchResult]:
        """전체 종목 ka10081 순차 조회 -- 확정 시세(일봉) 데이터 채우기용.

        공통 자료 계약(RawStockFetchResult)으로 변환하여 반환 (설계서 4.1, 4.3).
        실패 종목도 raw_payload=None 의 RawStockFetchResult로 반환하여 잃지 않음.
        진행률 콜백은 (처리 수, 성공 수, 실패 수, 전체 수)를 전달.
        """
        if self._rest_api is None:
            return {}
        from backend.app.core.kiwoom_stock_rest import fetch_ka10081_all_stocks_daily_confirmed
        raw_result = await fetch_ka10081_all_stocks_daily_confirmed(
            self._rest_api, krx_codes, qry_dt, interval_sec=interval_sec, on_progress=on_progress
        )
        # 공통 자료 계약으로 변환 — 성공·실패 종목 모두 보존 (설계서 4.3)
        results: dict[str, RawStockFetchResult] = {}
        for cd, detail in raw_result.get("success", {}).items():
            results[cd] = RawStockFetchResult(
                code=cd,
                raw_payload=detail,
            )
        for cd in raw_result.get("failures", {}):
            results[cd] = RawStockFetchResult(code=cd)
        return results
