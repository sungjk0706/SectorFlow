...
async def patch_setting_field(field_name: str, body: dict, _: str = Depends(get_current_user)):
    ...
    try:
        ...
        if is_engine_running():
            ...
            # 텔레그램 설정(tele_on·토큰·chat_id)은 엔진 실행 여부와 무관하게
            # 폴링 태스크에 즉시 반영 (원칙 17). 단일 진입: apply_telegram_polling_change.
            from backend.app.services.telegram_bot import (
                TELEGRAM_POLLING_KEYS, apply_telegram_polling_change,
            )
            if changed_keys & TELEGRAM_POLLING_KEYS:
...
async def _reset_positions_and_account() -> None:
    ...
    logger.info("[연산] 보유종목, 실시간 필드 및 REST 보완 저장데이터 리셋")
