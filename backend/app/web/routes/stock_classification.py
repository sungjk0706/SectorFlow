...
async def check_download_data_exists(_: str = Depends(get_current_user)):
    """수동 다운로드 대상 거래일의 데이터 저장 여부 확인 (설계서 결정 5, 세션 4).

    가장 최근 확정된 거래일(소속 거래일의 직전 거래일) 기준으로:
    - 일봉 시세(master_stocks_table.date) 존재 여부 (기존 — 하위 호환)
    - 5거래일 일봉(stock_5d_bars.dt) 존재 여부 (기존 — 하위 호환)
    - 종목별 최근 5거래일 완전성 확인 (신규 — 정상·준비 중·부적합 종목 수,
      빠진 거래일, 거래대금·고가 누락 여부)

    프론트엔드에서 수동 다운로드 버튼 클릭 시 사전 확인용 (P21).
    다운로드 파이프라인이 qry_dt=직전 거래일을 사용하므로, 이 API도 동일 기준으로
    일관성 유지 (P10 SSOT). 장 전 실행 시 07-15가 아닌 07-14 기준으로 존재 여부 판단.

    완전성 검증은 저장 모듈의 ``verify_5d_completeness`` 를 재사용 — 자동·수동이
    같은 완전성 기준을 사용한다 (설계서 4.5, P10 SSOT).
    """
    from backend.app.db.database import get_db_connection
    ...
    from backend.app.services.market_close_storage import _expected_5d_days
    from backend.app.services.market_close_calc import verify_5d_completeness
    from backend.app.services import engine_state
    ...
    # ── 신규: 종목별 최근 5거래일 완전성 확인 (설계서 결정 5, 세션 4) ──────────
    expected_days = _expected_5d_days(trading_day)

    # 매매 적격 종목(active) 코드 — 메모리 캐시 기준 (다운로드 대상과 동일)
    active_codes = {
        cd for cd, entry in engine_state.state.master_stocks_cache.items()
    ...
    has_missing_highs = False

    if expected_days and active_codes:
        # 예상 5거래일에 해당하는 모든 행을 한 번에 조회 (단일 쿼리 — P24 단순성)
        placeholders = ",".join("?" for _ in expected_days)
        cursor = await conn.execute(
            f"""SELECT code, dt, trade_amount, high_price
                FROM stock_5d_bars
        ...
        # 종목별로 그룹화 (active 종목만)
        per_code: dict[str, list[tuple[str, int | None, int | None]]] = {}
        for r in rows:
            code = str(r["code"])
            if code not in active_codes:
                continue
            per_code.setdefault(code, []).append(
                (str(r["dt"]), r["trade_amount"], r["high_price"])
            )

        # 각 종목별 완전성 검증 — 저장 모듈과 동일 기준 (설계서 4.5, P10 SSOT)
        for code in active_codes:
            rows_for_code = per_code.get(code, [])
            dts = [r[0] for r in rows_for_code]
            amts = [r[1] for r in rows_for_code]
            highs = [r[2] for r in rows_for_code]

            is_complete, problems = verify_5d_completeness(
                dts, amts, highs, expected_days,
            )

            if is_complete:
                ok_count += 1
            elif any("행 수 부족" in p for p in problems):
                preparing_count += 1
            else:
                ineligible_count += 1

            # 빠진 거래일 수집 (정상 종목은 빠진 날 없음)
            expected_set = set(expected_days)
            actual_set = set(dts)
            all_missing_days |= (expected_set - actual_set)

            # 값 누락 여부 — problems 메시지에서 판별 (정확성)
            for problem in problems:
                if "거래대금 누락" in problem:
                    has_missing_amounts = True
                if "고가 누락" in problem:
                    has_missing_highs = True
    ...
    return {
        ...
        "missing_days": sorted(all_missing_days),
        "has_missing_amounts": has_missing_amounts,
        "has_missing_highs": has_missing_highs,
