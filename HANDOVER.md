...
## 세션 개요
- 날짜: 2026-07-18 (일반설정 탭 재구성 다단계 작업 5세션 — Step 3 사전 준비 시간 + 거래소 고정 시간 이동 완료)
- 작업: 일반설정 페이지 탭 재구성 다단계 작업 5세션(Step 3) 진행. 사용자 명시적 실행 지시어("5세션 시작해")로 진행 승인. Step 3 "사전 준비 시간 + 거래소 고정 시간 이동": `renderAutoTradeTab()`의 "장 시작 전 사전 준비 시간" 섹션(sectionTitle + 설명 + 3개 슬롯: 실시간 항목 초기화 07:58 / 구독 사전 시작 07:59 / 정규장 사전 구독 08:59) + "거래소 고정 시간 참고" 박스(7개 행: 08:00~20:00) 제거 → `renderTimeSettingsTab()`에 자동매도 시간쌍 행 아래, 안내 문구 위에 이동 (설계서 3-2 시간순 배치 준수). 모듈 상태 변수(`timetableResetSlot/WsSlot/KrxSlot` 등)·`scheduleTimetableSave()` 함수·`syncFromSettings()` 타임테이블 3개 키 동기화 블록은 위치 유지 (모듈 스코프라 탭과 무관하게 동작). `unmount()`는 timetable* 변수 초기화 코드가 기존에도 없었으므로 변경 불필요 (기존 패턴 유지). 백엔드 변경 없음. P10/P16/P21/P23/P24 준거.
- 상태: 5세션(Step 3 구현) 완료. 커밋 완료. **6세션(Step 4: 1일봉 다운로드 이동) 승인 대기.**
...
## 다음 세션 진행 대기: 일반설정 탭 재구성 다단계 작업 6세션 (Step 4: 1일봉 다운로드 이동)
...
- **6세션 (승인 대기)**: Step 4 구현 — 1일봉 다운로드 이동.
  - **예상 작업**: `renderApiSettingsTab()`의 1일봉차트 자동다운로드 행(토글 + 시간 슬롯, `confirmed_download_time` / `scheduler_market_close_on`)을 `renderTimeSettingsTab()`으로 이동. 시간 설정 탭의 거래소 고정 시간 참고 박스 위에 배치 (설계서 3-2 시간순 배치: 사전 준비 시간 → 1일봉 다운로드 → 거래소 고정 시간 참고). API 설정 탭에서 해당 행 제거. `syncFromSettings()`의 확정 시세 다운로드 동기화 블록은 위치 유지 예상 (변수 참조 그대로). `scheduleConfirmedDlSave()` 함수도 위치 유지.
  - **예상 검증**: 빌드 + 브라우저에서 시간 설정 탭에 1일봉 다운로드 토글 + 시간 슬롯 표시 + API 설정 탭에서 해당 행 제거 + 저장 동작 정상
