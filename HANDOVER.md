...
## 세션 개요
- 날짜: 2026-07-18 (일반설정 탭 재구성 다단계 작업 6세션 — Step 4 1일봉차트 자동다운로드 이동 완료)
- 작업: 일반설정 페이지 탭 재구성 다단계 작업 6세션(Step 4) 진행. 사용자 명시적 실행 지시어("6세션 진행해")로 진행 승인. Step 4 "1일봉차트 자동다운로드 이동": `renderApiSettingsTab()`의 1일봉 다운로드 행(토글 + 시간 슬롯 + 설명 문구, `confirmed_download_time` / `scheduler_market_close_on`) 제거 → `renderTimeSettingsTab()`에 정규장 사전 구독 슬롯 아래, 거래소 고정 시간 참고 박스 위에 이동 (설계서 3-2 시간순 배치 준수: 사전 준비 → 1일봉 다운로드 → 거래소 고정 시간 참고). "실시간 데이터 통신" 섹션 제목은 플래시 효과(Step 5 이동 대상)를 위해 API 설정 탭에 유지. 모듈 상태 변수(`confirmedDlSlot/Toggle/H/M/savingConfirmedDl`)·`scheduleConfirmedDlSave()` 함수·`syncFromSettings()` 확정 시세 동기화 블록은 위치 유지 (5세션 패턴 일관성 — 모듈 스코프). `renderTimeSettingsTab()` 함수 주석에 Step 4 반영 갱신. 백엔드 변경 없음. P10/P16/P21/P23/P24 준거.
- 상태: 6세션(Step 4 구현) 완료. 커밋 완료. **7세션(Step 5: 플래시 효과 이동) 승인 대기.**
...
## 다음 세션 진행 대기: 일반설정 탭 재구성 다단계 작업 7세션 (Step 5: 플래시 효과 이동)
...
- **7세션 (승인 대기)**: Step 5 구현 — 플래시 효과 이동.
  - **예상 작업**: `renderApiSettingsTab()`의 실시간 현재가 플래시 효과 토글 행(`ui_price_flash_on`) + "실시간 데이터 통신" 섹션 제목을 `renderAutoTradeTab()` 하단으로 이동. 자동매매 탭 하단에 "화면 표시" 섹션 추가 예정 (설계서 3-1). 모듈 상태 변수 `uiFlashToggle`은 위치 유지 예상 (5/6세션 패턴 일관성). `syncFromSettings()` 플래시 효과 동기화 블록도 위치 유지 예상.
  - **예상 검증**: 빌드 + 브라우저에서 자동매매 탭 하단에 플래시 효과 정상 표시 + 토글 변경 시 백엔드 저장 + API 설정 탭에서 플래시 효과 + "실시간 데이터 통신" 섹션 제거 확인
