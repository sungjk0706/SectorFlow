...
## 세션 개요
- 날짜: 2026-07-18 (P-NEW-5 — 15개 모듈 state 참조 패턴 A → B 단계적 전환 6세션, engine_ws_reg.py + engine_loop.py 2개 모듈 전환)
- 작업: HANDOVER.md P-NEW-5에 기록된 15개 모듈의 `state` 참조 패턴 A(모듈 레벨 `from backend.app.services.engine_state import state`) → 패턴 B(`from backend.app.services import engine_state` + `engine_state.state.X`) 단계적 전환 6세션. 사용자 명시적 실행 지시어("P-NEW-5 이어서 진행")로 진행 승인. 사전조사(규칙 0-2): 의존성 — 백엔드 2개 파일 + 테스트 1개 파일 수정 필요(test_engine_loop.py patch 26곳 변경 + import 추가), test_engine_ws_reg.py는 순수 payload 빌더만 검증하여 테스트 수정 불필요, 프론트엔드 영향 없음. 원칙 부합 — P23(일관성) 5세션까지 전환한 9개 모듈과 동일 패턴 통일, P16(살아있는 경로) 모듈 로드 시점 고정 바인딩 제거, P10/P24 부합. 기존 공통 자산 — daily_time_scheduler.py가 패턴 B 참조 모델. 백엔드 2개 파일 수정: (1) `engine_ws_reg.py` — line 15 모듈 레벨 import 패턴 B 전환 + `state.X` 38곳 → `engine_state.state.X` 치환(master_stocks_cache / connector_manager / active_connector / login_ok / integrated_system_settings_cache / ws_account_subscribed). 함수 내 지역 import 없음. (2) `engine_loop.py` — line 18 모듈 레벨 import 패턴 B 전환 + `state.X` 60곳 → `engine_state.state.X` 치환(broker_tokens / integrated_system_settings_cache / login_ok / connector_manager / active_connector / token_ready_event / master_stocks_cache / running / engine_loop_ref / preboot_cache_loaded / preboot_ready_event / account_rest_lock / broker_spec / access_token / auto_trade / engine_stop_event / ws_window_changed_event / broker_rest_apis). line 144 지역 import(`from backend.app.services.engine_state import _notify_reg_ack`)는 `state`가 아닌 다른 심볼이므로 **유지**. line 46 docstring("발급된 토큰은 state.broker_tokens[broker_id]로 저장한다") + line 160 주석("# state.integrated_system_settings_cache는 app.py에서 이미 초기화됨")은 규칙(주석 무단 수정 금지)에 따라 원본 유지. 테스트 1개 파일 수정: `test_engine_loop.py` — `patch.object(engine_loop, "state", mock_state)` 26곳 → `patch.object(engine_state, "state", mock_state)` 치환 + `from backend.app.services import engine_loop, engine_state` import 추가. 정적 검증: py_compile OK + ruff check OK. pytest: test_engine_loop.py + test_engine_ws_reg.py 56 passed + 전체 회귀 **2928 passed, 0 failed**(이전 세션과 동일). 런타임 기동: `python -W error::RuntimeWarning main.py` 기동 성공 — RuntimeWarning 0건 + "앱 시작 완료" + "타임테이블 빌드 완료 — 11항목" + "업종 재계산 완료" + "LS증권 연결 완료 (테스트모드=True)" + 잔존 프로세스 0건. 사용자 체감 변화 없음(프로덕션 동작 100% 보존 — state 싱글톤 속성 참조 방식만 변경, 동일 객체 접근).
- 상태: P-NEW-5 15개 모듈 중 11개 전환 완료(engine_service.py, engine_strategy_core.py, engine_ws_dispatch.py, engine_sector_confirm.py, sector_data_provider.py, engine_radar.py, market_close_pipeline.py, engine_snapshot.py, ws_subscribe_control.py, engine_ws_reg.py, engine_loop.py). **잔존 5개 모듈**(engine_lifecycle.py, engine_ws.py, engine_config.py, engine_bootstrap.py, engine_cache.py)은 다음 세션에서 진행 → "미해결 문제" P-NEW-5 섹션에 진행 상황 갱신. **본 세션에서 HANDOVER 불일치 발견·수정**: 이슈 제목 "15개 모듈"이나 실제 추적 목록은 16개 항목(1-16번) — 이전 "잔존 6개" 기재는 실제 7개였음(리스트 10-16번). 본 세션에서 2개(10-11번) 전환 후 잔존 5개(12-16번)로 정정.
...
### P-NEW-5: 15개 모듈 state 참조 패턴 A(모듈 레벨 고정 바인딩) 잔존 — P23 일관성 위반 (진행 중 — 11/15 전환 완료)
...
**잔존 모듈 5개** (패턴 A — 모듈 레벨 `from backend.app.services.engine_state import state`):
...
**조치**: 본 세션에서는 `engine_ws_reg.py` + `engine_loop.py` 2개 전환. 잔존 5개 모듈은 다음 세션에서 1~2개씩 단계적 전환.
