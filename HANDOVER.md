...
## 세션 개요
- 날짜: 2026-07-18 (1일봉 다운로드 타임테이블 통합 다단계 작업 4세션 — 타임테이블 통합 + 멱등성 가드 완료 · **5세션 승인 대기**)
- 작업: 1일봉차트 확정 다운로드 시간 DB 타임테이블 통합 다단계 작업 4세션(타임테이블 통합 + 멱등성 가드) 진행. 사용자 명시적 실행 지시어("4세션 진행해")로 진행 승인. 백엔드 3개 파일 수정: `engine_state.py`(`ws_subscribe_timer_handles` 제거 + `last_confirmed_download_date` 추가 — P14 단일 타이머 + P22 멱등성 가드), `daily_time_scheduler.py`(`build_timetable_from_cache()` 11번째 항목 추가 — 토글 OFF 시 엔트리 스킵 P16 + `_on_confirmed_download()` 멱등성 가드 추가 P22 + `_on_midnight()` 가드 리셋 + `schedule_ws_subscribe_timers()` + `_fire_confirmed_download()` 제거 + `start/stop_daily_time_scheduler()` 정리 + `retry_pipeline_catchup_after_bootstrap()` 키 참조 변경), `engine_service.py`(`_WS_SCHEDULE_KEYS` 제거 + `_TIMETABLE_KEYS`에 `timetable.confirmed_download` + `scheduler_market_close_on` 추가 + 분기 재작성 — 단일 타임테이블 재빌드 경로). 테스트 2개 파일: `test_daily_time_scheduler.py`(import 정리 + mock settings 키 치환 + `TestFireWrappers.test_fire_confirmed_download_schedules` 제거 + `TestScheduleWsSubscribeTimers` 전체 제거 + `TestOnConfirmedDownload` 멱등성 가드 테스트 2개 추가 + `TestTimetableBuilder` 11항목/토글 스킵 테스트 2개 추가 + `TestStopDailyTimeScheduler`/`TestStartDailyTimeScheduler`/`TestOnMidnight`/`test_stop_cancels_timetable_timer` 모킹 정리 + `test_schedule_next_event_at_2030_reserves_next_day_0758` → `test_schedule_next_event_at_2030_reserves_2040` 갱신 + 20:45 익일 예약 테스트 추가), `test_engine_settings.py`(`test_confirmed_download_triggers_rebuild` + `test_scheduler_market_close_on_triggers_rebuild` 테스트 2개 추가). 정적 검증: py_compile OK + ruff check OK + pytest test_daily_time_scheduler 219/219 통과 + test_engine_settings+test_settings_store 138/138 통과 + 전체 회귀 2901 passed (기존 실패 1개 `test_trading.py::TestExecuteBuyGates::test_rebu_block_disabled`은 개별 실행 시 통과 — 테스트 격리 문제, 본 수정과 무관, 규칙 4-1 준수). 런타임 기동 검증: `python -W error::RuntimeWarning main.py` 기동 성공 — RuntimeWarning 0건 + "타임테이블 빌드 완료 — 11항목" 로그 확인 + "단절 구간 기동 — 확정 다운로드 시각(20:40) 이전 — 타이머 대기" 로그 확인(키 참조 정상) + 잔존 프로세스 0건. 신규 함수/상수/컴포넌트 생성 없음 — 기존 공통 자산 재사용(P23/P24). 사용자 체감 변화 없음.
- 상태: 4세션(타임테이블 통합 + 멱등성 가드) 완료. 커밋 완료. **5세션(프론트엔드 + 최종 검증 + 계획서 삭제) 승인 대기.**
...
## 1일봉 다운로드 타임테이블 통합 다단계 작업 — 진행 중 (4/5세션)
...
## 다음 세션 진행 대기: 5세션 (프론트엔드 + 최종 검증 + 계획서 삭제)

> 4세션(타임테이블 통합 + 멱등성 가드) 완료. 5세션 진행은 사용자 명시적 실행 지시어 대기.
> 5세션 작업 범위: `types/index.ts` `confirmed_download_time` 제거 → `'timetable.confirmed_download': string` 추가 → `general-settings.ts` `scheduleConfirmedDlSave()` 제거 → `scheduleTimetableSave()` 시그니처 확장 + 4곳 키 참조 변경 + `savingConfirmedDl` 변수 제거 → `ARCHITECTURE.md` 2줄 갱신 → 최종 검증(typecheck + build + vitest + 런타임 기동 + 브라우저 확인) → 설계서 + 태스크 파일 삭제(규칙 11).
