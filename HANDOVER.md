...
- **수익상세 페이지(profit-detail.ts) 기간 선택 상태 유지 미적용** — 후속 작업 대상
  - `selectedView`(당일/당월/누적/드릴다운)와 `dateRangeInput` 초기값이 항상 `monthStart ~ todayStr`로 고정, localStorage 저장 로직 없음
  - 수익현황에 적용한 동일 패턴(localStorage quickLabel 포함 저장/복원) 적용 가능

## 직전 완료 작업
- **2026-07-11: 수익현황 페이지 기간 선택 상태 재기동 후 유지 — 근본 해결**
  - 근본 원인 A: 빠른 버튼 활성 인덱스(`_activeQuickIdx`)가 클로저 변수로만 존재해 영속화되지 않음 → 앱 재기동 시 `-1` 복귀로 모든 버튼 비활성
  - 근본 원인 B: "5일"(days=5), "전체"(days=0) 선택 시 `if (days === undefined) saveProfitDateRange()` 조건으로 localStorage 미저장 → 재기동 시 이전 당일/당월 날짜 복원
  - 수정: `canvas-profit-chart.ts` — `onDateRangeChange` 콜백에 `label?: string` 파라미터 추가, `initialActiveQuickLabel` 옵션 추가로 초기 활성 버튼 복원
  - 수정: `profit-overview.ts` — localStorage에 `quickLabel` 포함 저장/복원, `if (days === undefined)` 조건 제거 후 항상 저장, 초기 API 호출 시 quickLabel 기반으로 `days` 파라미터 사용
  - 검증: `npm run typecheck` + `npm run build` 통과 (59 modules transformed) + `npm test` 112 passed
  - 수정 파일: `canvas-profit-chart.ts`, `profit-overview.ts`
...
## 다음 단계
- **1순위: 수익상세 페이지(profit-detail.ts) 기간 선택 상태 유지 적용**
  - 수익현황에 적용한 동일 패턴(localStorage quickLabel 포함 저장/복원) 적용
  - `selectedView`(당일/당월/누적/드릴다운) + `dateRangeInput` 초기값 영속화
- **2순위: engine_sector_confirm 커버리지 추가 개선 (91.20% → 95%+ 목표)**
