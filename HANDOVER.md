...
## 세션 개요
- 날짜: 2026-07-15 (실시간 자동 연결 토글 라벨 + 설명 개선)
- 작업: 일반설정 API 설정탭의 "실시간 연결" 토글 라벨을 "실시간 자동 연결"로 변경하고, 설명 텍스트에 ON 시 자동 연결/해제 시간(거래일 08:00~20:00, 주말·공휴일 제외) 명시. 사용자가 토글 ON 시 동작 시간을 직관적으로 파악 가능.
- 상태: 구현 + 검증 완료, 커밋 완료.
