...
### 작업: P23 사전 절차 규칙 추가 — "신규 구현 전 기존 공통 자산 먼저 검색" — 완료

**진행 단계**: 완료 (문서 전용 변경, 코드 변경 없음). 다음 작업: 업종 점수 산정 방식 리팩토링 Step 4 사전조사 대기.

**완료 내용 (2026-07-15)**:
- **현상**: 업종순위 슬라이더 추가 시 `createDualLabelSlider`(공통)가 이미 있었는데 `createSlider`(기본형)를 사용해 두 번 수정 발생. 진행률 바 구현 시 `header.ts`에 인라인 패턴이 있었는데 공통 컴포넌트 추출 누락. "기존 공통 자산을 먼저 찾아 활용하라"는 사전 절차가 문서 체계에 없었음.
- **근본 원인**: 기존 P23은 "중복 발견 → 공통 추출(사후)"만 다루고, "신규 구현 전 → 기존 공통 자산 검색(사전)" 절차가 없어, 중복이 만들어진 뒤에야 P23이 트리거되는 구조적 공백.
- **수정 내용**: P23을 실행하는 사전 절차 규칙을 5개 파일에 추가 (P25 신규 원칙이 아닌 절차 규칙으로 배치):
  - `AGENTS.md`: 섹션3 규칙 0-2를 3항목 → 4항목으로 확장 (4번: 기존 공통 자산 확인). 섹션2 백엔드/프론트엔드 체크리스트에 P23 공통 자산 재사용 항목 각 1줄.
  - `.devin/skills/frontend-fix/SKILL.md`, `backend-fix/SKILL.md`, `problem-solve/SKILL.md`: 사전조사 섹션에 1줄씩 추가.
  - `ARCHITECTURE.md`: P23 구현 가이드에 "신규 구현 전 기존 공통 자산 검색" 사전 절차 1줄 보강.
  - safe-trade, db-backup 스킬은 맥락상 제외 (사용자 승인).
  - 모든 파일이 `AGENTS.md 섹션3 규칙 0-2.4`로 동일 교차 참조.
- **영향 범위**: 문서 5개 파일만 변경 (+8/-1). 코드 변경 없음. 향후 모든 코드 수정 시 사전조사 4번째 항목으로 기존 공통 자산 검색 의무화.
- **검증**: `git diff --stat`으로 5개 파일 변경 확인, `git diff` 본문으로 교차 참조 일관성(`섹션3 규칙 0-2.4`)과 P23 용어 통일 항목과의 중복 없음 확인. 런타임/빌드 불필요(문서 전용 변경).
- **P23**: 사후 추출 규칙을 사전 검색 절차로 보완.
...
**진행 단계**: 완료 (커밋 `9b9c1e9`). 다음 작업: 업종 점수 산정 방식 리팩토링 Step 4 사전조사 대기.
...
### 작업: 업종순위 수신률 표시 개선 — 진행률 바 + 임계치 마커 + 동적 색상 + 상태 라벨 — 완료

**진행 단계**: 완료 (커밋 `b0a5d68`). 다음 작업: 업종 점수 산정 방식 리팩토링 Step 4 사전조사 대기.

**완료 내용 (2026-07-15)**:
- **현상**: 업종순위 설정 ② 영역이 퍼센트 숫자만 표시되어 직관적 파악이 어려움. 라벨("업종순위 계산 수신율")이 모호했으며, 수신률과 종목 수가 분리되어 한눈에 상태 파악이 안 됨. 진행률 바 없음, 임계치 위치 표시 없음, 색상 고정.
- **근본 원인**: `sector-settings.ts:120-170` — 진행률 바 없이 텍스트만 표시, 라벨이 기술적 표현("수신율"), 설정 입력란과 상태 표시가 같은 행에 혼재. fill 색상이 고정(파랑)으로 표시되어 임계치 대비 현재 수신률 단계를 색상으로 파악 불가.
- **수정 내용**: 프론트엔드 2파일 (신규 1 + 수정 1, +205/-27):
  - `components/common/progress-bar.ts` (신규): 진행률 바 공통 컴포넌트. fill 바 + 임계치 마커(세로선) + 우측 % 텍스트. `setValue(pct)` / `setThreshold(pct)` 인터페이스. `_computeDynamicColor()` — 임계치 기준 3구간 색상: 빨강(COLOR.up, 미달) → 주황(COLOR.warning, 근접) → 초록(COLOR.success, 도달). `setThreshold` 호출 시 자동 색상 변화 모드 활성화. 미호출 시 기존 고정 색상 유지 (후방 호환). fill/pctSpan에 `transition: background/color 0.3s ease` — 부드러운 색상 전환.
  - `pages/sector-settings.ts` (수정): ② 영역 4행 구조로 재구성: 1행 임계치 설정 입력란 (라벨 "업종순위 계산 수신율" → "업종순위 계산 임계치" 명확화), 2행 동적 상태 라벨 ("업종순위 계산 대기 중" 파랑 / "업종순위 계산 진행 중" 초록), 3행 진행률 바 + 임계치 마커 + 우측 % 수치, 4행 수신/미수신 종목 수 + "(임계치 N%)" 정보. 임계치 변경 시 마커 위치 + 임계치 표시 + 상태 라벨 자동 갱신 (syncFromSettings에 갱신 로직 추가). 수신률 변경 시 진행률 바 fill + % 수치 + 종목 수 + 상태 라벨 자동 갱신 (uiStore 구독). 동적 라벨 판정: 프론트엔드 로컬 `pct >= threshold` (A 방식 — 백엔드 변경 없음).
- **영향 범위**: 프론트엔드 2파일. `progress-bar.ts`는 신규 공통 컴포넌트이므로 기존 페이지 영향 없음. `header.ts`의 인라인 진행률 바는 별도 패턴이므로 이번에 교체하지 않음 (후속 리팩토링 가능). 백엔드, DB, 거래 로직 변경 없음.
- **검증**: TypeScript typecheck 통과, 빌드 통과 (2.10s). 브라우저 확인 필요 — 업종순위 설정 페이지 ② 영역에서 진행률 바 + 임계치 마커 + 동적 색상(빨강→주황→초록) + 동적 라벨(대기 중/진행 중) + 종목 수가 정상 표시되는지, 임계치 변경 시 마커 이동 + 색상 즉시 재계산되는지 확인.
- **P21/P23/P24**: 진행률 바 + 동적 라벨 + 색상으로 현재 상태 직관적 전달 (투명성), COLOR 상수 재사용 + 진행률 바 공통 컴포넌트 추출 (일관성), 백엔드 변경 없이 프론트엔드 로컬 판정 + 3구간 단순 분류 + 후방 호환 유지 (단순성).
