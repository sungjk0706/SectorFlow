...
| 날짜 | 세션 | 작업 | 상태 |
|------|------|------|------|
| 2026-07-24 | GS-S3 | 일반설정 탭 재분류 다단계 워크플로우 3세션 — Step 1 파일 분할 (1443줄 → 7개 파일, 순수 이동) — P10/P23/P24 | 완료 (Step 1, Step 2 대기) |
...
**일반설정 탭 재분류 다단계 워크플로우 (진행 중)**:
- Step 2 (4세션) — UI 변경: 자동매수/매도 토글 시간 설정 탭 이동(통합 행) + 자동매매 탭 상태 배지 + 뉴스/화면 설정 탭 신설 (5→7개 탭). 태스크 파일 `docs/plan_general_settings_redesign.md` Step 2, 설계서 `docs/architecture_general_settings_redesign_design.md` 참조.
...
- F-04 잔여 파일 분할 (stock-classification.ts 1618줄) — general-settings.ts는 본 워크플로우 Step 1에서 분할 완료
