...
## 세션 개요
- 날짜: 2026-07-16 (실시간 시세 0/- 혼용 근본 해결 — DB NULL→None 보존 통일)
- 작업: 업종별 종목 시세/보유종목 테이블에서 현재가·대비=0, 등락률·체결강도·거래대금=- 혼용 문제의 근본 원인 해결. DB 로드 시 NULL 폴백(or 0) 제거, None 보존 패턴으로 4개 실시간 필드 통일.
- 상태: 구현 + 검증 완료, 커밋 완료.
