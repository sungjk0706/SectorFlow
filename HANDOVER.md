...
## 세션 개요
- 날짜: 2026-07-18 (P-NEW-5 — 15개 모듈 state 참조 패턴 A → B 단계적 전환 7세션, engine_lifecycle.py + engine_ws.py 2개 모듈 전환)
- 작업: HANDOVER.md P-NEW-5에 기록된 15개 모듈의 `state` 참조 패턴 A(모듈 레벨 `from backend.app.services.engine_state import state`) → 패턴 B(`from backend.app.services import engine_state` + `engine_state.state.X`) 단계적 전환 7세션. 사용자 명시적 실행 지시어("P-NEW-5 이어서 진행")로 진행 승인. 사전조사(규칙 0-2): 의존성 — 백엔드 2개 파일 + 테스트 1개 파일 수정 필요(test_engine_ws.py patch 33곳 변경 + docstring 1곳), test_engine_lifecycle.py는 존재하지 않아 테스트 수정 불필요, 프론트엔드 영향 없음. 원칙 부합 — P23(일관성) 6세션까지 전환한 11개 모듈과 동일 패턴 통일, P16(살아있는 경로) 모듈 로드 시점 고정 바인딩 제거, P24(단순성) engine_ws.py 함수 내 지역 import 2곳 제거, P10/P22 부합. 기존 공통 자산 — daily_time_scheduler.py가 패턴 B 참조 모델. 백엔드 2개 파일 수정: (1) `engine_lifecycle.py` — line 15 모듈 레벨 import 패턴 B 전환 + `state.X` 57곳 → `engine_state.state.X` 치환(engine_task/engine_user_id/running/integrated_system_settings_cache/engine_stop_event/ws_account_subscribed/quote_subscribed/ws_connection_status/account_rest_bootstrapped/login_ok/access_token/account_snapshot/broker_rest_totals/positions/snapshot_history/auto_trade/data_ready_event/bootstrap_event/sector_summary_ready_event/ws_reg_pipeline_done/master_stocks_cache/sector_summary_cache/connector_manager/active_connector/broker_tokens/engine_loop_ref). line 173 주석("# broker별 실제 연결 상태 (state.broker_tokens 기반)")은 규칙(주석 무단 수정 금지)에 따라 원본 유지. 함수 내 지역 import 없음. (2) `engine_ws.py` — line 10 모듈 레벨 import 패턴 B 전환 + `state.X` 50곳 → `engine_state.state.X` 치환(connector_manager/active_connector/reg_seq_lock/reg_ack_event/reg_ack_return_code/REG_POST_ACK_GAP_SEC/ws_reg_pipeline_done/master_stocks_cache/login_ok/integrated_system_settings_cache). line 256, 268 함수 내 지역 import 2곳(subscribe_dynamic_data/unsubscribe_dynamic_data) 제거(P24 단순성 — 모듈 레벨 import로 충분). 테스트 1개 파일 수정: `test_engine_ws.py` — 28곳 단일 patch `patch("...engine_ws.state", ...)` → `patch("...engine_state.state", ...)` 치환 + 5곳 이중 patch(line 452/462/473/484/494) `engine_ws.state` patch 라인 제거 후 `engine_state.state` 단일 patch로 승격 + line 54 docstring "engine_ws.state 기본 mock" → "engine_state.state 기본 mock" 갱신(테스트 docstring은 코드와 일치해야 함 — Code Removal Rules). 정적 검증: py_compile OK + ruff check OK. pytest: test_engine_ws.py 42 passed + 전체 회귀 **2928 passed, 0 failed**(이전 세션과 동일). 런타임 기동: `python -W error::RuntimeWarning main.py` 기동 성공 — RuntimeWarning 0건 + "앱 시작 완료" + "타임테이블 빌드 완료 — 11항목" + "업종 재계산 완료" + "LS증권 연결 완료 (테스트모드=True)" + 잔존 프로세스 0건. 사용자 체감 변화 없음(프로덕션 동작 100% 보존 — state 싱글톤 속성 참조 방식만 변경, 동일 객체 접근).
- 상태: P-NEW-5 15개 모듈 중 13개 전환 완료(engine_service.py, engine_strategy_core.py, engine_ws_dispatch.py, engine_sector_confirm.py, sector_data_provider.py, engine_radar.py, market_close_pipeline.py, engine_snapshot.py, ws_subscribe_control.py, engine_ws_reg.py, engine_loop.py, engine_lifecycle.py, engine_ws.py). **잔존 3개 모듈**(engine_config.py, engine_bootstrap.py, engine_cache.py)은 다음 세션에서 진행 → "미해결 문제" P-NEW-5 섹션에 진행 상황 갱신.
...
### P-NEW-5: 15개 모듈 state 참조 패턴 A(모듈 레벨 고정 바인딩) 잔존 — P23 일관성 위반 (진행 중 — 13/15 전환 완료)
...
**잔존 모듈 3개** (패턴 A — 모듈 레벨 `from backend.app.services.engine_state import state`):
