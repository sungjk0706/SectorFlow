...
- **2026-07-15**: P-001 Step 1·2 완료 — 틱 수신 빈 FID 0 폴백 제거 (None 유지), `_has_any_realtime_data` 검증 완료.
...
### 1. P-001 Step 3 (최우선 — 사용자 승인 후)
- **Step 1 완료**: `engine_radar.py:73-77` 틱 수신 폴백 제거 (커밋 0c0d018)
- **Step 2 완료**: `pipeline_compute.py:628-631` 보유종목 틱 rate/diff 빈 FID 0 폴백 제거 + `_has_any_realtime_data` 검증 (커밋 7023c24, f5047b6)
- **Step 3 대기**: `sector_calculator.py:69,78` 업종 점수 폴백 제거 (영향 범위 가장 넓음·위험도 높음)
- Step 3 시작 시 사용자 명시적 승인 필요

### 2. KRX 수신률 미표시 문제 추가 조사 (타이머 미실행 근본 원인)
...
- **잔존 원인 (Step 3 대기)**: `sector_calculator.py:69,78`에서 None을 0으로 폴백 — 원인 A·B 수정 후에도 업종 점수에 0 왜곡 잔존. Step 3에서 해결 예정.
- **영향 경로**: 0 폴백 → 수신률 100% 왜곡 → 임계값 통과 → 업종순위 계산 시작 → 1·2·3차 가산점 모두 0 왜곡 포함 → 업종 점수 순위 왜곡
- **관련 원칙**: P10, P20, P21, P22, P23
- **검증 방법 (Step 3 수정 후)**: 틱 일부만 수신 시 수신률이 100%가 아닌 실제 비율 표시, 0/- 표시 종목이 일관되게 "-" 표시, 업종 점수 순위가 0 왜곡 없이 계산
