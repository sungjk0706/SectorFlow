...
- **2026-07-11: 테스트 커버리지 Stage 8 (P4-b) — 대형 브로커 통신 3개 파일, 신규 250건, 전체 1922건 통과**
  ...
  - **`test_kiwoom_rest.py` (90건)**: `TokenInfo` `is_expired_soon` 5건 (미만료/만료임박/짧은 dt/파싱 예외/커스텀 buffer), `KiwoomRestAPI` `__init__`/context 5건 (파라미터 strip/기본 base_url/trailing slash 제거/빈 크리덴셜/sync+async context manager), `_get_client`/`_reset_client` 5건 (신규 생성/재사용/정상 종료/no client/이미 closed), `_ensure_token` 3건 (유효 토큰/만료→발급/토큰 없음→발급), `get_spec` 3건 (성공/no router/예외→None), `_call_api` 6건 (성공+adaptive delay 축소/429→성공+adaptive delay 증가/HTTP 500/예외 재시도 실패/토큰 없음/성공 시 delay 감소), `_issue_token` 7건 (성공/크리덴셜 없음/HTTP 실패/토큰 필드 없음/429→성공/access_token 필드/예외 재시도), `revoke_token` 4건 (성공/토큰 없음/HTTP 실패/예외), `get_access_token`/`get_auth_headers` 4건 (성공/실패 각), `_request` 5건 (성공/토큰 없음/HTTP 500/예외/429→성공), `_paginated_request` 4건 (단일 페이지/토큰 없음/연속조회 2페이지/HTTP 실패), `get_account_number` 3건 (정상/from body/no data), `get_deposit_detail` 2건 (성공+qry_tp/acnt_no 기본값), `get_balance_detail` 1건 (위임), `fetch_ka10081_daily_price`/`fetch_ka10081_sector_all` 2건 (위임), `fetch_market_stock_name_map` 1건 (NotImplementedError), `fetch_ka20001_index` 6건 (정상/sign 4 음수/sign 5 음수/resp None/JSON 파싱 실패/pric fallback), `fetch_ka10099_full` 7건 (숫자 코드/알파벳 코드/resp None/빈 list/no list key/짧은 코드 패딩/예외), `fetch_ka10099_market_code_list`/`fetch_market_stock_list` 2건 (위임), `fetch_ka10001_nxt_enable` 6건 (정상/중첩 output/중첩 output list/resp None/예외/no nxtEnable→N), `fetch_nxt_enable_map` 2건 (배치/빈 리스트), `kiwoom_try_token` 6건 (성공/크리덴셜 없음/HTTP 실패/토큰 필드 없음/access_token 필드/예외)
  - **`test_ls_rest.py` (58건)**: `LsTokenInfo` `is_expired` 4건 (미만료/만료/issued_at=0/커스텀 buffer), `LsRestAPI` `__init__`/context 5건 (파라미터 strip/기본 base_url/trailing slash/빈 크리덴셜/async context manager), `ensure_client` 3건 (신규 생성/재사용/루프 변경), `ensure_token`/`get_token` 4건 (유효 토큰/만료→발급/토큰 없음→발급/get_token 2건), `_issue_token` 7건 (성공/크리덴셜 없음/HTTP 실패/토큰 필드 없음/429→성공/no client/예외 재시도), `revoke_token` 5건 (성공/토큰 없음/HTTP 실패/예외/no client), `call_api` 7건 (GET 성공/POST 성공/429→성공/HTTP 500/예외/토큰 없음/no client), `call_tr` 8건 (성공+tr_cont 추출/rsp_cd 실패/429→성공/HTTP 500/예외/토큰 없음/no client/tr_cont header fallback), `buy_order` 7건 (성공/HTTP 실패/429→성공/예외/토큰 없음/no client/rsp_cd 실패), `sell_order` 4건 (성공/HTTP 실패/예외/토큰 없음), `get_balance`/`get_daily_history`/`get_themes` 3건 (call_api 위임)
  - 수정 중 이슈:
    - `_LsSocket.disconnect()`: `await self._recv_task` → AsyncMock는 `await` 불가 → 커스텀 `_FakeTask` 클래스 구현 (`__await__`에서 `CancelledError` 발생)
    - `LsRestAPI.__aenter__`: `self._client = httpx.AsyncClient()`로 덮어쓰므로 → `httpx.AsyncClient` 자체를 patch
    - `LsRestAPI.ensure_token()`: `buffer_seconds` 인자 없음 → `LsTokenInfo(expires_in=100)`으로 만료 유도
    - `build_broker_urls` import 경로: `ls_rest.py`에서 import하므로 `backend.app.core.ls_rest.build_broker_urls` patch
    - `mock_socket.send.call_args_list` 인덱싱: `c.call_args[0][0]` → `c[0][0]` 패턴 (call_args_list 직접 인덱싱)
  - 검증: 전체 1922 passed, 0 failed (기존 1672 + 신규 250), regression 없음, 런타임 기동 정상 (101ms 부트, 에러 없음, 잔존 프로세스 0건)
  - 커밋: `13ad817`

- **2026-07-11: 테스트 커버리지 Stage 7 (P4-a) — 소형 브로커 3개 파일, 신규 123건, 전체 1672건 통과**
  - 목적: 테스트 커버리지 Stage 7 — 소형 브로커 `kiwoom_order.py`(93줄) + `ls_providers.py`(195줄) + `connector_manager.py`(269줄) 단위 테스트 작성
  - **`test_kiwoom_order.py` (30건)**: `resolve_exchange` 10건 (_NX 접미사/대소문자/공백, exchange_mode KRX/NXT/SOR, 빈 코드/None/무효값→SOR), `_send_request` 5건 (성공/HTTP 500 재시도 실패/예외 재시도 실패/2회차 성공/1회 성공), `send_order` 10건 (BUY/SELL 성공, 알 수 없는 주문 타입, rt_cd non-zero, 통신 장애, NXT trde_tp P/U 조정+ord_uv clear, SOR trde_tp 유지, 주문 타입 대소문자), `market_sell` 2건 (send_order SELL 위임/api-id kt10001, 통신 장애), `get_unexecuted_orders` 4건 (all_stk_tp 1/0 분기, 통신 장애, rt_cd non-zero)
  - **`test_ls_providers.py` (39건)**: `LsAuthProvider` 11건 (신규/캐시 재사용/캐시 저장, app_key/secret 전달/strip/누락, get_access_token 성공/실패, ensure_token 위임, broker_name, rest_api property), `LsAccountProvider` 10건 (acnt_no 추출/누락/공백 보존, rest_api 없음, get_account_number, get_deposit_detail 성공/실패/no rest_api, get_balance_detail 성공/None/no rest_api), `get_account_balance` 7건 (no rest_api, 실패 rsp_cd, None 응답, 성공+stock_list, zero qty 필터, 빈 stock_list, total_rate 계산), `LsOrderProvider` 9건 (no rest_api, buy/sell 성공, buy/sell 실패, None 응답, A 접두사 buy/sell), `LsWebSocketProvider` 2건 (get_ws_uri 반환/호출)
  - **`test_connector_manager.py` (54건)**: `__init__`/`_build` 5건 (단일/다중/알수없는 스킵/빈/websocket None 폴백), `_create_single` 3건 (성공/알수없는 증권사/no create_connector), 콜백 5건 (message_callback 전파/저장/빈, reconnect_callback 지원/미지원), `connect_all` 4건 (성공/빈/일부 실패/reconnect 콜백 설정), `_on_reconnect_success` 2건 (restore 호출/예외 처리), `disconnect_all` 3건 (성공/빈/일부 실패), 상태조회 9건 (is_connected any/all/빈, get_connector 있음/없음, active_broker_ids/빈, broker_id 첫번째/None), `send_message` 2건 (연결됨/연결없음), `subscribe_stocks` 5건 (단일/연결없음/다중 분산/일부 실패/sub_codes 추적), `unsubscribe_stocks` 5건 (구독된 커넥터/매치 없음 폴백/전체/실패/연결안됨), `subscribe_account`/`subscribe_index` 3건, `supports_ack` 3건, `unsubscribe_all` 3건, `subscribe_dynamic`/`unsubscribe_dynamic` 2건
  - 수정 중 이슈:
    - `kiwoom_order.py`: `trde_tp="3"`일 때 `ord_uv`는 항상 `""` (소스: `ord_uv = "" if str(trde_tp) == "3" else str(price)`) — 테스트 assertion 수정
