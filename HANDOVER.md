...
**배경**: P25 수정 계획 Tier 3 넷째 세션. T3-S12b(B1-02-05/06/07) 완료 후 진행. 사전조사 → 수정 계획 보고 → 승인(12곳 모두 수정) → 수정 진행.

**작업 내용** (총 12곳 = silent pass 1 + exc_info 누락 11):
1. **silent pass 제거 (1곳)** — `market_close_pipeline.py:492-493` `except (ValueError, TypeError): pass` (strength 변환 실패 시 조용히 무시) → `logger.warning("[데이터] strength 변환 실패 (코드=%s, 값=%r)", raw_cd, str_val, exc_info=True)`. P20 직접 위반 해소.
2. **exc_info=True 누락 보강 (11곳)**:
   - `market_close_pipeline.py` (5곳): line 425, 860, 944, 1113, 1265 — 전체 매핑 DB 조회 실패(2곳 동일 메시지, replace_all로 한번에 처리), 필터 요약 전송 실패, 종목분류 페이지 갱신 전송 실패(무시), 다운로드 오류 발생
   - `daily_time_scheduler.py` (6곳): line 1274, 1288, 1328, 1355, 1447, 1508 — 구독 등록 파이프라인 실행 오류, 구독 해지 오류, 구독 해지 전송 오류, 자동매매 전환 콜백 오류, 자정 콜백 오류, 타이머 초기 예약 실패

**수정 파일**: 2개 (백엔드).
- `backend/app/services/market_close_pipeline.py` (6곳, +6/-6)
- `backend/app/services/daily_time_scheduler.py` (6곳, +6/-6)

**아키텍처 원칙 부합**:
- P20 (폴백 금지): 본 위반 직접 해결 — silent `except: pass` 제거 + 모든 예외 로깅에 `exc_info=True` 적용. 스택트레이스 확보로 장애 원인 추적 가능.
- P25 (격리된 실패): 기존 격리 구조(try/except + 계속 진행) 유지, 로깅만 보강. 한 컴포넌트 실패가 전체 루프 중단으로 이어지지 않음.
- P23 (일관성): 이미 다수 적용된 `logger.warning(..., exc_info=True)` 패턴과 일치. 신규 패턴 없음.
- P16 (살아있는 경로): dead code 발생 없음 — 모두 실제 예외 처리 경로.
- P24 (단순성): 키워드 인자 1개 추가만, 신규 추상화 없음.

**영향 범위**: 백엔드 2개 파일. 프론트엔드/테스트 영향 없음. 핵심 매매 로직 아님 (로깅 시그니처만 변경) → 규칙 0-4 해당 없음. 롤백 아님 (신규 보강만) → 규칙 0-3 해당 없음.

**UI 기준 화면 변화 (규칙 0-4)**:
- 화면(브라우저) 변화 없음. 백엔드 로그 파일(`backend/logs/trading_*.log`)에서 예외 발생 시 스택트레이스가 함께 기록되는지로 확인. 예외가 발생하지 않으면 로그에도 변화 없음.
...
**다음 세션 대기 사항**:
- **다음 세션: A3-07-08/09/10 통계 카드/라우트/addEventListener (프론트엔드)** — 사용자 지시. P25 프론트엔드 위반 3건.
- **Tier 3 잔여**: A3-07-08/09/10(프론트, 다음 세션), B5-08-01/02/04(백엔드, safe-trade 필수 + 규칙 0-4 핵심 로직). 총 2세션.
- **진행 순서**: 사용자 지시 — A3-07-08/09/10 → B5-08-01/02/04.
