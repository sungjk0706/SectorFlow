...
**배경**: 5일/전체 버튼은 `from`/`to` 없이 `days`만 가짐. quickBtn 클릭 핸들러가 `setValue('', '')` 호출 → `<input type="date">` 빈 값 → 회색 placeholder 표시. 데이터는 5일치/전체가 오지만 화면 입력란은 빈 값이라 사용자가 현재 조회 범위를 알 수 없음.

**수정 내용**:
- `applyDateRange()`를 라벨 하드코딩('직전' 분기)에서 **데이터 상태 기반 분기**로 일반화:
  - `needsRangeFill = !from || !to` — from/to가 빈 문자열인 quickRange(직전/5일/전체) 감지
  - `days === undefined` → 직전 케이스: 백엔드 `getPrevTradingDay()`에서 단일 거래일 조회
  - `days !== undefined` → 5일/전체 케이스: `getDailySummary()` 응답 데이터에서 첫/마지막 항목 `date`로 actualFrom/actualTo 추출
  - `needsRangeFill`인 경우 `chart.setDateRange(actualFrom, actualTo, label)`로 입력란 동기화
- 라벨 하드코딩 제거 → 라벨 변경에도 로직 깨지지 않음 (P23 일관성)

**검증 결과**: typecheck 통과, 빌드 성공 (62 modules).

**영향 범위**: 프론트엔드 1개 파일만 변경. 백엔드 변경 없음 (기존 응답 데이터에서 추출). 당일/당월/직전 버튼 동작 변화 없음.

**아키텍처 원칙 부합**: P23 (일관성 — 라벨 하드코딩 대신 데이터 상태 기반 분기), P20 (폴백 금지 — 빈 값을 그대로 두지 않고 실제 데이터에서 추출), P24 (단순성 — applyDateRange 하나의 흐름으로 3개 케이스 통일).
...
**검증 결과**: typecheck 통과, 빌드 성공, 백엔드 런타임 정상 응답 확인.
...
### 2. 헤더 flex 비율 조정 — 우측 필터요약 영역 확대 — `stock-classification.ts` (커밋 `5ba0b06`)

**배경**: 1차 커밋 후 앱 기동하여 시각 확인 결과, 우측 영역이 여전히 좌측과 동일 비율(flex:1)이라 텍스트가 짤림. 중앙 영역(flex:5)이 비어있음에도 5배 너비를 차지하는 것이 원인.

**수정 내용**: 헤더 flex 비율 조정 — 좌:중:우 = 1:5:1 → 1:1:3. 중앙(현재 비어있음) 축소, 우측(필터요약) 확대하여 전체 너비 절반 이상 차지.

**검증 결과**: typecheck 통과, 빌드 성공.

**영향 범위**: 프론트엔드 1개 파일, `buildTripleHeader()` 내 flex 값 2개만 변경.
