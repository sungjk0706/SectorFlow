...
2. 사용자에게 질문하여 요구사항 확인 — **problem-solve 섹션 1-1 "사용자 의도 파악 질문 프로세스" 적용** (사용자 의사소통 규칙 1~4 준수). 모호한 항목만 선별 질문(과잉 질문 금지 — P24 단순성).
3. 디자인 파일 작성: `docs/architecture_*_design.md` (설계안 비교표 + 선택안 동작 원리 + 표준 검토 근거 + **사용자 결정 항목** — problem-solve 섹션 1-1 "질문 결과 기록" 의무: 질문·답변을 "사용자 결정 항목" 섹션에 기록, 2세션 태스크 파일에서 그대로 활용)
...
3. 단계별 태스크 파일 작성: `docs/plan_*.md` (구현 Step + 세션 분할 + 테스트 계획 + 런타임 검증 방법 + 사용자 결정 항목)
