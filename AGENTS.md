...
4. **완료 작업 정리 (규칙 7 연계)**: 새 세션 시작 시 `HANDOVER.md`의 모든 섹션에서 완료된 항목 정리 (규칙 7 준수). "직전 완료 작업"은 최근 1~2건, "세션 개요"는 최근 3건만 유지하고 과거 완료 작업은 삭제. "미해결 문제"·"다음 세션 진행 대기"에서 완료 항목 제거.
...
- `세션 개요` (최근 3건) · `직전 완료 작업` (최근 1~2건, 항목당 3줄 이하) · `다음 세션 진행 대기` (완료 항목 즉시 제거, 항목당 3줄 이하) · `미해결 문제` (규칙 9, 완료 항목 즉시 제거)
...
7. **완료 작업 정리**: `HANDOVER.md`의 모든 섹션에서 완료된 항목은 즉시 정리. git history에 이미 기록되어 있으므로 중복 누적 방지.
   - **정리 시점 (2개)**:
     ...
     2. **새 세션 시작 시**: "직전 완료 작업"·"세션 개요"·"현재 상태"·"다음 단계" 섹션 최신 상태로 업데이트. **미집행 시 강제 압축** — 직전 완료 작업이 2건 초과 누적 시, 새 세션 시작 절차에서 최근 2건만 남기고 나머지 제거 (git history 참조).
   - **섹션별 유지 건수 + 줄수 상한**:
     - `직전 완료 작업` — 최근 1~2건, **항목당 3줄 이하** (수정 파일 + 검증 결과 + 커밋 해시만, 상세는 git history 참조)
     - `세션 개요` — 최근 3건
...
8-1. **HANDOVER.md 표준 섹션 구조 (P23 일관성)**: `HANDOVER.md`는 **인덱스 역할**만 수행 (규칙 8-2). 다음 표준 섹션 구조 유지. **표준 섹션 외 임의 섹션 생성 금지** (예: "해결 완료 문제 (참고용)" 같은 보관용 섹션 생성 금지 — 완료된 항목은 git history가 단일 진실 소스이므로 HANDOVER에서 제거만 수행).
    - `세션 개요` — 날짜, 작업, 상태, 참조 문서(계획서 경로). 최근 3건 유지 (규칙 7).
    - `직전 완료 작업` — 최근 1~2건, **항목당 3줄 이하** (수정 파일 + 검증 결과 + 커밋 해시). 상세 구현 내역은 git history 참조 (규칙 7, 8-2).
