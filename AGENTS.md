...
7. **완료 작업 정리**: `HANDOVER.md`의 모든 섹션에서 완료된 항목은 즉시 정리. git history에 이미 기록되어 있으므로 중복 누적 방지.
   - **정리 시점 (2개)**:
     ...
     2. **새 세션 시작 시**: "직전 완료 작업"·"세션 개요"·"현재 상태"·"다음 단계" 섹션 최신 상태로 업데이트.
   - **섹션별 유지 건수**:
     - `직전 완료 작업` — 최근 1~2건
     - `세션 개요` — 최근 3건
     - `미해결 문제` — 완료된 항목 즉시 제거 (해결 완료 보관용 섹션 생성 금지 — 규칙 8-1)
     - `다음 세션 진행 대기` — 완료된 항목 즉시 제거
...
8-1. **HANDOVER.md 표준 섹션 구조 (P23 일관성)**: `HANDOVER.md`는 다음 표준 섹션 구조 유지. **표준 섹션 외 임의 섹션 생성 금지** (예: "해결 완료 문제 (참고용)" 같은 보관용 섹션 생성 금지 — 완료된 항목은 git history가 단일 진실 소스이므로 HANDOVER에서 제거만 수행).
    - `세션 개요` — 날짜, 작업, 상태, 참조 문서(계획서 경로). 최근 3건 유지 (규칙 7).
    - `직전 완료 작업` — 최근 1~2건 (규칙 7 준수)
    - `다음 세션 진행 대기` — 진행 중 작업의 단계 진행 상황 + 승인 대기 항목 + 참조 문서(계획서 경로). 완료 항목 즉시 제거 (규칙 7).
