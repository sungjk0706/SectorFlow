...
- **사용자 의도 파악 질문 (모호함 감지 시 — problem-solve 스킬 섹션 1-1 참조)**: 사전조사 완료 후 사용자 지시에 수치/기간/위치/범위/예외처리 중 모호한 항목이 있으면 problem-solve 스킬 섹션 1-1의 질문 카테고리(수치/기간 파라미터, UI 조작 위치/UX, 범위/예외 케이스, 우선순위/충돌, 검증 기준)에서 선별하여 질문. 명확한 항목은 질문 생략 (과잉 질문 금지 — P24 단순성). 단일 버그 수정은 모호성 감지 시에만 1~2개 질문(경량 적용). 거래 로직 수정 시 problem-solve 1-1 적용 후 safe-trade의 거래 특화 질문(실전/모의 전환, 주문 경로 영향, 리스크 임계값)을 추가로 적용.
- **수정안 제시 원칙 (AGENTS.md 섹션3 규칙 0-6 준수)**: 조사 결과(수정 계획) 제시 시 (a) 버그 수정이면 아키텍처 원칙에 가장 부합하는 근본 해결안 1개만 제시 (임시방편/땜빵/우회 금지, 선택지 나열 금지), (b) 신규 구현이고 사용자 요청이 아키텍처 원칙 위반이면 위반 사실(위반 원칙 번호+사유) 경고 + 원칙 부합 대안 제시. 사용자는 코딩 지식이 없어 본인 요청의 원칙 위반 여부를 스스로 판단할 수 없으므로 에이전트 경고·대안 제시 의무 필수.
...
정적 검증(섹션 3) + 런타임 기동(섹션 4) + 테스트(섹션 5) 중 어느 단계에서든 실패/오류 발생 시, 규칙 4-1(실패 추적) 후 원인 수정 → 재검증을 **전부 통과할 때까지 자동 반복**. 사용자 개입 없이 루프 종료 조건까지 순환.
- **종료 조건**: py_compile + pytest(해당 범위) + 런타임 기동(`-W error::RuntimeWarning`) 전부 pass + 잔존 프로세스 0건(규칙 5-1).
- **중단 조건**: 3회 반복 후 동일 실패 지속 시 사용자 보고 후 대기. 루프 중 아키텍처 원칙 위반 발견 시 규칙 2-1 즉시 중단+보고. 수정 범위가 원 계획 벗어남 시 규칙 3-1(범위 외 구현 금지) 적용.
- **규칙 0-1(세션당 1단계)과의 관계**: 검증 루프 내 수정 사이클은 1단계 내 허용. 단, 루프 중 규칙 0-3(롤백)·0-5(사용자 설계 로직 변경) 해당 시 중단 + 승인받아야 재개.
- **보고**: 섹션 6 보고에 루프 반복 횟수, 해결된 실패 목록, 최종 통과 상태 명시 (규칙 0-7).
...
- 검증한 항목 (정적 검증 + 런타임 기동)
- 사용자가 직접 확인할 방법
- **용어 사전 준수 (P23)**: 사용자 보고, 로그 메시지, 문서 작성 시 `ARCHITECTURE.md` 부록 M 표준 용어 사전 준수 — "업종" not "섹터", "종목" not "주식", "매수 후보" not "바이 리스트"
