...
# =============================================================================
# 0단계: 완료된 작업의 계획서 자동 정리 (규칙 10 — 안전안)
# =============================================================================
# HANDOVER.md "전체 상태" 가 "작업 완료" 일 때만 실행.
# 각 태스크 파일(docs/태스크_*.md)의 "상태" 값이 "완료" 인 경우에 한해
# 해당 태스크 파일 + "설계서 경로" 로 연결된 설계서/조사보고서만 삭제.
# 진행 중(단계 완료·대기/중단·보류)이면 전체 건너뜀 — 다음 세션이 이어서 쓸 수 있게 유지.
# 다른 작업의 계획서는 절대 건드리지 않음 (안전안).
cleanup_completed_planning_docs() {
  local HANDOVER="$PROJECT_ROOT/HANDOVER.md"
  local DOCS_DIR="$PROJECT_ROOT/docs"
  [ -f "$HANDOVER" ] || return 0
  [ -d "$DOCS_DIR" ] || return 0

  # 게이트: HANDOVER "전체 상태" 가 "작업 완료" 인지 확인
  if ! grep -E '전체 상태.*작업 완료' "$HANDOVER" >/dev/null 2>&1; then
    log "계획서 정리: 건너뜀 (HANDOVER 전체 상태가 '작업 완료' 아님 — 진행 중 계획서 유지)"
    return 0
  fi

  log "계획서 자동 정리 시작 (작업 완료 상태 — 규칙 10 안전안)"
  local deleted=0

  for taskfile in "$DOCS_DIR"/태스크_*.md; do
    [ -f "$taskfile" ] || continue
    # 태스크 파일 "상태" 값이 "완료" 인지 확인 — "진행 중"/"일부 완료" 제외
    # 매칭: "**상태**: 완료" 또는 "상태: 완료" (콜론 직후 첫 단어가 '완료'인 경우만)
    local status_line
    status_line=$(grep -E '상태' "$taskfile" | head -1)
    if ! echo "$status_line" | grep -qE '상태\*\*:[[:space:]]*완료|상태:[[:space:]]*완료'; then
      log "  - $(basename "$taskfile"): 상태가 완료 아님 → 유지"
      continue
    fi
    # "설계서 경로" 참조 문서 삭제 (설계서 또는 조사보고서)
    local ref
    ref=$(grep -oE 'docs/[^` ]+\.md' "$taskfile" | head -1)
    if [ -n "$ref" ] && [ -f "$PROJECT_ROOT/$ref" ]; then
      rm -f "$PROJECT_ROOT/$ref"
      log "  - 삭제: $ref (연결된 설계서/조사보고서)"
      deleted=$((deleted + 1))
    fi
    # 태스크 파일 자체 삭제
    rm -f "$taskfile"
    log "  - 삭제: $(basename "$taskfile") (완료된 태스크 파일)"
    deleted=$((deleted + 1))
  done

  if [ "$deleted" -gt 0 ]; then
    log "계획서 정리 완료: ${deleted}건 삭제 (완료된 작업 계획서만)"
  else
    log "계획서 정리 완료: 삭제 대상 없음 (완료된 태스크 파일 없음)"
  fi
}
