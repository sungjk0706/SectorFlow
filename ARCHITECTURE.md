...
> 통합 근거: P18(테스트모드 동등성).
> **현재 상태(W6 투명성)**: 키움 응답 구조는 시뮬레이터가 모방(`data.output.ord_no` 등). 단 LS 증권 응답 구조(`order_no`/`raw_res`)와 시뮬레이터 불일치 — 공통 데이터 모델 부재로 증권사별 분기 처리 필요. 후속 과제: 공통 데이터 모델 도입.
...
**후속 과제(별도 승인 필요)**: 실전 모드 기동 시 증권사 잔고·보유 종목 조회 후 내부 기록과 대조·불일치 차단 로직 구현.
...
**후속 과제(별도 승인 필요)**: 실전 모드 런타임 주기적 reconciliation 스케줄링 + 체결 누락 감지·복구 로직 구현.
