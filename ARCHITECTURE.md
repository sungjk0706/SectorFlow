...
- Redis/Pub-Sub 금지
- 콜백 리스트 옵서버 패턴 금지
- `tick_queue` → `Compute Loop` → `order_queue` → `OMS` 명시적 파이프라인
...
- tick_queue -> Compute Engine -> order_queue -> OMS Pipeline 명시적 asyncio.Queue 파이프라인 구조는 투명성 확보, 디버깅 속도 압도적 향상
...
- 데이터 소스 확인 (DB, API, WebSocket)
- 데이터 변환 계층 확인 (mapper, serializer)
- state/store에 저장되는가
- UI에 전달되는가
