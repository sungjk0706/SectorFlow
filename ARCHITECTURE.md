...
본 문서의 원칙은 다음 두 종류로 구분한다.

### 안전 불변 원칙

주문·정합성·복구·실전 거래 안전과 직접 관련되므로 구조가 변경되더라도 동일한 안전성을 보장해야 하는 원칙.

### 현재 구조 선택

현재 1인 로컬·단일 프로세스 환경에 적합하여 선택한 구현 방식. 향후 규모·요구사항·측정 결과가 바뀌면 재검토할 수 있다.
...
### 원칙 5: 현재 단일 프로세스에서는 EventBus를 사용하지 않음
**분류: 현재 구조 선택**
**내용**: 현재는 직접 호출 체인과 asyncio.Queue 파이프라인을 사용한다.
**구현 가이드**:
- 현재 구조에서는 별도의 EventBus·Redis Pub/Sub·분산 메시지 브로커를 도입하지 않음
- 콜백 리스트 옵서버 패턴 대신 명시적 호출 체인을 유지
- `tick_queue` → `Compute Loop` → 직접 호출(`execute_buy`/`execute_sell`) → 주문 경로
- 프로세스 분리나 다중 소비자가 필요해질 경우 호출 추적성·예외 전파·재처리·종료 처리를 검토한 뒤 재평가

### 원칙 6: SQLite 단일화
**분류: 현재 구조 선택**
**내용**: 1인 로컬 앱의 데이터 규모와 운영 방식을 고려하여 현재는 SQLite를 단일 영속 저장소로 사용한다.
**구현 가이드**:
- SQLite WAL 모드
- Raw SQL 직통 사용
- 단일 커넥션 생명주기 관리 (원칙 12)
...
### 원칙 11: 이벤트 기반 루프
**분류: 안전 불변 원칙**
**내용**: 내부 상태 확인을 위한 폴링 대신 이벤트 기반으로 처리한다.
**구현 가이드**:
- 내부 상태 확인을 위한 `while + sleep` 폴링 금지
- `asyncio.Queue` + `asyncio.wait()` 사용
- `daily_time_scheduler.py`에서 `asyncio.call_later()` 사용

#### 외부 서비스 long polling 예외
외부 서비스가 WebSocket이나 webhook 등 push 방식을 제공하지 않는 경우 해당 서비스의 long polling은 예외적으로 허용한다. 단, 긴 대기시간·취소 가능한 비동기 요청·종료 시 태스크 취소·오류 재시도·빠른 반복 요청 방지·엔진 루프와의 실패 격리를 갖춰야 한다.
...
### 원칙 18: 테스트모드 동등성
**분류: 안전 불변 원칙**
**내용**: 전략 판단·리스크 게이트·주문 상태 전이는 동일하게 유지하고, 계좌·잔고·체결·정산의 외부 원장만 테스트용 원장으로 대체한다.
**구현 가이드**:
- 모드 분기는 돈 I/O 최소 지점에만 (`dry_run.fake_send_order()`)
- 모든 안전장치는 테스트모드에서 검증 가능
- 업종 점수 계산, 필터링, 타이밍 로직은 동일
- 수수료/세금 계산은 테스트모드에서만 수행 (실전모드는 증권사 서버가 수수료/세금의 SSOT이므로 SectorFlow가 자체 계산하지 않음 — 외부 원장의 일부로 취급)
...
**분류: 안전 불변 원칙**
**내용**: 정상 경로의 데이터 누락·초기화 실패를 기본값으로 덮어 숨기지 않는다.
**구현 가이드**:
...
매매 차단 원인(리스크 초과, 가드 조건 미충족, 자동매수 OFF 등)은 화면 상단 헤더 칩뿐 아니라
해당 페이지(매수 후보/보유 종목)의 각 항목 배지에서도 즉시 확인할 수 있어야 한다.
사용자가 헤더까지 올라가지 않고도 해당 종목 옆에서 차단 사유를 바로 볼 수 있도록 한다.
...
- **프론트엔드 레이아웃 단일 소스**: UI 레이아웃 값(padding, margin, gap 등)과 배지/칩 스타일은 공통 컴포넌트와 단일 소스(`layout/shell.ts`, `components/common/badge.ts` 등)에서만 관리. 페이지별로 동일 레이아웃 값을 중복 오버라이드하는 코드 금지 (P10 SSOT와 중복 강화)
...
**내용**: 같은 기능을 더 단순한 방법으로 구현할 수 있다면 단순한 쪽을 선택. 불필요한 추상화, 과도한 간접 계층, 복잡한 구현을 금지. 코드 줄 수를 무조건 줄이는 것보다, 동일한 로직이 여러 곳에 중복되지 않고 단일 진실 소스(SSOT)로 관리되는 것이 더 근본적인 단순성이다. 중복 코드는 줄 수와 관계없이 제거 대상이다.
...
**구현 가이드**:
- **중복 제거 우선 (근본 단순성)**: 동일한 로직이 여러 곳에 중복되는 것은 줄 수·복잡도 기준과 무관하게 단순성 위반. 중복은 한 곳 변경 시 다른 곳 전파 누락으로 P22(데이터 정합성) 위반까지 유발하므로, 줄 수 단축보다 중복 제거를 먼저 검토 (P10 SSOT, P23 일관성과 중복 강화)
...
- **복잡도·규모 기준**: 함수나 파일이 지나치게 길어지면 책임 분리와 테스트 가능성을 검토한다. 50줄·500줄은 자동 위반 기준이 아니라 분할 검토를 시작하는 참고 기준이다.
  - 책임이 여러 개로 섞였는지, 상태 전이와 예외 경계가 불명확한지 함께 판단
  - 분할 후 호출 흐름이 더 복잡해지지 않는지 확인
  - 순환 복잡도는 분할 검토를 위한 참고 지표로 사용
...
**P10(SSOT)/P23(일관성)과의 관계**: P10은 데이터 출처 단일화, P23은 표현/구현 방식 단일화, P24는 코드 로직 중복의 단순성 위반을 각각 담당 — 중복 제거는 세 원칙의 교차 강화점

### 원칙 25: 격리된 실패 (Isolated Failure)
**분류: 안전 불변 원칙**

**내용**: 한 구성요소의 실패(예외/렌더링 오류)가 전체 시스템 기동/운영을 블로킹하지 않아야 한다. 실패는 해당 구성요소에서 차단되고 로깅되며, 다른 구성요소는 정상 작동을 유지해야 한다.

**배경**: 2026-07-23 F-02 사례 — header.ts 장 상태 칩 하나의 `PHASE_STYLE[phase]` undefined → TypeError가 store.ts listener 루프를 통해 setState 호출자(WS 핸들러)까지 전파 → 앱 전체 렌더링 중단(하얀 화면). 기존 P7(블로킹)은 "느린 연산"을, P20(폴백 금지)은 "빈값 덮기"를 다루지만 "예외 전파 차단"을 명시하는 원칙은 없었음.

**구현 가이드**:
- **프론트엔드 store listener 루프**: listener throw 시 try/catch로 차단, console.error/logger 로깅, 다른 listener는 계속 실행 (silent pass 금지 — P20/P23과 일관)
- **프론트엔드 개별 칩/컴포넌트**: 렌더링 실패 시 해당 요소만 기본/neutral 표시, 나머지 화면 정상 작동 (P21 투명성 — 사용자가 "왜 화면이 하얗게?"라는 의문을 갖지 않도록)
- **백엔드 단일 이벤트 루프**: 한 태스크/코루틴 실패가 루프 전체를 중단하지 않도록 — `schedule_engine_task()`의 `add_done_callback` 예외 로깅과 일관 (P5/P14와 연계)
- **에러 로깅 의무**: 격리 ≠ silent 무시. 차단 시 반드시 `logger.warning`/`console.error`로 로깅 (P20 "에러 로그 출력" 의무, P23 silent `except:pass` 금지와 중복 강화)

**P24(단순성)와의 균형**: 본 원칙은 "최소한의 전파 차단"에 국한. microservice식 과도한 격리 추상화, 각 함수마다 try/catch 래퍼 등은 P24 위반. 격리는 경계(store listener 루프, 칩 렌더링 진입점, 태스크 콜백)에서만 적용.

**P20(폴백 금지)과의 구분**: P20은 정상 경로의 데이터 누락·초기화 실패를 기본값으로 덮어 숨기지 않는 원칙이다. P25는 렌더링·태스크 예외가 전체 시스템으로 전파되지 않도록 막는 원칙이다. 렌더링 실패 시 neutral 표시를 사용하더라도 오류 로그와 오류 상태 표시가 필요하며, 이를 주문·정산 데이터의 정상값 대체에 사용해서는 안 된다.
...
- 모든 I/O를 aiosqlite, httpx 등 async/await 계열 라이브러리로 일관하여 이벤트 루프 블로킹을 줄임

#### SQLite 단일화
**현재 구조에서의 선택 이유**
- 1인 로컬 앱에서는 별도 DB 서버 운영 비용을 추가하지 않음
- SQLite는 로컬 파일 기반으로 운영이 단순함
- Raw SQL 직통 조작과 aiosqlite 연결 생명주기를 현재 구현 방식으로 사용함
...
#### 현재 단일 프로세스에서 EventBus를 사용하지 않음
**현재 구조에서의 선택 이유**
- 직접 호출 체인은 호출 흐름과 예외 경계를 추적하기 쉬움
- `tick_queue` → Compute Engine → 직접 호출(`execute_buy`/`execute_sell`) → 주문 경로를 현재 구조로 사용함
- 프로세스 분리나 다중 소비자가 필요해질 경우 EventBus 도입을 재검토함
...
- SQLite 단일 커넥션 + WAL 모드 (현재 Raw SQL 사용)
...
check_sell_conditions()  — 스냅샷 루프에서 주기적 호출
    ...
    ├── 익절: pnl_rate >= tp_val
    └── T/S: drop_rate <= ts_drop_val (둘 다 음수 — 후안 B 부호 규칙)
         │
         ▼
    execute_sell()
         │
...
DB 스키마 변경이나 마이그레이션 전에는 다음 파일을 같은 타임스탬프로 백업한다.

- `stocks.db`
- `stocks.db-shm`
- `stocks.db-wal`

백업은 애플리케이션을 안전하게 정지하거나 SQLite 일관성이 보장되는 방식으로 수행한다. 기존 DB 파일을 직접 삭제하거나 덮어쓰지 않는다.

#### 복구

1. 애플리케이션과 거래 관련 태스크를 정지한다.
2. 동일 타임스탬프의 DB 백업 세트를 복원한다.
3. 애플리케이션을 기동한다.
4. 기동 시 정산·거래 이력·파생 캐시의 정합성 검사를 수행한다.
5. 불일치가 확인되면 자동매매를 재개하지 않고 원인을 확인한다.

복구는 데이터베이스 파일만 되돌리는 절차이며, 브로커 서버의 주문·체결 상태를 변경하지 않는다.
...
- 증권사 이름 공통 기능 침투 금지
- 현재 단일 프로세스에서는 EventBus/발행구독을 사용하지 않음
- SQLite 단일화 (현재 구조 선택)
- 블로킹 = 지연 = 왜곡 = 망함
- 실시간 파이프라인과 배치 파이프라인 분리
- 각 파이프라인 독립, 상호 간섭 금지
- 단일 소스 진리
- 이벤트 기반 루프 (내부 상태 폴링 금지, 외부 long polling은 예외 조건 적용)
- DB 연결 매번 생성/파기 금지
...
- `stores/store.ts`: 전역 상태 (Record 기반)
...
전략 판단·리스크 게이트·주문 상태 전이는 동일하게 유지한다.
계좌·잔고·체결·정산의 외부 원장만 테스트용 원장으로 대체한다.

실전 모드의 외부 계좌·주문·체결·수수료 정보는 증권사 서버를 SSOT로 취급한다.
SectorFlow는 이를 자체 원장으로 대체하지 않는다.
...
- **길이**: 함수나 파일이 지나치게 길어지면 책임 분리와 테스트 가능성을 검토한다. 50줄·500줄은 분할 검토를 시작하는 참고 기준이다.
...
이 문서는 영구적인 최적성을 주장하지 않으며, 실제 사용량·장애 기록·측정 결과에 따라 구조를 재검토한다.

### 핵심 요약

- 단일 asyncio 루프와 메모리 캐시는 현재 로컬 환경에서 호출 흐름과 상태를 직접 추적하기 쉽다.
- SQLite와 Raw SQL은 현재 데이터 규모와 운영 범위에 맞는 구조 선택이다.
- WAL 모드와 DB 쓰기 직렬화 큐는 읽기·쓰기 병행성을 개선하지만 특정 지연 수치를 보장하지 않는다.
