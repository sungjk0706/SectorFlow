...
- 수수료/세금 계산은 테스트모드에서만 수행 (실전모드는 증권사 서버가 수수료/세금의 SSOT이므로 SectorFlow가 자체 계산하지 않음 — 이는 "돈 I/O"의 일부로 취급)
...
**P20(폴백 금지)과의 구분**: P20은 "정상 경로의 빈값을 기본값으로 덮지 말 것". P25는 "실패를 여기서 막고 전파 차단". 격리 시 에러 로그를 출력하므로 폴백이 아님.
