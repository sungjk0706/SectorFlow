...
class LifespanOn:
    ...
    async def startup(self) -> None:
        self.logger.info("애플리케이션 시작을 기다리는 중.")
        ...
        if self.startup_failed or (self.error_occurred and self.config.lifespan == "on"):
            self.logger.error("애플리케이션 시작에 실패했습니다. 종료 중입니다.")
    ...
    async def shutdown(self) -> None:
        if self.error_occurred:
            return
        self.logger.info("애플리케이션 종료를 기다리는 중.")
        ...
        if self.shutdown_failed or (self.error_occurred and self.config.lifespan == "on"):
            self.logger.error("애플리케이션 종료에 실패했습니다. 종료 중입니다.")
            self.should_exit = True
        else:
