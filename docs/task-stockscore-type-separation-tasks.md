...
| 태스크 | 대상 | 의존성 | 검증 |
|--------|------|--------|------|
| T1 | `types/index.ts` — `StockScore` 추가, `SectorStock` 축소 | 없음 | typecheck |
| T2 | `utils/stock-search.ts` — `filterStocksBySearch` 제네릭화 | T1 | typecheck |
| T3 | `stores/hotStore.ts` — 동기화 함수 시그니처 변경 | T1, T2 | typecheck |
...
| T7 | 최종 검증 — typecheck + build + test 3종 | T1~T6 | 3종 전체 통과 |
...
### 변경 명세 (설계 5.1)
1. `StockScore` 인터페이스 신규 추가 — 설계 2.1의 19개 필드 그대로 적용.
   - 식별 5개 + 실시간 파생 5개 + 정적 스코어 5개 + 매수후보 전용 파생 5개 (news_boost_title 포함).
2. `SectorStock` 인터페이스 축소 — 설계 2.2의 10개 필드(식별 5 + 실시간 파생 5)만 남기고, 매수후보 전용 필드(`avg_amt_5d`, `rank`, `guard_pass`, `reason`, `boost_score`, `order_ratio`, `high_5d`, `program_net_buy`, `news_boost`, `news_boost_title`) 제거.
...
5. `applyRealData`/`applyOrderbookUpdate`/`applyProgramUpdate` 내부 `t` 타입은 TS 추론에 맡김 — 명시적 타입 annotation이 `SectorStock`으로 되어 있으면 제거하여 추론 유도. 단, 본문 로직은 변경 없음 (설계 3.3).
6. `applyRealtimeReset`의 `nullifyFields` 대상은 sectorStocks이므로 `SectorStock` 유지. `rebindBuyTargetsRealtime` 호출부는 T3 변경을 따라감.
7. 주석 업데이트: buyTargets 동기화 관련 주석이 "SectorStock"을 참조하면 "StockScore"로.
...
### 아키텍처 원칙 점검
- [ ] **P10 (SSOT)**: `sectorStocks`가 실시간 SSOT, `buyTargets`가 파생 캐시 — 분리 후에도 `rebindBuyTargetsRealtime` 동기화 경로 유지 확인.
...
- [ ] **P24 (단순성)**: 불필요한 추상화 없음 — 제네릭은 `filterStocksBySearch` 1곳만.
- [ ] **P25 (격리된 실패)**: 타입 분리로 인해 한 페이지 렌더링 실패가 다른 페이지로 전파되지 않는지 확인.

### 완료 기준
- 3종 검증 모두 통과 + 원칙 점검 체크리스트 모두 충족.
...
T1 승인 → T1 실행 → T1 검증(typecheck, 예상 오류 범위 확인)
  ...
  → T7 승인 → T7 실행 → T7 검증 (typecheck + build + test 3종)
```
