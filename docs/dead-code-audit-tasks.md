...
| 세션 | 우선순위 | 대상 | 계획 상태 | 비고 |
|---|---|---|---|---|
| DC-S1 | P16/P24 | DC-01, DC-05, DC-06 | ☐ | 운영 경로 0건 후보와 미사용 import |
| DC-S2 | P16/P24 | DC-02, DC-03, DC-04 | ☐ | `engine_state` 잔여 플래그·상수 및 문서/메타 테스트 |
| DC-S3 | P16/P23/P24 | DC-07, DC-10 | ☐ | 키움 5일 조회·구형 WS FID 파서 |
| DC-S4 | P15/P16/P20/P22 | DC-08, DC-11 | ☐ | 서킷브레이커·정산 매수능력 검증. safe-trade 필수 |
| DC-S5 | P10/P16/P24 | DC-09, DC-12 | ☐ | 계좌 집계·설정 저장 보조 함수 |
...
**상태:** ☐ 미시작  
**대상 원칙:** P16 살아있는 경로, P24 단순성

#### 대상 코드

- `backend/app/core/settings_defaults.py:178`
  - `DEFAULT_BROKER_CREDENTIALS`
- `backend/app/core/ls_rest.py:212`
  - `LsRestAPI.call_api`
- `backend/app/web/ws_manager.py:8`
  - 미사용 `asyncio` import
- 각 대상의 전체 참조·문서·테스트

#### 수정 내용

1. DC-01과 DC-05가 외부 import·동적 접근·문서 계약으로 필요한지 최종 확인한다.
2. 참조 0건이 확정된 경우 해당 상수·메서드와 잔존 설명을 제거한다.
3. DC-06은 `asyncio`가 파일 내 실제 사용되지 않는 것이 확정되면 import만 제거한다.
4. 삭제 대상 이름을 전체 저장소에서 재검색하고, 관련 테스트가 있으면 의도에 맞게 제거 또는 갱신한다.
5. 사용처가 발견되면 삭제하지 않고 상태를 `⊘`로 기록한다.

#### 검증 방법

- 대상 이름 전체 검색: 운영 코드·테스트·문서에 의도하지 않은 잔존 참조 없음
- 관련 백엔드 테스트 및 전체 백엔드 테스트
- `.venv/bin/python -W error::RuntimeWarning main.py` 기동
- 잔존 프로세스 0건 확인
- vulture/pyflakes 재실행하여 해당 경고가 사라졌는지 확인
