...
> **추가 배경**: 세션 3 구현 중 발견 — 세션 3 이전에는 매매부적격 보유종목이 `master_stocks_cache`에서 삭제되어 `get_sector_stocks()`에 자연히 포함되지 않았음(업종 순위에서도 자동 차단). 세션 3로 보유종목을 캐시에 유지하면서, 매매부적격이지만 1차 필터링(거래대금)은 통과한 보유종목이 `get_sector_stocks()`를 통과할 수 있게 됨 — 기존 `avg_5d_trade_amount` 값(매매적격이던 시점의 값)이 캐시에 그대로 남아 있고, 0B 실시간 틱(`engine_ws_reg.py:86-96` 보유종목 최우선 구독)으로 `cur_price`가 갱신되어 `get_sector_stocks()`의 두 필터(86줄 cur_price 검사·93줄 거래대금 검사)를 모두 통과. 이는 세션 3의 부작용 — `get_sector_summary_inputs()`(34줄)이 `get_sector_stocks()` 기반이므로 업종 순위 계산에도 매매부적격 보유종목이 포함되어 순위 왜곡 유발.
>
