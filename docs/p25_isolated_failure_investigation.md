...
- 등락률/체결강도 가드, 재매수 차단, 시간대 차단, 최대보유수, 종목당한도, 일일한도 — 모두 공통
...
- **P18 테스트모드 동등성 (대체)**: RiskManager 게이트, 등락률/체결강도 가드, 재매수 차단, 시간대 차단, 한도 체크 모두 테스트/실전 공통. 모드 분기는 돈 I/O 최소 지점에 한정 (B5-08-02 미세 위반 제외).
