...
| B5-08-01 | B5 매매·테스트모드 | `backend/app/services/trading.py:477-482, 666-671` | `execute_buy`/`execute_sell`에서 `asyncio.create_task(dry_run.fake_fill_event(...))`를 직접 호출하고 `add_done_callback`을 수동 연결. `schedule_engine_task()`(engine_lifecycle.py:279-309)가 동일 기능(add_done_callback 포함) 제공하는데 미사용 → ARCHITECTURE.md 금지 패턴 2 "create_task 무분별 분리 금지 → schedule_engine_task() 사용" 위반 | 기능적으로는 동등(엔진 루프 내 호출이므로 schedule_engine_task 두 번째 분기와 동일). 격리/로깅은 add_done_callback로 보장되어 영향도 낮음. 단 P23 일관성 위반 | LOW | P23 | 세션 8 | 미승인 |
| B5-08-02 | B5 매매·테스트모드 | `backend/app/services/trading.py:572-598` | `execute_sell` 평균매입가 조회가 테스트/실전 분기됨 (`build_positions_from_trades` vs `get_positions`). 이는 돈 I/O가 아닌 "조회" 분기로, P18 "모드 분기는 돈 I/O 최소 지점에만"의 엄격 해석상 미세 위반. 결과 동등성은 보장되나 분기 위치가 원칙 문언 부합 여부 검토 대상 | 매도 주문 금액/체결이력에는 영향 없음. avg_buy_price/buy_date 조회 경로만 분기. 영향도 낮으나 P18 엄격 준수 관점에서 정비 권장 | LOW | P18 | 세션 8 | 미승인 |
| B5-08-03 | B5 매매·테스트모드 | `backend/app/services/trading.py:477-482, 666-671` (연관 `dry_run.py:153-198`) | `fake_fill_event` 태스크가 실패/취소될 경우: 주문 접수는 이미 `record_buy`/`record_sell`(trading.py:459-463, 655-661)로 영속화되어 있으나, `fake_fill_event` 내부 `_apply_buy`/`_apply_sell`(Settlement Engine 예수금 차감/매도 정산)이 누락될 수 있음 → trade_history와 Settlement Engine 잔고 불일치 가능. 단 `tests/test_settlement_verification.py` S4-1(207-260)에 동일 시나리오 재현 테스트 존재 → 인지된 영역 | 테스트모드에서 가상 체결 태스크 실패 시 잔고 정합성 깨질 수 있음. 기동 시 대조(reconciliation) 메커니즘 보유 여부는 본 세션 범위 외(세션 9 교차 점검 대상) | MEDIUM | P22 | 세션 8 | 미승인 |
| B5-08-04 | B5 매매·테스트모드 | `backend/app/services/trading.py:204-210` | `_execute_buy_locked` 실시간 지연 체크 `except Exception:`이 로깅은 하되 체크 실패 시 매수를 차단하지 않고 계속 진행. P20/P25 관점에서 "체크 실패 시 안전 차단(fail-closed)"이 더 보수적일 수 있음 (현재는 게이트 우회 형태) | 실시간 지연 상태 확인 불가 시 매수가 진행됨. 지연 중단 게이트 의도(실시간 통신 200ms 초과 시 매수 차단)가 우회될 수 있음. 영향도 중간~낮음 | LOW | P20, P25 | 세션 8 | 미승인 |
...
|------|-----------|--------|---------------------------|
| prices | 25 | HIGH | `buy-targets-delta`(114-161), `sector-scores`(287-310), `sector-stocks-delta`(95-112), `sell-history-append`(234-242), `circuit-breaker-open`(318-322, showToast) |
...
> 상태: 완료 (2026-07-23)
> 조사 파일: `trading.py` (859줄), `buy_order_executor.py` (234줄), `dry_run.py` (333줄)
> 조사 범위:
...
> - P15(단일 주문 경로)/P18(테스트모드 동등성) 교차 점검

### 10.1 조사 결과

#### 10.1.1 P15 단일 주문 경로 점검 — 준수

| 함수 | 주문 경로 | 분기/우회 |
|------|-----------|-----------|
| `execute_buy` (trading.py:396-403) | `dry_run.fake_send_order` 또는 `get_router().order.send_order` | 없음 |
| `execute_sell` (trading.py:603-609) | 동일 2경로 | 없음 |

- 주문 로직은 `execute_buy()`/`execute_sell()` 단일 경로. 별도 매수 함수/라우터 우회 없음.
- `buy_order_executor.evaluate_buy_candidates`(buy_order_executor.py:203-206)는 `state.auto_trade.execute_buy` 호출 — 단일 경로 유지.

#### 10.1.2 P18 테스트모드 동등성 점검 — 대체 준수, 1건 미세 이슈

**준수 부분**:
- RiskManager 게이트(trading.py:365-373) — 테스트/실전 공통 호출
- 등락률 가드, 재매수 차단, 시간대 차단, 최대보유수, 종목당한도, 일일한도 — 모두 공통
- 모드 분기는 돈 I/O 최소 지점(`fake_send_order`, `reserve_test_buy_power`, `fake_fill_event` 예약)에 한정

**잠재 미세 위반 (B5-08-02)**:
- `execute_sell` 평균매입가 조회(trading.py:572-598)가 테스트/실전 분기됨 (`build_positions_from_trades` vs `get_positions`). 돈 I/O가 아닌 "조회" 분기로 P18 엄격 해석상 미세 위반. 결과 동일성은 보장되나 원칙 문언상 분기 위치 검토 대상.

#### 10.1.3 P25 격리된 실패 / dry_run 태스크 격리 점검 — 2건 이슈

**이슈 A (B5-08-01, P23 일관성 / ARCHITECTURE.md 금지 패턴 2)**:
- trading.py:477-482(매수), 666-671(매도) — `asyncio.create_task` 직접 사용 + `add_done_callback` 수동 연결.
- `schedule_engine_task()`(engine_lifecycle.py:279-309)가 동일 기능(add_done_callback 포함) 제공하므로, ARCHITECTURE.md 금지 패턴 2 및 P23 일관성 관점에서 통일 권장.
- 기능적으로 동등(엔진 루프 내 호출이므로 schedule_engine_task 두 번째 분기와 동일). 심각도 낮음.

**이슈 B (B5-08-03, P22 데이터 정합성 잠재)**:
- `fake_fill_event` 태스크 실패/취소 시: 주문 접수는 이미 `record_buy`/`record_sell`(trading.py:459-463, 655-661)로 영속화되어 있으나, `fake_fill_event` 내부 `_apply_buy`/`_apply_sell`(Settlement Engine 예수금 차감/매도 정산)이 누락될 수 있음 → trade_history와 Settlement Engine 잔고 불일치 가능.
- 단 `tests/test_settlement_verification.py` S4-1(207-260)에 동일 시나리오 재현 테스트 존재 → 인지된 영역. 기동 시 대조(reconciliation) 메커니즘 보유 여부는 본 세션 범위 외(세션 9 교차 점검 대상).

#### 10.1.4 매매 경로 실패 전파 — 양호

| 위치 | 실패 처리 | 평가 |
|------|-----------|------|
| `execute_buy` 주문 실패 (trading.py:405-430) | 사전 차감 롤백 + RiskManager 실패 보고 + 서킷브레이커 OPEN 시 마스터 스위치 강제 OFF + WS 브로드캐스트 | 적절 (P20/P21/P22) |
| `execute_sell` 주문 실패 (trading.py:611-631) | `_recent_sells` 해제 + RiskManager 실패 보고 + 서킷브레이커 OPEN 시 마스터 스위치 강제 OFF | 적절 |
| `buy_order_executor` 예외 (buy_order_executor.py:232-234) | `break` 안전 종료 + `logger.warning(exc_info=True)` | 적절 (silent pass 아님) |
| RiskManager 보고 실패 (trading.py:428-429) | `logger.warning`만, 주문 결과 불변 | 적절 (격리된 실패) |

#### 10.1.5 기타 발견 (B5-08-04)

- trading.py:204-210 실시간 지연 체크 `except Exception:` — 로깅은 하되 체크 실패 시 매수를 차단하지 않고 계속 진행. P20/P25 관점에서 "체크 실패 시 안전 차단(fail-closed)"이 더 보수적일 수 있음(현재는 게이트 우회 형태). 미세 검토 대상.
- dry_run.py:173 `await asyncio.sleep(FAKE_FILL_DELAY)` — 태스크 취소 시 `CancelledError`가 `add_done_callback`의 `t.exception()`에서 에러로 로깅됨. 정상 취소와 실패 구분 여부 검토 대상(미세).

### 10.2 위반 목록

| ID | 등급 | 파일:줄 | 위반 | 관련 원칙 |
|----|------|---------|------|-----------|
| B5-08-01 | LOW | `trading.py:477-482, 666-671` | `asyncio.create_task` 직접 사용 + add_done_callback 수동 연결. `schedule_engine_task()` 미사용 → ARCHITECTURE.md 금지 패턴 2 위반 | P23 |
| B5-08-02 | LOW | `trading.py:572-598` | `execute_sell` 평균매입가 조회 모드 분기 (돈 I/O 아닌 분기) → P18 엄격 해석상 미세 위반 | P18 |
| B5-08-03 | MEDIUM | `trading.py:477-482, 666-671` (연관 `dry_run.py:153-198`) | `fake_fill_event` 태스크 실패 시 trade_history와 Settlement Engine 잔고 불일치 가능 (테스트는 존재) | P22 |
| B5-08-04 | LOW | `trading.py:204-210` | 실시간 지연 체크 실패 시 매수 차단 아닌 통과 (게이트 우회 형태) | P20, P25 |

### 10.3 양호 항목 (P25/P15/P18 준수 확인)

- **P15 단일 주문 경로**: `execute_buy`/`execute_sell` 단일 경로. 분기/우회 없음. `buy_order_executor`도 동일 경로 호출.
- **P18 테스트모드 동등성 (대체)**: RiskManager 게이트, 등락률 가드, 재매수 차단, 시간대 차단, 한도 체크 모두 테스트/실전 공통. 모드 분기는 돈 I/O 최소 지점에 한정 (B5-08-02 미세 위반 제외).
- **매수 실패 전파**: 사전 차감 롤백 + RiskManager 실패 보고 + 서킷브레이커 OPEN 시 마스터 스위치 강제 OFF + WS 브로드캐스트 (P20/P21/P22 준수).
- **매도 실패 전파**: `_recent_sells` 해제 + RiskManager 실패 보고 + 서킷브레이커 OPEN 시 마스터 스위치 강제 OFF.
- **buy_order_executor 예외 처리**: `break` 안전 종료 + `logger.warning(exc_info=True)` (silent pass 아님).
- **RiskManager 보고 실패 격리**: `logger.warning`만, 주문 결과 불변 (격리된 실패로 취급).
- **safe-trade 스킬 연계**: 본 세션 조사 시작 시 safe-trade 스킬 호출하여 거래 로직 수정 안전 절차 준수. 조사만 수행하여 수정 없음.
...
> 상태: 완료 (2026-07-23)
> 조사 범위: 세션 1~8 결과 취합 + 교차 원칙 매트릭스 작성 + 우선수정 추천 + B5-08-03 기동 시 대조 메커니즘 확인
> 성격: 조사만 수행 (코드 수정 없음). P25 전수 조사 9/9 세션 완료.

### 11.1 P25 위반 전체 목록 (취합 — 40건)

세션 1~8에서 식별된 위반 40건 취합. 등급별 분포:

| 등급 | 건수 | 비고 |
|------|------|------|
| CRITICAL | 2 | A1-01-01, A1-01-02 (WS 디스패치 — 화면 전체 멈춤) |
| HIGH | 8 | A1-01-04, B1-02-01, B1-02-04, B2-03-01, B3-05-01, A3-07-01, A3-07-02, A3-07-04 |
| MEDIUM | 14 | A1-01-03, B1-02-02, B1-02-03, B2-03-02, A2-04-01, A2-04-02, B3-05-02, B4-06-01, B4-06-03, A3-07-03, A3-07-05, A3-07-06, A3-07-07, B5-08-03 |
| LOW | 16 | A1-01-05, B1-02-05, B1-02-06, B1-02-07, B2-03-03, B2-03-04, B2-03-05, B3-05-03, B3-05-04, B4-06-02, A3-07-08, A3-07-09, A3-07-10, B5-08-01, B5-08-02, B5-08-04 |
| **합계** | **40** | |
...
| B1 엔진 코어 루프 | 7 | 세션 2 |
| B2 파이프라인 연산 | 5 | 세션 3 |
| A2 Store listener | 2 | 세션 4 |
| B3 스케줄러·파이프라인 | 4 | 세션 5 |
| B4 워커·IO·재시도 | 3 | 세션 6 |
| A3 UI 컴포넌트 렌더링 | 10 | 세션 7 |
| B5 매매·테스트모드 | 4 | 세션 8 |

전체 상세 목록은 섹션 2 매트릭스 참조.

### 11.2 교차 원칙 매트릭스 (P25 × P7/P9/P16/P20/P23)

> ✓ = 해당 원칙 위반 연관. 공백 = 해당 없음. "기타" 열 = P21/P22/P18 등 본 조사 범위 외 원칙 연관.

| 위반 ID | 등급 | P25 | P7 | P9 | P16 | P20 | P23 | 기타 |
|---------|------|-----|----|----|-----|-----|-----|------|
| A1-01-01 | CRITICAL | ✓ | | | | | | P21 |
| A1-01-02 | CRITICAL | ✓ | ✓ | | | | | |
| A1-01-03 | MEDIUM | | | | | | ✓ | P21 |
| A1-01-04 | HIGH | ✓ | | | ✓ | | | |
| A1-01-05 | LOW | ✓ | | | | | | |
...
| 원칙 | 연관 위반 수 | 주요 패턴 |
|------|-------------|-----------|
| **P25** (격리된 실패) | 32 | 본 조사 주 원칙. 루프/디스패치/태스크 격리 부재 |
| **P7** (블로킹 금지) | 6 | A1-01-02, B1-02-01, B2-03-01/03, A2-04-02 — 틱/real-data 고빈도 경로 블로킹 |
| **P9** (파이프라인 독립) | 0 | 본 조사에서 P9 위반 연관 항목 없음. 파이프라인 간 독립성은 구조적 분리로 유지됨 |
| **P16** (살아있는 경로) | 3 | A1-01-04, B1-02-04, A2-04-01 — 핸들러 본문/updater/캐시 로드 보호 부재로 경로 사망 |
| **P20** (폴백 금지) | 5 | B3-05-02/03, B4-06-03, B5-08-04 — silent pass 또는 빈 폴백. A3-07-09는 P23 일관성 |
| **P23** (일관성) | 11 | A1-01-03, B2-03-01/02/04/05, B3-05-04, B4-06-02, A3-07-09, B5-08-01 — 격리 패턴 혼용, exc_info 누락 |
| P21 (사용자 투명성) | 11 | UI 갱신 누락/잘못된 로그. 본 조사 범위 외이나 P25와 빈번 교차 |
| P22 (데이터 정합성) | 3 | B1-02-02, B3-05-01, B5-08-03 — 잔고/DB 불일치 |
| P18 (테스트모드 동등성) | 1 | B5-08-02 — 조회 모드 분기 미세 위반 |

#### 11.2.2 교차 원칙 핵심 인사이트

1. **P9 위반 0건**: 파이프라인 간 독립성(P9)은 구조적 분리(`pipeline_compute.py` vs `market_close_pipeline.py` vs `trading.py`)로 유지되어 본 조사에서 P25와 교차 위반 없음. P9는 P25와 직접 교차하지 않는 독립 축.
2. **P7 × P25 교차 6건**: 모두 고빈도 real-data/틱 경로. WS 디스패치(A1), 엔진 루프(B1), 파이프라인 compute(B2), Store dispatchEvent(A2). 한 예외가 고빈도 경로 전체를 블로킹하는 패턴.
3. **P16 × P25 교차 3건**: 핸들러 본문/캐시 로드 보호 부재. F-02 fix는 listener "간" 격리만 제공, 핸들러 "내부"와 updater는 미보호 — 경로 사망 시 살아있는 경로 배선 원칙 위반.
4. **P20 × P25 교차 5건**: silent pass/빈 폴백이 격리와 결합. B3-05-01(HIGH)은 DB 저장 실패를 True 반환하는 가장 심각 사례 — P22 정합성 + P21 투명성 동시 위반.
5. **P23 × P25 교차 11건 (최다)**: 격리 패턴 혼용이 가장 빈번. `schedule_engine_task` vs 직접 `create_task`, `except Exception` vs `except CancelledError` 비대칭, exc_info 누락. 일관성 위반은 즉시 중단 유발 아닌 점진적 부채 축적.

### 11.3 우선수정 추천 (영향도 순)

> 수정은 별도 승인 세션에서 진행. 본 추천은 영향도 기준 정렬. 거래 로직 수정 시 safe-trade 스킬 필수.

#### Tier 1: 즉시 수정 권장 (CRITICAL/HIGH, 시스템 전체 중단 위험)

| 순위 | ID | 등급 | 파일:줄 | 수정 요약 | 스킬 |
|------|----|------|---------|-----------|------|
| 1 | A1-01-01 | CRITICAL | ws.ts:193 | `_dispatchMessage` forEach → 핸들러별 try/catch 루프. 한 핸들러 throw 시 `console.error` + 다른 핸들러 계속 실행 | frontend-fix |
| 2 | A1-01-02 | CRITICAL | ws.ts:164-174 | 디코딩 try와 핸들러 디스패치 try 분리. per-event try (A1-01-01 수정으로 자연 해결) | frontend-fix |
| 3 | A3-07-01 | HIGH | virtual-scroller.ts:293-316 | renderRange 루프 내 renderRow per-row try/catch | frontend-fix |
| 4 | A3-07-02 | HIGH | data-table-fixed.ts:230-236 | 신규 키 추가 루프 내 renderDataRow per-row try/catch | frontend-fix |
| 5 | A3-07-04 | HIGH | header.ts:365-494 | onStateChange 칩 간 per-chip try/catch (F-02 잔존 위험) | frontend-fix |
| 6 | B2-03-01 | HIGH | pipeline_compute.py:646-670 | `_phase2_batch_recompute_loop` while 본문 try/except + continue 추가 | backend-fix |
| 7 | B1-02-01 | HIGH | engine_loop.py:304 | `is_ws_subscribe_window` 호출 try/except + 로깅 + 안전 폴백 | backend-fix |
| 8 | B1-02-04 | HIGH | engine_loop.py:31 | `_load_caches_preboot` 호출 try/except — 기동 실패 명시적 알림 | backend-fix |
| 9 | B3-05-01 | HIGH | market_close_pipeline.py:645-650 | inner except에서 `return True` 제거 → `return False` (DB 저장 실패 시 실패 반환) | backend-fix |
| 10 | A1-01-04 | HIGH | binding.ts 33개 핸들러 | 고위험 핸들러(buy-targets-delta 등) 본문 try/catch (A1-01-01 수정과 상호 보완) | frontend-fix |

#### Tier 2: 단계적 수정 권장 (MEDIUM, 일부 기능 손실)

| 순위 | ID | 등급 | 파일:줄 | 수정 요약 | 스킬 |
|------|----|------|---------|-----------|------|
| 11 | B5-08-03 | MEDIUM | trading.py:477-482, 666-671 | **기동 시 대조 메커니즘 신설** (11.5 참조) — trade_history에서 orderable 재계산 후 settlement_engine과 대조, 불일치 시 차단+로깅 | safe-trade |
| 12 | A2-04-01 | MEDIUM | store.ts:19 | setState updater 함수 try/catch 보호 | frontend-fix |
| 13 | A2-04-02 | MEDIUM | hotStore.ts:367-431 | dispatchEvent per-handler try/catch | frontend-fix |
| 14 | B1-02-02 | MEDIUM | engine_loop.py:374,377 | finally 블록 disconnect_all/disconnect try/except | backend-fix |
| 15 | B1-02-03 | MEDIUM | engine_loop.py:387,389 | REST API 정리 루프 per-client try/except | backend-fix |
| 16 | B2-03-02 | MEDIUM | pipeline_compute.py:673-686 | `_sector_recompute_loop_impl`에 `except Exception` 추가 (CancelledError 외) | backend-fix |
| 17 | B3-05-02 | MEDIUM | market_close_pipeline.py:897 | 빈 폴백 `confirmed = {}` 제거 → 실패 시 파이프라인 중단 | backend-fix |
| 18 | B4-06-01 | MEDIUM | db_writer.py:79 | 실패 시 task_done() 누락 수정 — 큐 미완료 카운트 정합 | backend-fix |
| 19 | B4-06-03 | MEDIUM | engine_cache.py:148-149 | 치명적 오류(RuntimeError) 폴백 제거 → 기동 차단 | backend-fix |
| 20 | A3-07-03 | MEDIUM | data-table.ts:104-112 | extractSamples 루프 per-cell try/catch | frontend-fix |
| 21 | A3-07-05 | MEDIUM | profit-overview-sector-pnl.ts:139-168 | 업종×종목 이중 루프 per-item try/catch | frontend-fix |
| 22 | A3-07-06 | MEDIUM | stock-classification.ts:278-322 | 3개 루프 per-item try/catch | frontend-fix |
| 23 | A3-07-07 | MEDIUM | profit-overview-mount.ts:101-133 | buildAccountRows per-row try/catch | frontend-fix |
| 24 | A1-01-03 | MEDIUM | ws.ts:172,181 | catch 로그 메시지 분리 (디코딩 vs 핸들러 예외) | frontend-fix |

#### Tier 3: 일관성 정비 (LOW, 즉시 중단 유발 아님)

| 순위 | ID | 등급 | 파일:줄 | 수정 요약 | 스킬 |
|------|----|------|---------|-----------|------|
| 25 | B5-08-01 | LOW | trading.py:477,666 | create_task → schedule_engine_task 통일 | backend-fix |
| 26 | B5-08-02 | LOW | trading.py:572-598 | execute_sell 평균매입가 조회 공통 헬퍼 추출 (모드 분기 제거) | safe-trade |
| 27 | B5-08-04 | LOW | trading.py:204-210 | 실시간 지연 체크 실패 시 fail-closed (매수 차단) | safe-trade |
| 28 | B3-05-03 | LOW | market_close_pipeline.py:492 | silent `except (ValueError, TypeError): pass` → 로깅 추가 | backend-fix |
| 29 | B3-05-04 | LOW | 11곳 | exc_info=True 누락 보완 | backend-fix |
| 30 | B4-06-02 | LOW | kiwoom_rest.py:353-356 | 재시도 일관성 정비 (또는 의도적 설계 명시) | backend-fix |
| 31 | B2-03-03 | LOW | pipeline_compute.py:521-526 | _handle_real_tick per-item try/except | backend-fix |
| 32 | B2-03-04 | LOW | pipeline_compute_tick_handlers.py:92-104 | _handle_real_0j_tick try/except 추가 (형제 핸들러와 일치) | backend-fix |
| 33 | B2-03-05 | LOW | pipeline_gateway.py:32 | _gateway_task done_callback 추가 | backend-fix |
| 34 | A3-07-08 | LOW | profit-detail-mount.ts:185-222 | buildStatRow per-card try/catch | frontend-fix |
| 35 | A3-07-09 | LOW | router.ts:105-109 | notifyRouteChange cb 루프 try/catch | frontend-fix |
| 36 | A3-07-10 | LOW | 프론트엔드 전역 87개 addEventListener | 사용자 구동 핸들러 try/catch (빈도 낮음, 점진적 적용) | frontend-fix |
| 37 | A1-01-05 | LOW | ws.ts:132-136 | _scheduleReconnect setTimeout 콜백 try/catch | frontend-fix |
| 38 | B1-02-05 | LOW | engine_ws_dispatch.py:149-153 | _handle_real_00 호출 try/except | backend-fix |
| 39 | B1-02-06 | LOW | engine_ws_dispatch.py:162 | _handle_real_balance 호출 try/except | backend-fix |
| 40 | B1-02-07 | LOW | engine_lifecycle.py:38 | _refresh_positions_if_dirty 호출 try/except | backend-fix |

#### 11.3.1 수정 순서 원칙

1. **A1-01-01/02 선수정**: WS 디스패치 격리는 A1-01-04, A2-04-01/02의 상위 보호 계층. 먼저 확보 시 하위 위반 영향도 감소.
2. **A3-07-01/02/04 묶어 수정**: 행 단위 격리 2건 + 헤더 칩 격리 1건은 동일 패턴(per-row/per-chip try/catch). 한 세션에서 공통 헬퍼 추출 후 일괄 적용 권장 (P23 일관성).
3. **B5-08-03은 safe-trade 스킬 필수 + 사용자 승인 필수**: 기동 시 대조 메커니즘 신설은 거래 정합성 핵심. 본 조사 결과(11.5)를 설계 입력으로 활용.
4. **B3-05-01은 단독 수정 권장**: DB 저장 실패를 True 반환하는 P22/P21 위반. 독립적이고 영향 명확.
5. **Tier 3 일관성 정비는 그룹별 묶어 수정**: B3-05-04(11곳 exc_info), B5-08-01(create_task 통일) 등 동일 패턴 반복 건은 한 세션에서 일괄 처리.

### 11.4 B5-08-03 기동 시 대조 메커니즘 조사 결과

> 세션 8 이월 항목. 본 세션에서 조사 완료.

#### 11.4.1 조사 범위
- `settlement_engine.py` load_state/_load/_persist
- `dry_run.py` _refresh_positions_if_dirty
- `trade_history.py` 잔고 재계산 함수 존재 여부
- `engine_lifecycle.py` start_engine 기동 시 대조 로직
- `tests/test_settlement_verification.py` S4-1 재현 시나리오

#### 11.4.2 조사 결과: **기동 시 대조 메커니즘 부재**

| 확인 항목 | 결과 | 근거 |
|-----------|------|------|
| settlement_engine.load_state() | SQLite KV 스토어에서 orderable 그대로 로드만 | settlement_engine.py:191-258. trade_history와 대조 없음 |
| dry_run._refresh_positions_if_dirty() | trade_history에서 **포지션**만 재구축, **잔고 대조 없음** | dry_run.py:36-65. build_positions_from_trades 호출, orderable 미관여 |
| trade_history.py 잔고 재계산 함수 | **부재** | `def .*cash|orderable|balance|reconcil|verify` 검색 — 매칭 0건 |
| engine_lifecycle.start_engine() 기동 시 대조 | **부재** | engine_lifecycle.py:22-45. _refresh_positions_if_dirty 호출만, 잔고 대조 없음 |
| S4-1 테스트 | 정합성 위반 **재현만**, 복구 메커니즘 부재 | test_settlement_verification.py:207-254. 엔진 재시작 후 stale orderable 로드 확인 |

#### 11.4.3 정합성 위반 영속화 경로

```
fake_fill_event 태스크 실패/취소
  → on_buy_fill/on_sell_fill 미실행
  → settlement_engine._orderable 미갱신 (차감/증가 누락)
  → _persist()는 stale orderable을 SQLite에 저장 (이미 저장된 값 유지)
  → 엔진 재시작 시 load_state()가 stale orderable 그대로 로드
  → trade_history._buy_history에는 매수 이력 존재 + orderable은 차감 전 값
  → 불일치 영속화 (재시작으로 복구 불가)
```

#### 11.4.4 수정 방향 (참고용, 승인 시 별도 세션 — safe-trade 스킬 필수)

기동 시 대조 메커니즘 신설 권장:
1. `trade_history.py`에 `compute_expected_orderable(initial_deposit)` 함수 신설 — _buy_history/_sell_history에서 orderable 재계산 (매수 차감 + 매도 정산 + 수수료/세금)
2. `settlement_engine.load_state()` 로드 후 `compute_expected_orderable()`과 대조
3. 불일치 시: (a) 로깅 + (b) 사용자 UI 알림 + (c) 자동 복구(재계산값 적용) 또는 차단(사용자 확인 대기) — 방침은 수정 세션에서 결정
4. P22(데이터 정합성) + P21(사용자 투명성) + P25(격리된 실패) 교차 준수

### 11.5 조사 완료 정의
- [x] 본 보고서 섹션 2 매트릭스에 모든 위반 누적 완료 (40건)
- [x] 교차 원칙 매트릭스 (11.2) 작성 완료 (P25 × P7/P9/P16/P20/P23 + P21/P22/P18)
- [x] 우선수정 추천 (11.3) 작성 완료 (Tier 1/2/3, 40건 영향도 순 정렬)
- [x] B5-08-03 기동 시 대조 메커니즘 조사 완료 (11.4) — 메커니즘 부재 확정
- [x] 각 위반에 대해 별도 수정 세션 승인 대기 상태로 이관
- [x] P25 전수 조사 9/9 세션 완료
...
| 2026-07-23 | 세션 8 | B5 매매·테스트모드 태스크 조사 완료. 위반 4건 식별 (B5-08-01~04). 섹션 2 매트릭스 + 섹션 10 결과 작성. MEDIUM 1건, LOW 3건. safe-trade 스킬 연계. P15 단일 주문 경로 준수 확인. P18 테스트모드 동등성 대체 준수 (B5-08-02 미세 위반). 매수/매도 실패 전파 양호 (사전 차감 롤백+서킷브레이커 강제 OFF). B5-08-03 (MEDIUM) fake_fill_event 태스크 실패 시 P22 정합성 잠재 위험이 가장 심각 (테스트는 존재). B5-08-01 create_task 직접 사용 2곳은 schedule_engine_task 통일 권장 |
| 2026-07-23 | 세션 9 | 교차 점검·총합 보고 완료. P25 전수 조사 9/9 세션 완료. 위반 40건 취합 (CRITICAL 2, HIGH 8, MEDIUM 14, LOW 16). 섹션 11 작성: (1) 위반 전체 목록 취합, (2) P25 × P7/P9/P16/P20/P23 교차 매트릭스 + 원칙별 집계 (P25 32건, P7 6건, P9 0건, P16 3건, P20 5건, P23 11건), (3) 우선수정 추천 Tier 1/2/3 (40건 영향도 순), (4) B5-08-03 기동 시 대조 메커니즘 조사 — **메커니즘 부재 확정** (settlement_engine.load_state는 SQLite 그대로 로드, trade_history 대조 없음, 불일치 영속화). 조사만 수행, 코드 수정 없음 |
