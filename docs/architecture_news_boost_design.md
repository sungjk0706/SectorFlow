...
매수 후보 선정은 2단계 구조:
1. **1차 필터**: 장마감 후 5거래일 평균 거래대금 기준 필터링 (매수후보 테이블에 올라온 종목만 2단계 대상)
...
1. **NWS 실시간 뉴스 가산점 추가**: 4대 가산점 완성 (5거래일 고가 · 호가비율 · 프.순.매 · 뉴스)
...
**UI 표시**: "실시간 뉴스 가산점" — 기존 3개(5거래일 고가 돌파, 호가 잔량 비율, 프로그램 순매수)와 동일 위계.
...
#### 4.4.1 컬럼 위치 — 5거래일 고가 컬럼 왼쪽

현재 매수후보 테이블 컬럼 순서 (12개):
```
순번 → 종목코드 → 종목명 → 현재가 → 전일대비 → 등락률 → 호가잔량비 → 프.순.매 → 5거래일 고가 → 가산점 → 제한 → 원인
```

뉴스 가산점 컬럼을 **5거래일 고가 컬럼(`high_5d`, 라인 100) 직전**에 삽입. 변경 후 순서 (13개):
```
순번 → 종목코드 → 종목명 → 현재가 → 전일대비 → 등락률 → 호가잔량비 → 프.순.매 → 📰뉴스 → 5거래일 고가 → 가산점 → 제한 → 원인
```
...
- 색상: `COLOR.up`(상승색) — 호재를 직관적으로 표현 (기존 5거래일 고가 돌파 ▲ 아이콘과 동일 색상, P23 일관성)

**주의**: 이모지 사용은 사용자가 명시적으로 요청한 UI 표시이므로 예외 허용. 기존 5거래일 고가 돌파 표시(`▲` 아이콘, 라인 35-43)와 동일한 "직관적 아이콘" 패턴 (P23).
...
| **P21 (사용자 투명성)** | ✅ | 매수설정에 가산점 토글/점수 UI. 일반설정 자동매매 탭에 키워드 편집 UI. 매수후보 테이블에 📰뉴스 컬럼(5거래일 고가 왼쪽)으로 가산점 부여 종목 한눈에 확인. 뉴스 가산점 부여 시 `logger.info()`로 종목+제목 로깅 |
| **P22 (데이터 정합성)** | ✅ | `news_boost_cache`는 파생 데이터(원본: NWS 패킷 + 키워드 사전). 중복 저장 없음. TTL 만료 시 자동 제거로 정합성 유지 |
| **P23 (일관성)** | ✅ | NWS 구독 = JIF 패턴. NWS 핸들러 = PGM 캐시 갱신 패턴. 가산점 UI = 기존 3개와 동일 패턴. 📰뉴스 컬럼 = 기존 5거래일 고가 ▲ 아이콘과 동일 직관적 아이콘 패턴. 키워드 편집 = 자동매매 탭 내 매매 설정 섹션과 동일 위계. 용어: "뉴스" (뉴스 가산점, 호재 키워드). `_TR_KOR` 딕셔너리 확장 |
...
| 매수후보 테이블 표시 | **📰뉴스 컬럼 신규 추가 (5거래일 고가 왼쪽)** | 사용자 보완 요청. 뉴스 가산점 부여 종목 한눈에 확인 |
...
3. **기존 공통 자산 확인**: 태그 칩 컴포넌트 존재 여부(`components/common/`에서 chip/tag/badge 패턴 검색), `buy-target.ts` 기존 컬럼 아이콘 패턴(5거래일 고가 ▲), `engine_state` 설정 동기화 기존 패턴, `createNumInput`/`createToggleLabelControlsRow` 재사용
