...
| COUPLING-S2 | P0 | C-02 설정 키 영향 매트릭스 | ☑ | `docs/coupling-settings-impact-matrix.md` 작성 (525줄). DEFAULT_USER_SETTINGS 66키 + DEFAULT_SYSTEM_CONFIG 17키 + 동적 증권사 자격증명 + 파생 키 전체 파이프라인(DB→기본값→정규화→캐시→서비스→API/UI) 매트릭스화. 후속 세션(2026-07-27)에서 P10 SSOT 위반 6건 중 5건 근본 해결 — #3 `max_position_size` 제거, #4 `telegram_on` 파생 제거, #5 마켓시간 14키 제거, #6 `boost_order_ratio_side`/`buy_interval_min` 레거시 제거, #2 `max_single_stock_exposure` 매트릭스·ARCHITECTURE.md 정정(살아있는 매수 차단 로직). #1 `max_daily_loss_limit`은 다음 세션 safe-trade 대상. DB 마이그레이션 3개 추가(16개 레거시 키 DB 삭제). 백엔드 2753 passed, RuntimeWarning 기동 정상(220ms, 16키 DB 삭제 로그 확인), 잔존 프로세스 0건. |
...
**상태:** ☑ 완료 (매트릭스 문서 작성 + 후속 SSOT 위반 5건 수정)
**대상 원칙:** P10 SSOT, P20 폴백 금지, P21 사용자 투명성, P22 데이터 정합성, P23 일관성, P24 단순성
**결과:** `docs/coupling-settings-impact-matrix.md`에 설정 키 전체 파이프라인 매트릭스 작성. 후속 세션(2026-07-27)에서 P10 SSOT 위반 6건 중 5건 근본 해결(#1 `max_daily_loss_limit`은 다음 세션 safe-trade 대상).
...
- P10 SSOT 위반 후보 6건 → 5건 수정 완료: (1) `max_daily_loss_limit`/`daily_loss_limit` 중복 — ☐ 다음 세션(safe-trade), (2) `max_single_stock_exposure` — **매트릭스 원 판정 "dead read" 부정확, 살아있는 매수 차단 로직** → 제거 금지 + 매트릭스·ARCHITECTURE.md 정정, `max_position_size` — ☑ 제거, (3) `tele_on`/`telegram_on` 복제 — ☑ `telegram_on` 파생 제거, (4) DEFAULT_SYSTEM_CONFIG 마켓시간 14키(원 "11키" 부정확) — ☑ 제거, (5) `boost_order_ratio_side` 레거시 — ☑ 제거, (6) `buy_interval_min` 레거시 — ☑ 제거.
...
- 단일화 우선순위 5건: (1) `sector_stock_layout` 원본 SSOT 명확화 — ☐, (2) `confirmed_data_broker` PATCH 후처리 추가 — ☐, (3) 레거시 리스크 3키 — ☑ 부분 완료(`max_position_size` 제거, `max_single_stock_exposure` 정정, `max_daily_loss_limit` 다음 세션), (4) `tele_on`/`telegram_on` 중복 제거 — ☑, (5) 수치 검증 누락 키 추가 — ☐.
