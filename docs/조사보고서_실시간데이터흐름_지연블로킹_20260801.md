...
> **작성일**: 2026-08-01
> **최종 갱신**: 2026-08-02 (코드베이스 정밀 검증 — 라인 드리프트 24건 수정, 구조적 오류 4건 정정, M-37 기각) + 2026-08-02 (master_stocks_cache 크기 정정 — "2000~2500개" 추정이 ka10099 전종목 수 오인, 실제 1337개 런타임 검증 완료, 본문 7곳 정정) + 2026-08-02 (설계서 반영 상태 갱신 — H-09/M-16 기각, 영향 미미 13건·구조 변경 보류 5건·검토 중 6건 상태 정정, §5 권장 조치 우선순위 기각 방향 제거)
...
**심각도별 발견 사항** (총 86건 — M-37 기각으로 87→86):

| 심각도 | 건수 | 정의 |
|---|---|---|
| high | 11 | 틱 핫패스 / 실시간 수신 경로 블로킹 |
| mid | 36 | 파이프라인 / 기동 / WS 브로드캐스트 경로 (M-37 기각 제외) |
| low | 39 | 드문 경로 / 스타일 / 이미 최적화 양호 |
| **합계** | **86** | — |

> 참고: 상태 파일 `next_action`에 "88개 / 40 low"로 기재되어 있으나, 실제 `findings` 배열 카운트는 87건 (high 11 + mid 37 + low 39). 2026-08-02 검증 결과 M-37 기각(실제 코드와 불일치)으로 유효 발견 사항은 86건.

**조치 상태별 분포** (갱신 포인트 — 보고서 항목 총수 87건 기준, M-37 ~표시 포함):

| 상태 | 건수 | 항목 |
|---|---|---|
| 신규 | 60 | 상태 미변경 항목 (후속 조치 미정) |
| 검토 중(설계 승인 완료 — 구현 대기) | 6 | H-05, H-06, H-07, H-08, M-12, M-23 |
| 수정 완료 | 0 | — |
...
| 기각(실제 코드와 불일치) | 3 | H-09, M-16, M-37 |

> 참고: §1.2 심각도별 발견 사항 86건은 M-37 기각 제외 기준. 본 상태별 분포 87건은 보고서 항목 총수(M-37 ~표시 포함) 기준. 유효 발견 사항(기각 3건 제외) = 84건.
...
#### H-06 · engine_account.py:360 · high · 검토 중(설계 승인 완료 — 구현 대기)
- **함수**: `_on_fill_after_ws`
- **설명**: 시세 체결 수신 틱 핫패스에서 `_refresh_account_snapshot_meta()` await + `auto_trade.check_sell_conditions()` await 직렬. `check_sell_conditions`는 전체 포지션 순회 매도 조건 검사. 시세 체결마다 매도 조건 전체 재검사로 틱 지연 가능.
- **정정 (2026-08-02 설계서 반영)**: 실제 개선 대상. 주문 체결 종목 한정 매도 점검으로 축소(설계서 결정 3). 사전조사로 01 시세 체결 누락 시나리오 없음 확인. 매도 판단 결과 동일성 보장. 4단계 — 모의투자 관찰 필수.
...
#### H-10 · ws_manager.py:204 · high · 보류(영향 미미 — 단일 사용자 앱 전제)
- **함수**: `_send_broadcast`
...
#### H-11 · ws_manager.py:217 · high · 보류(영향 미미 — 단일 사용자 앱 전제)
...
### 3.2 Mid (파이프라인 / 기동 / WS 브로드캐스트 경로) — 36건 (M-37 기각)
...
#### M-12 · engine_account.py:389 · mid · 검토 중(설계 승인 완료 — 구현 대기)
...
#### M-18 · engine_sector_confirm.py:286 · mid · 보류(영향 미미 — 단일 사용자 앱 전제)
...
#### M-22 · risk_manager.py:228 · mid · 보류(영향 미미 — 단일 사용자 앱 전제)
- **함수**: `check_buy_order_allowed`
...
#### M-23 · sector_data_provider.py:68 · mid · 검토 중(설계 승인 완료 — 구현 대기)
...
#### M-24 · sector_data_provider.py:183 · mid · 보류(영향 미미 — 단일 사용자 앱 전제)
...
#### M-25 · sector_data_provider.py:267 · mid · 신규
...
- **설명**: `trading_dates` 각각에 대해 `_buy_history` + `_sell_history` O(n) 순회 (dates × (buys+sells)). 추가로 per-date `get_base_asset_for_period` DB 조회. N거래일 × M건의 주문 체결 × 2 + N DB 조회. UI 요약 갱신 시 호출.
...
#### M-32 · trading.py:375 · mid · 보류(영향 미미 — 단일 사용자 앱 전제)
- **함수**: `_execute_buy_locked`
...
- **함수**: WS 연결 핸들러 (인라인 — `_send_initial_snapshot_delayed`라는 별도 함수 없음, 연결 핸들러 내 인라인 코드)
- **설명**: WS 연결 시 `get_all_sector_stocks()` + `build_initial_snapshot()` 등 다수 O(n) await 순차 실행. WS 연결 1회성이나 누적 지연.
...
#### ~M-37~ · ~~frontend/src/stores/hotStore.ts:250~~ · ~~mid~~ · 기각(실제 코드와 불일치)
- **함수**: ~~`applyAccountSummaryUpdate`~~
- **설명**: 검증 결과 `applyAccountSummaryUpdate`는 경량 payload로 `changed_positions` 루프가 없음 (snapshot만 갱신). 보고서에 기술된 `findIndex` O(n) 루프는 `applyAccountUpdate`(M-36)에만 존재. 중복 기술이므로 기각.
- **조치 상태**: 기각(실제 코드와 불일치) (2026-08-02 검증)
...
#### L-09 · buy_order_executor.py:127 · low · 보류(영향 미미 — 단일 사용자 앱 전제)
...
#### L-10 · buy_order_executor.py:59 · low · 신규
...
#### L-26 · frontend/src/components/common/data-table-virtual.ts:404 · low · 신규
- **함수**: `scheduleRender`
...
#### L-29 · frontend/src/pages/buy-target.ts:130 · low · 신규
...
#### L-30 · frontend/src/pages/buy-target.ts:378 · low · 신규
- **함수**: `renderTableRows`
...
#### L-32 · frontend/src/pages/profit-overview.ts:139 · low · 신규
...
#### L-35 · frontend/src/stores/hotStore.ts:506 · low · 신규
...
#### L-36 · frontend/src/stores/hotStore.ts:557 · low · 신규
...
#### L-37 · frontend/src/stores/hotStore.ts:602 · low · 신규
- **함수**: `applyNewsHit`
...
#### L-38 · frontend/src/stores/hotStore.ts:637 · low · 신규
...
| frontend/src/stores/hotStore.ts | 7 (유효 6) | M-36, ~M-37~, L-34, L-35, L-36, L-37, L-38 |
...
>
> **2026-08-02 설계서 반영 정정**: 설계서(`docs/설계서_실시간_지연_개선.md` §2 비목표)에서 기각한 방향(지연 스케줄, code→index 맵, 구독 종목 set, WS broadcast 병렬화, 배치/컨슈머 구조 변경, DB 쓰기 큐 구조 변경)을 아래에서 제거. 보류/기각 항목은 권장 조치에서 제외.
...
| H-06, H-07 | 주문 체결 알림(00) 중 매도 조건 전체 재검사 | 주문 체결 종목 한정 매도 검사 (지연 스케줄 기각) | 검토 중(구현 대기) |
...
- **M-23**: 업종 종목 표 복사 최적화 — 화면 필드명 불일치 해소 후 복사 축소. 검토 중(구현 대기, 3단계).
- **M-12**: 계좌 화면 거래내역 계산 — 메모리 기반 확인, 측정 후 조건부 분리. 검토 중(구현 대기, 6단계).
