...
> **작성일**: 2026-08-01
> **최종 갱신**: 2026-08-02 (코드베이스 정밀 검증 — 라인 드리프트 24건 수정, 구조적 오류 4건 정정, M-37 기각) + 2026-08-02 (master_stocks_cache 크기 정정 — "2000~2500개" 추정이 ka10099 전종목 수 오인, 실제 1337개 런타임 검증 완료, 본문 7곳 정정)
...
#### H-05 · engine_account.py:318 · high · 신규
...
#### H-06 · engine_account.py:360 · high · 신규
...
#### H-10 · ws_manager.py:204 · high · 신규
- **함수**: `_send_broadcast`
...
#### H-11 · ws_manager.py:217 · high · 신규
...
#### M-12 · engine_account.py:389 · mid · 신규
...
#### M-22 · risk_manager.py:228 · mid · 신규
- **함수**: `check_buy_order_allowed`
...
| H-06, H-07 | 주문 체결 알림(00) 중 매도 조건 전체 재검사 | 주문 체결 종목 한정 매도 검사 또는 지연 스케줄 |
| H-04 | broadcast 단일 컨슈머 | 큐 적체 모니터링 + 전송 병렬화 (H-10/H-11 연계) |

### 5.2 2순위 — 파이프라인/기동/WS 경로 (mid)

- **O(n) 전체 종목 스캔 군집** (M-15, M-16, M-18, M-20, M-23, M-24): `master_stocks_cache` 전체 순회 (n=1337, 필터링된 적격종목). 구독 종목 집합 별도 유지 시 O(구독 수)로 축소 가능 (P10 점검).
