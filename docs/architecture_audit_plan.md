...
> 작성일: 2026-07-11  
> 기준 문서: `ARCHITECTURE.md` (P1~P25, 총 25개 원칙)
...
### 1.1 목적
SectorFlow 전체 코드베이스를 `ARCHITECTURE.md`에 정의된 P1~P25, 총 25개 원칙 기준으로 체계적으로 점검하여, 아키텍처 위반 사항을 발견하고 근본 원인 해결 계획을 수립.
...
## 2. 원칙 평가 기준표 (P1~P25, 총 25개)

> 각 점검 항목에서 해당 원칙 번호(P1~P25)로 평가. 체크박스 ☐ = 미점검, ☑ = 준수, ☒ = 위반.
...
| P11 | 이벤트 기반 루프 | 내부 상태의 `while + sleep` 금지, 외부 long polling은 예외 조건 확인 |
...
| P24 | 단순성 | 더 단순한 대체 가능성, 불필요한 추상화, 길어지는 코드의 분할 검토, 중복 로직 검토 |
...
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 길어지는 코드의 분할 검토와 복잡도 참고 지표
...
| 파일 | 줄 수 | 규모 | 점검 완료 |
|------|-------|------|----------|
| `services/ws_subscribe_control.py` | 230 | 대형 | ☑ |
| `services/sector_data_provider.py` | 282 | 대형 | ☑ |
...
- [x] P11: 이벤트 기반
- [x] P13: 설정 메모리 조회
- [x] P16: dead code 없음 — B19-01/B19-02 no-op 2건 제거
- [x] P20: 폴백/silent except 없음 — B19-04 try/except 중복 제거
- [x] P23: 용어 사전 준수, 에러/비동기/네이밍/상수 패턴 파일 간 일관
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 길어지는 코드의 분할 검토와 복잡도 참고 지표 — B19-03 루프 중복 제거
...
|------|-------|------|----------|
| `main.ts` | 322 | 대형 | ☑ |
| `layout/header.ts` | 500 | 대형 | ☑ (분할 완료) |
| `router.ts` | 237 | 대형 | ☑ |
| `layout/shell.ts` | 168 | 중형 | ☑ |
| `layout/sidebar.ts` | 99 | 중형 | ☑ |
| `settings.ts` | 98 | 중형 | ☑ |

**원칙 체크리스트**:
- [x] P5: 직접 호출 체인 (이벤트 버스 없음) — 준수
- [x] P10: 전역 상태 SSOT 참조 — 준수
- [x] P16: dead code/미사용 라우트 없음 — 해결 (WebComponentPage, createGlobalWsBadge, settingsModuleCache 주석, router tail 주석, shell contentArea export)
- [x] P19: 비동기 초기화 누락 없음 — 준수
- [x] P21: 엔진 상태/연결 상태가 헤더에 표시됨 — 준수
- [x] P23: 용어 사전 준수, 에러/비동기/네이밍/상수 패턴 파일 간 일관 — 해결 (settings.ts "Python GC" 주석, main.ts 중복 번호)
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 길어지는 코드의 분할 검토와 복잡도 참고 지표 — 해결 (header.ts 519→500줄, onStateChange 분할)

---

### 세션 F-03: P2 — 핵심 매매 페이지 (업종순위/매수후보/보유종목) ◐ 부분완료
> **우선순위**: P2 (핵심 매매 UI)
> **상태**: 6건 해결 종결 (P16/P19/P23/P24). 잔여 보류 4건만 아래 명시.
...
| ID | 원칙 | 파일 | 상세 | 비고 |
|----|------|------|------|------|
| F03-07 | P20/P22 | `pages/sell-position.ts` | 다단 폴백 다수 (`p.buy_price ?? p.avg_price ?? 0`, `sectorStock?.cur_price ?? p.cur_price`, `state.positionCount ?? state.positions.length` 등) | 사용자 설계 로직 — 백엔드 전송 여부 확인 후 제안 |
| F03-08 | P24 | `pages/sector-stock.ts` (655줄), `pages/buy-target.ts` (539줄) | 파일 500줄 초과 — 분할 검토 | — |
| F03-09 | P24 | `pages/sector-stock.ts`, `pages/buy-target.ts` | 50줄 초과 함수 점검 — 함수 분할 검토 | — |
...
| 파일 | 줄 수 | 규모 | 점검 완료 |
|------|-------|------|----------|
| `pages/profit-overview.ts` | 175 (분할 후) | 대형 | ☑ (742줄→4개 파일 분할 175/62/219/377줄 + renderSectorStockPnl 146줄→5개 함수 분할) |
| `pages/profit-detail.ts` | 166 (분할 후) | 대형 | ☑ (674줄→4개 파일 분할 166/52/215/326줄) |
| `pages/profit-shared.ts` | 493 (분할 후) | 대형 | ☑ (598줄→493줄, 컬럼 정의 3개 profit-columns.ts 분할) |

**원칙 체크리스트**:
- [x] P5: 직접 호출 체인 — 준수
- [x] P10: 수익 데이터가 Store에서 관리됨 — 해결 (F05-05 buildSectorDonutRows shared SSOT)
- [x] P16: dead code 없음 — 해결 (F05-12 불필요 변수 제거)
- [x] P19: 비동기 데이터 로딩 누락 없음 — 해결 (F05-11 _applyDateRangeSeq 레이스 가드)
- [x] P22: 수익 데이터와 백엔드 정산 데이터 간 정합성 — 해결 (F05-01/F05-02 폴백 제거, 백엔드 accumulated_investment 누락 수정)
- [x] P23: 용어 사전 준수, 에러/비동기/네이밍/상수 패턴 파일 간 일관 — 해결 (F05-07 "보유주식"→"보유 종목", F05-06 filterTradeRows shared SSOT)
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 길어지는 코드의 분할 검토와 복잡도 참고 지표 — 해결 (F05-08/09/10 파일 분할 + mount 함수 분할)
...
| 파일 | 줄 수 | 규모 | 점검 완료 |
|------|-------|------|----------|
| `components/common/data-table.ts` | 176 (분할 후) | 대형 | ☑ (F06-01: 1045줄→176/454/454줄 분할) |
| `components/common/setting-row.ts` | 분할 후 | 대형 | ☑ (F06-02: 분할 완료) |
| `components/common/ui-styles.ts` | 252 (분할 후) | 대형 | ☑ (F06-03: 581줄→252/211/148줄 분할) |
...
| `components/common/toast.ts` | 189 | 중형 | ☑ (F06-12: rgba 하드코딩→hexToRgba 공통 헬퍼) |
| `components/common/create-slider.ts` | 173 | 중형 | ☑ (F06-11: #e9ecef→COLOR.inactiveBg) |
...
| `components/common/account-labels.ts` | 32 | 소형 | ☑ (F06-10: "보유주식"→"보유 종목" 6곳) |
...
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 길어지는 코드의 분할 검토와 복잡도 참고 지표 — 해결 (F06-01/02/03 대형 파일 3개 분할)
...
| **HIGH** | 아키텍처 원칙 위반 (P1~P3, P15, P16 등), 안전장치 미배선 |
...
| B13-03 | B-13 | `engine_settings.py` 61곳 | P10/P16 | LOW | `merged = {**DEFAULT_USER_SETTINGS, **flat}` 후에도 `_build_*` 함수들이 `merged.get(key, default)`로 기본값 하드코딩 — `settings_defaults.py`와 중복 (SSOT 위반). `merged`에 항상 값 존재하므로 `else default` 분기는 dead code | 해결 (B13-03 세션: 옵션 1 근본 해결 — 진입점 None 정규화(`flat_clean`)로 None 들어오는 근본 원인 차단 + 모든 하드코딩 기본값 `merged[key]` 직접 접근 전환 + None 방어 분기 전체 제거 + "None" 문자열/빈문자열 방어 제거(number 타입은 _parse_value가 int 파싱하므로 프로덕션 불가) + 누락 키 2개(`quote_auto_subscribe`/`confirmed_data_broker`) DEFAULT 추가 + `rebu_block_on` dead key → `rebuy_block_on` 올바른 키로 통일(원래 키 불일치 버그) + 테스트 의미 갱신. 2770 tests passed, 런타임 기동 172ms 정상) |
...
| F05-07 | F-05 | `profit-overview.ts:385,413`, `profit-shared.ts:502`, `account-labels.ts`, `sell-position.ts` | P23 | LOW | UI 텍스트/주석에 "보유주식" 사용 — 용어 사전 기준 "보유 종목". 프로젝트 전역 이슈 | 해결 (F-06-c: account-labels.ts 6곳 + sell-position.ts 6곳, F-06-d: profit-overview.ts 1곳 + profit-shared.ts 1곳. 잔여 "보유주식" 0건) |
| F05-08 | F-05 | 3개 파일 전체 | P24 | MEDIUM | 파일 길이 698/654/598줄 — P24 기준 500줄 초과 | 해결 (F-05-c: 컬럼 정의 3개 profit-columns.ts 분할, profit-shared.ts 598→493줄 + F-05-a 추가 세션: profit-overview.ts 742줄→4개 파일 분할 175/62/219/377줄 + F-05-b: profit-detail.ts 674줄→4개 파일 분할 166/52/215/326줄, 모두 500줄 이하) |
| F05-09 | F-05 | `profit-overview.ts:310-689` `mount()` | P24 | HIGH | 단일 함수 ~380줄 — P24 기준 50줄의 7.6배 | 해결 (F-05-b 2/2세션: 10개 헬퍼 분할, 모두 50줄 이하, 실전/테스트 루프 중복 제거 부수) |
...
| B-21 | P3 | 기타 Core 유틸 | ☑ 완료 (B-21-a journal 12건 + B-21-b logger+encryption 4건 + B-21-c classification+mapping+cache 3건 = 19건 P16) |
...
| F-03 | P2 | 핵심 매매 페이지 | ◐ 부분완료 (6건 해결 종결, 잔여 보류 4건 F03-07/08/09/10) |
| F-04 | P2 | 설정 페이지 | ☐ 미시작 |
...
| 진행 중 | 1 (F-03 부분완료 — 6건 해결 종결, 잔여 보류 4건) |
| 미시작 | 3 (B-13 부분완료, F-04 부분완료, F-07) |
| 발견된 문제 | 101 (79 + F05-01~12 + F03-07~10 보류 4 + F-03 해결 6) |
| 해결된 문제 | 87 (70 + F05-01~06, F05-11, F05-10, F05-12, F05-09, F05-07 + F-03 6건) |
| 보류된 문제 | 5 (B21-01, F03-07, F03-08, F03-09, F03-10) |
...
3. **25개 원칙에 대해 전체 코드베이스 위반 사항 0건 확인** (P1~P25, 총 25개)
