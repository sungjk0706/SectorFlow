...
> 작성일: 2026-07-11  
> 기준 문서: `ARCHITECTURE.md` (불변 원칙 25개 — 작성 당시 22개, 이후 P23~P25 확장)  
...
### 1.1 목적
SectorFlow 전체 코드베이스를 `ARCHITECTURE.md`에 정의된 불변 원칙(작성 당시 22개, 이후 25개로 확장) 기준으로 체계적으로 점검하여, 아키텍처 위반 사항을 발견하고 근본 원인 해결 계획을 수립.
...
## 2. 불변 원칙 평가 기준표 (작성 당시 22개 → 현재 25개)
...
- [x] P23: 용어 사전 준수, 에러/비동기/네이밍/상수 패턴 파일 간 일관
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 함수/파일 길이·복잡도 기준 — B19-03 루프 중복 제거
...
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 함수/파일 길이·복잡도 기준 — 해결 (header.ts 519→500줄, onStateChange 분할)
...
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 함수/파일 길이·복잡도 기준 — 해결 (F05-08/09/10 파일 분할 + mount 함수 분할)
...
- [x] P24: 더 단순한 대체 가능성, 불필요한 추상화, 함수/파일 길이·복잡도 기준 — 해결 (F06-01/02/03 대형 파일 3개 분할)
...
3. **25개 불변 원칙에 대해 전체 코드베이스 위반 사항 0건 확인** (작성 당시 22개 → 현재 25개)
